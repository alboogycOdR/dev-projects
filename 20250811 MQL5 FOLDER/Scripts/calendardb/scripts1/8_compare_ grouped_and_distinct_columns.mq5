//+------------------------------------------------------------------+
//|                      8_compare_ grouped_and_distinct_columns.mq5 |
...
//--- check a table
   ...
   if(db_obj.SelectTable(table_name))
      ...
      ::Print("\nMethod CDatabase::SelectFromGroupBy()");
      if(db_obj.PrintSqlRequest()<0)
         ::PrintFormat("Failed to print the SQL request, error %d", ::GetLastError());
      db_obj.FinalizeSqlRequest();
      if(!db_obj.SelectDistinctFrom(col_names_to_select))
         {
         db_obj.Close();
         return;
         }
      //--- print the SQL request
      ::Print("\nMethod CDatabase::SelectDistinctFrom()");
      if(db_obj.PrintSqlRequest()<0)
         ::PrintFormat("Failed to print the SQL request, error %d", ::GetLastError());
      db_obj.FinalizeSqlRequest();
      }
