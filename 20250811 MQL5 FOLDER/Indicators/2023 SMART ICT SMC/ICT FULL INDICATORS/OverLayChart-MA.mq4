...
extern string SubSymbol = "GBPUSDm";
...
extern bool ShowMA = false;

extern color MA_color = CornflowerBlue;
extern int MA_line_height = 1;
...
int init() {
   ...
   if (ShowCandles == true) {
      ...
      SetIndexStyle( 0, DRAW_NONE, DRAW_NONE, 1, BullBarColor );
      SetIndexStyle( 1, DRAW_NONE, DRAW_NONE, 1, BearBarColor );
      SetIndexStyle ( 2, DRAW_NONE, DRAW_NONE, 3, BullBarColor );
      SetIndexStyle( 3, DRAW_NONE, DRAW_NONE, 3, BearBarColor );
   }
   if (ShowMA == true) {
      SetIndexStyle( 4, DRAW_LINE, DRAW_LINE, MA_line_height, MA_color );
   } else {
      SetIndexStyle( 4, DRAW_NONE, DRAW_NONE, MA_line_height, MA_color );
   }
...
int start() {
   ...
   //Draw Candlesticks
	for ( _i = _LastBar; _i < _LastBar + _BarsCount; _i ++ ) {
      ...
      if (StringChangeToUpperCase(MA_price) == "CLOSE") {
         MA[_i] = _CurRangeCenter + _SubClose * _PipsRatio;
      } else if (StringChangeToUpperCase(MA_price) == "OPEN") {
         MA[_i] = _CurRangeCenter + _SubOpen * _PipsRatio;
      } else if (StringChangeToUpperCase(MA_price) == "HIGH") {
         MA[_i] = _CurRangeCenter + _SubHigh * _PipsRatio;
      } else {
         MA[_i] = _CurRangeCenter + _SubLow * _PipsRatio;
      }
   }
   ...
   ObjectSetText( Prefix + "Copyright", "OverLay Chart by S.B.T./MA by LEGRUPO", 8 );
