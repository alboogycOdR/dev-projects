...
input double lvl01=0;
input double lvl02=0;
input double lvl03=0;
input double lvl04=0;
input double lvl05=0;
input double lvl06=0;
input double lvl07=0;
input double lvl08=0;
 double leveldeviationUnder=0.2;//Sensitivity: deviation under
 double leveldeviationOver=0.5;//Sensitivity: deviation over
...
input bool                       EnableYellowGreenCrossAlert = true; // EnableMBL_RSI_PRICE_LINECross alerts.
input group "-other-";
...
int OnInit()
  {
// Check if at least one level is not 0.0
   if(lvl01 == 0.0 && lvl02 == 0.0 && lvl03 == 0.0 && lvl04 == 0.0 && lvl05 == 0.0 && lvl06 == 0.0 && lvl07 == 0.0 && lvl08 == 0.0)
     {
      Print("Error: At least one level must be set to a non-zero value.");
      Alert("Error: At least one level must be set to a non-zero value.");
...
datetime lastAlertTimeMBL = 0;


//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void CROSSING_OF_MBL_EQUAL()
  {
   double currentValue = NormalizeDouble(MARKET_BASE_LINE[TriggerCandle],1);
   for(int i = 0; i < ArraySize(levels); i++)
     {
      //int LVL_VALUE=NormalizeDouble(levels[i],1);
      double LVL_VALUE=NormalizeDouble(levels[i],1);
      if(currentValue >= LVL_VALUE-leveldeviationUnder && currentValue <= LVL_VALUE+leveldeviationOver)            // Level matched
        {
         string alertMessage = "MARKET_BASE_LINE touch " + (string)LVL_VALUE +
                               " on " + _Symbol + "("+PeriodToString(Period())+")" +
                               " at " + TimeToString(TimeCurrent(), TIME_MINUTES);
         if(EnableNativeAlerts && (TimeCurrent() - lastAlertTimeMBL >= 60))
           {
            Alert(alertMessage);
           }
         if(EnableEmailAlerts && (TimeCurrent() - lastAlertTimeMBL >= 60))
           {
            SendMail("TDI: " + alertMessage + " - " + _Symbol + " @ " + PeriodToString(_Period),
                     "Current rate = " + DoubleToString(SymbolInfoDouble(_Symbol, SYMBOL_BID), _Digits) + "/" +
                     DoubleToString(SymbolInfoDouble(_Symbol, SYMBOL_ASK), _Digits) +
         ...
         if(EnablePushAlerts && (TimeCurrent() - lastAlertTimeMBL >= 60))
           {
            SendNotification("TDI: " + alertMessage + " - " + _Symbol + " @ " + PeriodToString(_Period));
           }
         lastAlertTimeMBL=TimeCurrent();
        }
     }
  }
datetime lastAlertTimeRSI_PRICE_LINE = 0; // Variable to store the last alert time
void CROSSING_OF_RSI_PRICE_LINE_EQUAL()
  {
   double currentValue = NormalizeDouble(RSI_PRICE_LINE[TriggerCandle],1);
   for(int i = 0; i < ArraySize(levels); i++)
     {
      double LVL_VALUE=NormalizeDouble(levels[i],1);
      if(currentValue >= LVL_VALUE-leveldeviationUnder && currentValue <= LVL_VALUE+leveldeviationOver) 
        {
         // Level matched
         string alertMessage = "RSI_PRICE_LINE touch " + (string)LVL_VALUE +
                               " on " + _Symbol + "("+PeriodToString(Period())+")" +
                               " at " + TimeToString(TimeCurrent(), TIME_MINUTES);
         if(EnableNativeAlerts&& (TimeCurrent() - lastAlertTimeRSI_PRICE_LINE >= 60))
           {
            Alert(alertMessage);
           }
         if(EnableEmailAlerts && (TimeCurrent() - lastAlertTimeRSI_PRICE_LINE >= 60))
           {
            SendMail("TDI: " + alertMessage + " - " + _Symbol + " @ " + PeriodToString(_Period),
                     "Current rate = " + DoubleToString(SymbolInfoDouble(_Symbol, SYMBOL_BID), _Digits) + "/" +
                     DoubleToString(SymbolInfoDouble(_Symbol, SYMBOL_ASK), _Digits) +
         ...
         if(EnablePushAlerts && (TimeCurrent() - lastAlertTimeRSI_PRICE_LINE >= 60))
            SendNotification("TDI: " + alertMessage + " - " + _Symbol + " @ " + PeriodToString(_Period));
         // Add your alert or other actions here
         lastAlertTimeRSI_PRICE_LINE = TimeCurrent(); // Update the last alert time
        }
     }
  }
...
// Too few bars to work with.
   ...
   if(EnableCrossingOfMBL)
      CROSSING_OF_MBL_EQUAL();
//=======================================================
   if(EnableCrossingOfRSI_PRICE_LINE)
      CROSSING_OF_RSI_PRICE_LINE_EQUAL();
//=======================================================
//removed
// if(isCrash) {
...
//HIT THE FLOOR, HIT THE CEILING
   if((MARKET_BASE_LINE[TriggerCandle  ] > 100) && (MARKET_BASE_LINE[TriggerCandle + 1] <= 100) && (AlertPlayed != Time[0]))
     {
      ...
      if(EnableEmailAlerts)
        {
         SendMail("TDI Alert:Market Base Line crossed 100 " + _Symbol + " @ " + PeriodToString(_Period), "Current rate = " + DoubleToString(SymbolInfoDouble(_Symbol, SYMBOL_BID), _Digits) + "/" + DoubleToString(SymbolInfoDouble(_Symbol, SYMBOL_ASK), _Digits) + "\nIndicator buffers:\nVB High = " + DoubleToString(UpZone[TriggerCandle], 1) + "\nMarket Base Line = " + DoubleToString(MARKET_BASE_LINE[TriggerCandle], 1) + "\nVB Low = " + DoubleToString(DnZone[TriggerCandle], 1) + "\nRSI Price Line = " + DoubleToString(RSI_PRICE_LINE[TriggerCandle], 1) + "\nTrade Signal Line = " + DoubleToString(TRADE_SGNL_LINE[TriggerCandle], 1));
        }
      if(EnablePushAlerts)
         SendNotification("TDI Alert:Market Base Line crossed 100 " + _Symbol + " @ " + PeriodToString(_Period));
      AlertPlayed = Time[0];
     }
//HIT THE FLOOR, HIT THE CEILING
   if((MARKET_BASE_LINE[TriggerCandle  ] < 0) && (MARKET_BASE_LINE[TriggerCandle + 1] >= 0) && (AlertPlayed != Time[0]))
     {
   ...
   if(EnableHookAlert)
     {
      ...
      // Green line crosses lower blue line from below when both are below level 32.
         ...
         if((RSI_PRICE_LINE[TriggerCandle] > DnZone[TriggerCandle]) && (RSI_PRICE_LINE[TriggerCandle + 1] <= DnZone[TriggerCandle + 1]) && ((RSI_PRICE_LINE[TriggerCandle] < 32) || (RSI_PRICE_LINE[TriggerCandle + 1] < 32)) && ((DnZone[TriggerCandle] < 32) || (DnZone[TriggerCandle + 1] < 32)) && (HookAlertPlayed != Time[0]))
           {
            if(EnableNativeAlerts)
               Alert("Bullish Hook");
            if(EnableEmailAlerts)
         ...
         if(EnableHookAlert)
           {
            ...
            // Green line crosses lower blue line from below when both are below level 32.
               ...
               if((RSI_PL[i + TriggerCandle] > dnzone[i + TriggerCandle]) && (RSI_PL[i + TriggerCandle + 1] <= dnzone[i + TriggerCandle + 1]) && ((RSI_PL[i + TriggerCandle] < 32) || (RSI_PL[i + TriggerCandle + 1] < 32)) && ((dnzone[i + TriggerCandle] < 32) || (dnzone[i + TriggerCandle + 1] < 32)))
                 {
