...
input color ibull_ob_css = C'49,121,245';
input color ibear_ob_css = C'247,124,128';
input color bull_ob_css = C'24,72,204';
input color bear_ob_css = C'178,40,51';
...
input color bull_fvg_css = C'0,255,104';
input color bear_fvg_css = C'255,0,8';
input int fvg_extend = 1;

// Previous day/week high/low
input bool show_pdhl = false;
input string pdhl_style = "⎯⎯⎯";
input color pdhl_css = C'33,87,243';
input bool show_pwhl = false;
input string pwhl_style = "⎯⎯⎯";
input color pwhl_css = C'33,87,243';
input bool show_pmhl = false;
input string pmhl_style = "⎯⎯⎯";
input color pmhl_css = C'33,87,243';

// Premium/Discount zones
input bool show_sd = false;
input color premium_css = C'242,54,69';
input color eq_css = C'178,181,190';
input color discount_css = C'8,153,129';
...
{
    string lineName = "StructureLine_" + IntegerToString(x);
    string labelName = "StructureLabel_" + IntegerToString(x);
    ...
    ObjectSetInteger(0, labelName, OBJPROP_XDISTANCE, 0);
    ObjectSetInteger(0, labelName, OBJPROP_YDISTANCE, 0);
    ObjectSetInteger(0, labelName, OBJPROP_CORNER, down ? CORNER_LEFT_LOWER : CORNER_LEFT_UPPER);
...
void swings(int len, double &top, double &btm)
{
    int os = 0;
    
    double upper = High[ArrayMaximum(High, 0, len)];
    double lower = Low[ArrayMinimum(Low, 0, len)];
...
void ob_coord(bool use_max, int loc, double &target_top[], double &target_btm[], datetime &target_left[], int &target_type[])
{
    ...
    double ob_threshold[];
    ArrayResize(ob_threshold, Bars(_Symbol, PERIOD_CURRENT));

    if(ob_filter == "Atr")
    {
        for(int i = 0; i < Bars(_Symbol, PERIOD_CURRENT) - loc - 1; i++)
        {
    ...
    ArrayResize(target_top, ArraySize(target_top) + 1);
    ArrayResize(target_btm, ArraySize(target_btm) + 1);
    ArrayResize(target_left, ArraySize(target_left) + 1);
    ArrayResize(target_type, ArraySize(target_type) + 1);
    
    target_top[ArraySize(target_top) - 1] = max;
    target_btm[ArraySize(target_btm) - 1] = min;
    target_left[ArraySize(target_left) - 1] = Time[idx];
    target_type[ArraySize(target_type) - 1] = use_max ? -1 : 1;
}

// Set order blocks
void display_ob(int &boxes[], double &target_top[], double &target_btm[], datetime &target_left[], int &target_type[], int show_last, bool swing, int size)
{
    for(int i = 0; i < MathMin(show_last - 1, size - 1); i++)
    {
        string boxName = "OrderBlock_" + IntegerToString(i);
        
        ObjectCreate(0, boxName, OBJ_RECTANGLE, 0, target_left[i], target_top[i], Time[0], target_btm[i]);
        ObjectSetInteger(0, boxName, OBJPROP_STYLE, swing ? STYLE_SOLID : STYLE_SOLID);
        ...
        if(swing)
        {
            if(style == Monochrome)
            {
                css = target_type[i] == 1 ? C'178,181,190' : C'93,96,107';
            }
            else
            {
...
{
    string highLineName = "PHL_High_" + EnumToString(tf);
    string highLabelName = "PHL_High_Label_" + EnumToString(tf);
    string lowLineName = "PHL_Low_" + EnumToString(tf);
    string lowLabelName = "PHL_Low_Label_" + EnumToString(tf);
    
    // High line and label
    ObjectCreate(0, highLineName, OBJ_TREND, 0, Time[0], h, Time[0] + PeriodSeconds(tf) * 20, h);
    ObjectSetInteger(0, highLineName, OBJPROP_STYLE, get_line_style(pdhl_style));
    ObjectSetInteger(0, highLineName, OBJPROP_COLOR, css);
    
    ObjectCreate(0, highLabelName, OBJ_LABEL, 0, 0, 0);
    ObjectSetInteger(0, highLabelName, OBJPROP_XDISTANCE, 0);
    ObjectSetInteger(0, highLabelName, OBJPROP_YDISTANCE, 0);
    ObjectSetInteger(0, highLabelName, OBJPROP_CORNER, CORNER_LEFT_UPPER);
    ObjectSetString(0, highLabelName, OBJPROP_TEXT, "P" + EnumToString(tf) + "H");
    ObjectSetInteger(0, highLabelName, OBJPROP_COLOR, css);
    ObjectSetInteger(0, highLabelName, OBJPROP_FONTSIZE, 8);
    ObjectSetDouble(0, highLabelName, OBJPROP_PRICE, h);
    
    // Low line and label
    ObjectCreate(0, lowLineName, OBJ_TREND, 0, Time[0], l, Time[0] + PeriodSeconds(tf) * 20, l);
    ObjectSetInteger(0, lowLineName, OBJPROP_STYLE, get_line_style(pdhl_style));
    ObjectSetInteger(0, lowLineName, OBJPROP_COLOR, css);
    
    ObjectCreate(0, lowLabelName, OBJ_LABEL, 0, 0, 0);
    ObjectSetInteger(0, lowLabelName, OBJPROP_XDISTANCE, 0);
    ObjectSetInteger(0, lowLabelName, OBJPROP_YDISTANCE, 0);
    ObjectSetInteger(0, lowLabelName, OBJPROP_CORNER, CORNER_LEFT_UPPER);
    ObjectSetString(0, lowLabelName, OBJPROP_TEXT, "P" + EnumToString(tf) + "L");
    ObjectSetInteger(0, lowLabelName, OBJPROP_COLOR, css);
    ObjectSetInteger(0, lowLabelName, OBJPROP_FONTSIZE, 8);
    ObjectSetDouble(0, lowLabelName, OBJPROP_PRICE, l);
}
...
int OnInit()
{
    ...
    SetIndexBuffer(7, SwingHighBuffer, INDICATOR_CALCULATIONS);
    SetIndexBuffer(8, SwingLowBuffer, INDICATOR_CALCULATIONS);
                ...
                const int &spread[])
{
    ...
    CopyHigh(_Symbol, PERIOD_CURRENT, 0, rates_total, HighBuffer);
    CopyLow(_Symbol, PERIOD_CURRENT, 0, rates_total, LowBuffer);
    CopyClose(_Symbol, PERIOD_CURRENT, 0, rates_total, CloseBuffer);
    CopyOpen(_Symbol, PERIOD_CURRENT, 0, rates_total, OpenBuffer);
    CopyTickVolume(_Symbol, PERIOD_CURRENT, 0, rates_total, VolumeBuffer);
    CopyTime(_Symbol, PERIOD_CURRENT, 0, rates_total, TimeBuffer);
    
    // Calculate ATR
    for(int i = 0; i < rates_total; i++)
    {
        ATRBuffer[i] = iATR(_Symbol, PERIOD_CURRENT, 14, i);
    }
    ...
    // ... rest of your existing OnCalculate code ...
...
void OnDeinit(const int reason)
{
    ...
    ObjectsDeleteAll(0, "PHL_High_");
    ObjectsDeleteAll(0, "PHL_High_Label_");
    ObjectsDeleteAll(0, "PHL_Low_");
    ObjectsDeleteAll(0, "PHL_Low_Label_");
}
