...
//+------------------------------------------------------------------+
		void SpotLight(string szObjectName = NULL)
			{
				if (m_MemNameObj != NULL) ObjectSetInteger(Terminal.Get_ID(), m_MemNameObj, OBJPROP_YSIZE, 1);
				if (szObjectName != NULL) ObjectSetInteger(Terminal.Get_ID(), szObjectName, OBJPROP_YSIZE, (szObjectName != NULL ? 3 : 1));
				m_MemNameObj = szObjectName;
			};
//+------------------------------------------------------------------+
virtual void PositionAxleY(string szObjectName, int Y)
			{
				int desly = (m_MemNameObj == szObjectName ? 1 : 0);
