   ...
   bool IsValid(string symbol,int period, int bar)
   {
      ...
      switch(_period)
     {
      ...
      if (IsInsideBar(bar+2))
      {
         ...
         {
            if (iHigh(_symbol, m_period, bar+1) < iHigh(_symbol, m_period, bar+2) && iHigh(_symbol, m_period, bar+1) < iHigh(_symbol, m_period, bar+3))
            {
               if ( iLow(_symbol, m_period, bar) > iHigh(_symbol, m_period, bar+1) && iHigh(_symbol, m_period, bar) > iHigh(_symbol, m_period, bar+1))
               {
                  Print("found fakey ",bar," ", iTime(_symbol, m_period, bar));
                  return true;
               }
            }
            ...
            if (iHigh(_symbol, m_period, bar+1) > iHigh(_symbol, m_period, bar+2) && iHigh(_symbol, m_period, bar+1) > iHigh(_symbol, m_period, bar+3))
            {
               if ( iHigh(_symbol, m_period, bar) < iHigh(_symbol, m_period, bar+1) && iHigh(_symbol, m_period, bar) < iHigh(_symbol, m_period, bar+1))
               {
                  Print("found fakey ",bar," ", iTime(_symbol, m_period, bar));
                  return true;
               }
