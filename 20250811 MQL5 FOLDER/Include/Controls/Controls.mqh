...
CWavResource::CWavResource(void)
  {
   Player=NULL;
   if((Player=new CWAVbox)==NULL)
      Print("::CWavResource create error");
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
CResources::CResources(void)
  {
   if((ResourcesList=new CArrayObj)==NULL)
      Print("::CResources create error");
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
   ...
   for(int i=0;i<ResourcesList.Total();i++)
     {
      ...
      if(current.Type()!=RS_FONT)
         continue;
      else
        {
         res=current;
         if(res.ResourcesName()==f_Name)
            return(res);
        }
     }
   return(NULL);
...
bool CControl::Visible(bool f_State)
  {
   ...
   if(f_State)
      result&=TimeFrames(m_LastTimeFramesFlag);
   else
     {
...
bool CControl::SaveControlState(int f_file_handle)
  {
   ...
   if(FileWriteLong(f_file_handle,-1)!=sizeof(long))
      return(false);
   if(FileWriteInteger(f_file_handle,m_Type,INT_VALUE)!=INT_VALUE)
      return(false);

   str=m_Name;
   len=StringLen(str);
   if(FileWriteInteger(f_file_handle,len,INT_VALUE)!=INT_VALUE)
      return(false);
   if(len!=0)
      if(FileWriteString(f_file_handle,str,len)!=len)
         return(false);

   if(FileWriteInteger(f_file_handle,(int)m_DrawOnBackground,CHAR_VALUE)!=sizeof(char))
      return(false);
   if(FileWriteInteger(f_file_handle,(int)m_Selectable,CHAR_VALUE)!=sizeof(char))
      return(false);
   if(FileWriteInteger(f_file_handle,(int)m_TimeFramesFlag,INT_VALUE)!=sizeof(int))
      return(false);
   if(FileWriteInteger(f_file_handle,(int)m_Corner,INT_VALUE)!=sizeof(int))
      return(false);
   return(true);
  }
//+------------------------------------------------------------------+
...
bool CControl::LoadControlState(int f_file_handle,string f_Name,ENUM_OBJECT f_Type)
  {
   ...
   if(FileReadLong(f_file_handle)!=-1)
      return(false);
   if(FileReadInteger(f_file_handle,INT_VALUE)!=f_Type)
      return(false);
   len=FileReadInteger(f_file_handle,INT_VALUE);
   if(len!=0)
      str=FileReadString(f_file_handle,len);
   else
      str="";
   if(str!=f_Name)
      return(false);
...
CFrame::~CFrame(void)
  {
   if(CtrlsContainer!=NULL)
      delete CtrlsContainer;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
bool CFrame::Create(CContainer *p_Container=NULL,CResources *p_rs=NULL)
  {
   ...
   if(p_Container==NULL)
     {
      if((CtrlsContainer=new CContainer)==NULL)
         return(false);
     }
   else
      CtrlsContainer=p_Container;
...
bool CFrame::Visible(bool f_State)
  {
   ...
   if(_total>0)
      result&=CtrlsContainer.Visible(f_State);
   if(result)
     {
      m_Visible=f_State;
      OnVisible();
...
bool CFrame::Enabled(bool f_State)
  {
   ...
   int _total=CtrlsContainer.TotalItems();
   if(_total>0)
      result&=CtrlsContainer.Enabled(f_State);
   if(result)
     {
      m_Enabled=f_State;
      OnEnable();
...
bool CFrame::Hide(CArrayString *p_Exclusion=NULL)
  {
   ...
   int _total=CtrlsContainer.TotalItems();
   if(_total>0)
      result&=(p_Exclusion!=NULL)?CtrlsContainer.Visible(false,p_Exclusion):CtrlsContainer.Visible(false);
   return(result);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
bool CContainer::Enabled(bool f_State)
  {
   ...
   for(int i=0;i<_total;i++)
     {
      if((Control=ControlsList.At(i))!=NULL)
         result&=Control.Enabled(f_State);
     }
   if(result)
      m_Enabled=f_State;
   return(result);
   ...
   if(_total>0)
     {
      for(int i=0;i<_total;i++)
        {
         if((Control=ControlsList.At(i))!=NULL)
            result&=Control.ChangeControlPosition(f_TopShift,f_LeftShift);
        }
     }
   return(result);
  }
...
bool CGroup::Active(int f_Index)
  {
   ...
   for(int i=0;i<Controls.Total();i++)
     {
      obj=Controls.At(i);
      if(obj!=NULL && obj.Index()!=f_Index && obj.Active() && obj.Active(false))
         break;
     }
   return(true);
  }
   ...
   if(result)
     {
      ...
      if(len!=0)
         if(FileWriteString(f_file_handle,str,len)!=len)
            return(false);
      if(FileWriteInteger(f_file_handle,Color,INT_VALUE)!=INT_VALUE)
         return(false);
      if(FileWriteDouble(f_file_handle,Angle)!=sizeof(double))
         return(false);
      if(FileWriteInteger(f_file_handle,AnchorPoint,INT_VALUE)!=INT_VALUE)
         return(false);
     }
   return(result);
  }
...
bool CLabel::LoadControlState(int f_file_handle,string f_Name,ENUM_OBJECT f_Type)
  {
   ...
   len=FileReadInteger(f_file_handle,INT_VALUE);
   if(len!=0)
      Caption=FileReadString(f_file_handle,len);
   else
      Caption="";
   ...
   if(result)
     {
      ...
      if(result)
        {
         m_Enabled=f_State;
         if(m_objCaption!=NULL && m_FontColorDisabled!=CLR_NONE)
            m_objCaption.Enabled(m_Enabled);
        }
     }
   return(result);
  }
   ...
   if(m_objCaption!=NULL)
     {
      ...
      if(!result)
         Print("CRadioControl::Set font error");
     }
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
bool CRadioControl::ClickArea(int f_Top_Click,int f_Left_Click)
  {
   ...
   if((f_Top_Click>=objTop+5 && f_Top_Click<=objTop+m_ControlHeight) &&
      (f_Left_Click>=objLeft+5 && f_Left_Click<=objLeft+m_ControlWidth))
      return(true);
   else
      return(false);
  }
   ...
   if((Resources=p_ResObj)!=NULL && f_RS_Name!=" " && f_RS_Font!=" ")
     {
      ...
      if(!result)
         result=CControl::Create(f_Name,OBJ_BITMAP_LABEL,f_Top,f_Left);
...
bool CCheckbox::SaveControlState(int f_file_handle)
  {
   ...
   if(result)
     {
      if(FileWriteLong(f_file_handle,ObjectGetInteger(m_Chart,m_Name,OBJPROP_STATE))!=sizeof(long))
         return(false);
     }
   else
      return(false);
...
bool CCheckbox::LoadControlState(int f_file_handle,string f_Name,ENUM_OBJECT f_Type)
  {
   ...
   if(result)
     {
      if(!Checked(FileReadLong(f_file_handle)))
         return(false);
     }
   else
      return(false);
...
bool CButton::SetImgs(string f_FileNameOn,string f_FileNameOff=" ",string f_FileNameDis="")
  {
   ...
   BmpOn=f_FileNameOn;
   if(f_FileNameOff!=" ")
      BmpOff=f_FileNameOff;
   if(f_FileNameDis!=" ")
      BmpDis=f_FileNameDis;
   ...
   if(m_objCaption!=NULL)
     {
      ...
      if(IsDown)
         m_objCaption.ChangeControlPosition(BTN_CAPTION_SHIFT,BTN_CAPTION_SHIFT);
      else
         m_objCaption.ChangeControlPosition(-BTN_CAPTION_SHIFT,-BTN_CAPTION_SHIFT);
      result&=m_objCaption.SetColor(_clr);
     }
   ...
   if(result)
     {
      IsEnabled=f_State;
      if(m_objCaption!=NULL)
         m_objCaption.Enabled(IsEnabled);
     }
   return(result);
  }
//+------------------------------------------------------------------+
...
bool CButton::ClickArea(int f_Top_Click,int f_Left_Click)
  {
   ...
   if((f_Top_Click>=objTop+5 && f_Top_Click<=objTop+m_btnHeight) &&
      (f_Left_Click>=objLeft+5 && f_Left_Click<=objLeft+m_btnWidth))
      return(true);
   else
      return(false);
  }
...
CTradeButton::~CTradeButton(void)
  {
   if(Caption.objPrice!=NULL)
      delete Caption.objPrice;
   if(Caption.objPoints!=NULL)
      delete Caption.objPoints;
   if(Caption.objPips!=NULL)
      delete Caption.objPips;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
bool CTradeButton::ChangeFontColor(color f_Clr)
  {
   bool result=true;
   if(Caption.objPrice!=NULL)
      result&=Caption.objPrice.SetColor(f_Clr);
   if(Caption.objPoints!=NULL)
      result&=Caption.objPoints.SetColor(f_Clr);
   if(Caption.objPips!=NULL)
      result&=Caption.objPips.SetColor(f_Clr);
   if(result)
      m_LastColor=f_Clr;
   return(result);
  }
   ...
   if((Label=new CCaption)!=NULL)
     {
      ...
      if(f_Size==LBL_LARGE)
         result&=Label.SetFont(m_FontName,m_FontSizeL);
      else
         result&=Label.SetFont(m_FontName,m_FontSizeS);
      result&=Label.SetCaption(" ");
      if(!Visible())
         result&=Label.TimeFrames(OBJ_NO_PERIODS);
      if(!result)
        {
...
bool CTradeButton::Down(bool f_State)
  {
   ...
   if(IsDown)
      result&=ChangeMatrixPosition(BTN_CAPTION_SHIFT,BTN_CAPTION_SHIFT);
   else
      result&=ChangeMatrixPosition(-BTN_CAPTION_SHIFT,-BTN_CAPTION_SHIFT);
   ...
   if(f_State)
     {
      ...
      result&=ChangeFontColor(m_DisabledFontColor);
      ...
      case BTN_FACE_PRICEUP:
        {
         ...
         if(m_LastColor!=m_NormalFontColor)
            result&=ChangeFontColor(m_NormalFontColor);
        }
      break;
      ...
      case BTN_FACE_PRICENONE:
        {
         ...
         if(m_LastColor!=m_PNFontColor)
            result&=ChangeFontColor(m_PNFontColor);
        }
      break;

      default:
         if(!IsEnabled && m_LastColor!=m_DisabledFontColor)
            result&=ChangeFontColor(m_DisabledFontColor);
         break;

     }
   m_LastPriceChange=f_PriceChange;
   if(f_digits!=UCHAR_MAX)
      result&=SetCaption(f_price,f_digits);
   return(result);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
void CImage::SetImgs(string f_FileNameEnabled,string f_FileNameDisabled=" ")
  {
   if(f_FileNameEnabled!=" ")
      m_BmpEnabled=f_FileNameEnabled;
   if(f_FileNameDisabled!=" ")
      m_BmpDisabled=f_FileNameDisabled;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
bool CImage::Enabled(bool f_State)
  {
   ...
   if(result)
     {
      if(f_State)
         result&=Image(m_BmpEnabled);
      else
         result&=Image(m_BmpDisabled);
     }
   if(result)
      m_Enabled=f_State;
   return(result);
...
int CPanel::GetX(Align &p_align) const
  {
   ...
   switch(p_align.h_Align)
     {
      case H_ALIGN_CUSTOM_POS:
         f_Left=p_align.Left;
         break;
      case H_ALIGN_LEFT_POS:
         f_Left=1;
         break;
      case H_ALIGN_RIGHT_POS:
         f_Left=ChartWidth-Width();
         break;
      case H_ALIGN_MIDLLE_POS:
         f_Left=ChartMX-(int)round(Width()/2);
         break;
      default:
         break;
     }
...
int CPanel::GetY(Align &p_align) const
  {
   ...
   switch(p_align.v_Align)
     {
      case V_ALIGN_CUSTOM_POS:
         f_Top=p_align.Top;
         break;
      case V_ALIGN_TOP_POS:
         f_Top=1;
         break;
      case V_ALIGN_BOTTOM_POS:
         f_Top=ChartHeigth-Height();
         break;
      case V_ALIGN_MIDLLE_POS:
         f_Top=ChartMY-(int)round(Height()/2);
         break;
      default:
         break;
     }
   ...
   if(result)
     {
      ...
      Print("Error "+DoubleToString(GetLastError(),0));
   return(result);
  }
//+------------------------------------------------------------------+
//|                                                                  |
   ...
   if(result)
     {
      string str;
      int len;
      if(FileWriteInteger(f_file_handle,(int)ObjectGetInteger(m_Chart,m_Name,OBJPROP_READONLY),CHAR_VALUE)!=sizeof(char))
         return(false);
      str=Text();
      len=StringLen(str);
      if(FileWriteInteger(f_file_handle,len,INT_VALUE)!=INT_VALUE)
   ...
   if(result)
     {
      ...
      if(len!=0)
         ...
         str="";
      if(!ObjectSetString(m_Chart,m_Name,OBJPROP_TEXT,str))
         return(false);
     }
   else
     {
   ...
   class CIncSpinButton:public CSpinButton
     {
      ...
      public:
          ...
          virtual bool      Parent(CSpinEdit *p_ParentObject) 
          {
            return((ParentObject=p_ParentObject)!=NULL);
          }
     };
   ...
   if(result)
     {
      ...
      Print("CSpinEdit:: Create SpinEdit failed");
   return(result);
  }
//+------------------------------------------------------------------+
   ...
   if(result)
     {
      ...
      if(result)
         HandlerList.Add(GetPointer(edField));
     }
   return(result);
  }
//+------------------------------------------------------------------+
      ...
      if(result)
        {
         ...
         if(result)
            HandlerList.Add(GetPointer(btnInc));
        }
     }
   else
     {
   ...
   if(result)
     {
      ...
      if(result)
         HandlerList.Add(GetPointer(btnDec));
     }
   return(result);
  }
//+------------------------------------------------------------------+
...
//if(!Loaded)m_Last=(Default==0.0)?m_Min:Default;
   ...
   if(IsEnabled && btnDec!=NULL)
      ChangeStateImg(btnDec,(m_Last==m_Min)?BS_DISABLED:BS_ENABLED);
   if(IsEnabled && btnInc!=NULL)
      ChangeStateImg(btnInc,(m_Last==m_Max)?BS_DISABLED:BS_ENABLED);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
bool CSpinEdit::Value(const double f_Value)
  {
   if(f_Value<m_Min || f_Value>m_Max)
      return(false);
   m_Last=f_Value;
   if(edField!=NULL)
      edField.SetText(DoubleToString(m_Last,m_Digits));
   ...
   if(btnDec!=NULL)
     {
      if(m_Last==m_Min)
        {
         if(IsEnabled)
            ChangeStateImg(btnDec,BS_DISABLED);
        }
     }
   ...
   if(btnInc!=NULL)
     {
      if(m_Last==m_Max)
        {
         if(IsEnabled)
            ChangeStateImg(btnInc,BS_DISABLED);
        }
     }
   return(true);
  }
...
bool CSpinEdit::Enabled(bool f_State)
  {
   ...
   if(btnInc!=NULL)
     {
      if(m_Last==m_Max)
         result&=btnInc.Enabled(false);
      else
         result&=btnInc.Enabled(f_State);
     }
   if(btnDec!=NULL)
     {
      if(m_Last==m_Min)
         result&=btnDec.Enabled(false);
      else
         result&=btnDec.Enabled(f_State);
     }

   if(edField!=NULL)
      result&=edField.ReadOnly(!f_State);
   if(Bkg!=NULL)
      result&=Bkg.Enabled(f_State);
...
bool CSpinEdit::ChangeStateImg(CSpinButton *p_Button,const int f_StateType)
  {
   ...
   if(result)
      p_Button.SetButtonState(f_StateType);
   return(result);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
void CSpinEdit::ButtonsCheckState(const int f_EventType)
  {
   if(m_Last!=m_Max && m_Last!=m_Min)
      ...
      if(f_EventType==INC_EVENT)
        {
         ...
         if(m_UpBmpPressed!=" ")
           {
            if(btnInc.ButtonState==BS_ENABLED)
               ChangeStateImg(btnInc,BS_LASTPRESSED);
            if(btnDec.ButtonState==BS_LASTPRESSED)
               ChangeStateImg(btnDec,BS_ENABLED);
           }
        }
      else
        {
         ...
         if(m_DnBmpPressed!=" ")
           {
            if(btnDec.ButtonState==BS_ENABLED)
               ChangeStateImg(btnDec,BS_LASTPRESSED);
            if(btnInc.ButtonState==BS_LASTPRESSED)
               ChangeStateImg(btnInc,BS_ENABLED);
           }
        }
     }
   else
     {
      if(f_EventType==INC_EVENT)
        {
         if(m_Last==m_Max && btnInc.IsEnabled)
            ChangeStateImg(btnInc,BS_DISABLED);
         ChangeStateImg(btnDec,BS_ENABLED);
        }
      else
        {
         if(m_Last==m_Min && btnDec.IsEnabled)
            ChangeStateImg(btnDec,BS_DISABLED);
         ChangeStateImg(btnInc,BS_ENABLED);
        }
     }
...
bool CSpinEdit::LoadControlState(int f_file_handle,string f_Name,ENUM_OBJECT f_Type)
  {
   ...
   if(result)
      m_Last=(double)FileReadDouble(f_file_handle);
   return(result);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
bool CSpinEdit::SaveControlState(int f_file_handle)
  {
   ...
   if(result)
     {
      if(FileWriteDouble(f_file_handle,(m_Last==NULL)?(double)m_Min:(double)m_Last)!=sizeof(double))
         return(false);
...
void CGauge::Progress(const int f_Value)
  {
   ...
   if(Percent<=100 && Percent-m_LastPercent>=1)
     {
      if(m_ShowText)
         Text.SetCaption(IntegerToString(Percent)+Suffix);
      ...
      switch(m_Align)
        {
         ...
         case TO_RIGHT:
            break;
         case TO_LEFT:
            SetLeft(Parent.Left()-Width());
            break;
         case TO_TOP:
            SetTop(Parent.Top()-Height());
            break;
        }
   ...
   for(i=0;i<Sheets.TotalItems();i++)
     {
      ...
      if(obj.IsActive())
         ...
         result&=obj.TimeFrames(OBJ_NO_PERIODS);
     }
   return(result);
  }
//+------------------------------------------------------------------+
