...
private:
   ...
   int               common;
   ///
   /// Забивает пробелами открытый файл. Функция вызватется перед записью XML документа, если 
   /// совместно с флагом FILE_WRITE установлен флаг FILE_READ. 
   ///
   void ScoreSpaces(int handle);
public:
...
void CXmlDocument::ScoreSpaces(int handle)
{
   if(!FileGetInteger(handle, FILE_IS_READABLE))
      return;
   int size = (int)FileSize(handle);
   //printf("Size file = " + (string)size);
   uchar spaces[];
   ArrayResize(spaces, size);
   ArrayInitialize(spaces, ' ');
   FileSeek(handle, 0, SEEK_SET);
   FileWriteArray(handle, spaces);
}