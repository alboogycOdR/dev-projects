...
public:
   ...
   string            GetObjectName() const { return m_objname; }
   ENUM_OBJECT       GetObjectType() const { return m_objtype; }
   string            GetResourceName() const { return m_rcname; }
   int               GetWidth() const { return m_width; }
   int               GetHeight() const { return m_height; }
   ENUM_COLOR_FORMAT GetColorFormat() const { return m_format; }
   string            GetFontName() const { return m_fontname; }
   int               GetFontSize() const { return m_fontsize; }
   uint              GetFontFlags() const { return m_fontflags; }
   uint              GetFontAngle() const { return m_fontangle; }

   // Accessor for pixel data
   uint            GetPixels(uint &pixels[]) const
   {
       uint pixelsCopy[];
       ArrayResize(pixels, ArraySize(m_pixels));
       int copieditems= ArrayCopy(pixels, m_pixels);
       Print("pixel copied count: "+copieditems);
       return 1;
   }
