...
private:
   ...
   */ 
         ...
         for (int i = 0; i < ObjectsTotal(0,-1,-1);++i)//new
         {
   ...
   void LoadSR()
   {
      ...
      for (int i=0; i < lineCount;++i)
      {
         ...
         if (StringGetCharacter(priceTxt,0) == 100) // = d   //new
         {
...
public:
   ...
   void DrawSR()
   {
      ...
      for (int i=0; i < _levelCountD1; ++i)
      {
         ...
         ObjectCreate(0, name, OBJ_HLINE, 0, 0, _srLevelsD1[i]);
               ObjectSetInteger(0,name, OBJPROP_COLOR, clrPeru);
               ObjectSetInteger(0,name, OBJPROP_WIDTH, 1);
               ObjectSetInteger(0,name, OBJPROP_BACK, true);
   ...
   bool IsLevelAt(int period, int bar, double pips)
   {
      ...
      //MqlDateTime tm;//new
      //int day = TimeDayOfYear(TimeCurrent(tm));//new
   ...
   bool IsLevel(double &levels[], int levelCount, int period, int bar, double pips)
   {
      ...
      switch(period)
     {
      ...
      for (int i=0; i < levelCount;++i)
      {
         ...
         if (iLow(_symbol, m_period, bar) - pips <= price)
         {
           if (iHigh(_symbol, m_period, bar) + pips  >= price) return true;
         }
         
         if (iHigh(_symbol, m_period, bar) + pips >= price)
         {
           if (iLow(_symbol, m_period, bar) - pips  <= price) return true;
         }
      }
      return false;
   }
