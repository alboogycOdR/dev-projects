...
input bool     scan_before_obs_end_only   = true; // Restrict OB detection scan only to candidates that ended BEFORE observation end time
...
input bool     visual_key_price_line     = true;  // Enable/Disable drawing a horizontal line at the Key Time price - DEPRECATED by ObservationPriceLine
input bool     visual_order_blocks       = true;   // Enable/Disable drawing detected Order Blocks on the chart
input bool     visual_obs_price_line     = true;   // Enable/Disable drawing the line segment at the 13:00 price during observation

//--- Visual Colors ---
input color    vline_keytime_color       = clrSteelBlue;
input color    vline_obsend_color        = clrSalmon;
input color    hline_keyprice_color      = clrBlue; // Still here for visual_key_price_line if enabled (redundant but kept)
...
input color    obs_price_line_color      = clrDarkGray; // Color for the observation price line segment
...
bool       g_obs_price_line_drawn_today = false; // To control drawing of the observation price line
...
int OnInit()
{
    ...
    CalculateAndDrawDailyTimings(); // Also resets daily counts/flags and triggers initial OB scan
    ...
    Print("714 Method EA V5 Core initialized successfully on ", TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES));
...
void OnDeinit(const int reason)
{
    ...
    Print("714 Method EA V5 Core deinitialized. Reason: ", reason);
}
...
void OnTimer()
{
    ...
    // --- Draw Observation Price Line once key price is known and key candle has closed ---
    if (visual_enabled && visual_obs_price_line && !g_obs_price_line_drawn_today &&
        g_KeyPrice_At_KeyTime > 0 && g_TodayKeyTime_Server > 0 && g_ObservationEndTime_Server > 0 &&
        g_TodayKeyTime_Server < g_ObservationEndTime_Server) // Ensure all necessary times and price are valid
    {
        // Find the bar index of the actual key time bar
        ...
        {
            datetime actual_key_bar_open_time = iTime(Symbol(), Period(), key_time_actual_bar_idx);
            datetime actual_key_bar_close_time = actual_key_bar_open_time + PeriodSeconds(Period());

            // Draw if the current closed bar is at or after the key candle's close time for today
            if (current_closed_bar_time >= actual_key_bar_close_time)
            {
                ...
                if(ObjectFind(0, obs_price_line_name) == -1) // Draw only if it doesn't exist
                {
                    Print(visual_comment_text + " - Attempting to draw Observation Price Trend line. KeyTime: ", TimeToString(g_TodayKeyTime_Server, TIME_MINUTES), ", KeyPrice: ", DoubleToString(g_KeyPrice_At_KeyTime, Digits()));
                    if(ObjectCreate(0, obs_price_line_name, OBJ_TREND, 0, g_TodayKeyTime_Server, g_KeyPrice_At_KeyTime, g_ObservationEndTime_Server, g_KeyPrice_At_KeyTime))
                    {
                        ...
                        ObjectSetInteger(0, obs_price_line_name, OBJPROP_HIDDEN, true);
                        ChartRedraw(0);
                        Print(visual_comment_text + " - Observation Price Trend line drawn successfully.");
                    } else {
                        Print("Warning: Observation Price Trend line creation failed (GetLastError: ", GetLastError(), "). Times (Key: ", TimeToString(g_TodayKeyTime_Server), " ObsEnd: ", TimeToString(g_ObservationEndTime_Server) ,") Price (", DoubleToString(g_KeyPrice_At_KeyTime, Digits()),").");
                    }
                }
                g_obs_price_line_drawn_today = true; // Mark as attempted/drawn for today
            }
        }
    }
    ...
    if (!g_bias_determined_today && current_closed_bar_time >= g_ObservationEndTime_Server)
    {
         ...
         if (key_bar_idx >= 0 && obs_end_bar_idx >= 0 && obs_end_bar_idx < Bars(Symbol(), Period()) && obs_end_bar_idx >= key_bar_idx) // Used Bars() directly
         {
             ...
             // --- After bias is determined, perform OB detection scan if not already done for today ---
              if (!g_order_blocks_scanned_today)
              {
                   ...
                   // Call the detection function with a relevant scan range (indices of bars to scan for potential OB candidates)
                   // A reasonable scan range: from a few bars BEFORE the Key Time up to a few bars AFTER the Observation End Time
                   datetime scan_start_period_time = g_TodayKeyTime_Server - PeriodSeconds(Period()) * ob_MaxBlockCandles; // Time slightly before key time
                   datetime scan_end_period_time = g_ObservationEndTime_Server + PeriodSeconds(Period()) * ob_Lookback_Bars_For_Impulse; // Time slightly after observation end

                   // Find the chart bar indices for these times (scan_to_idx will be the newest index, scan_from_idx the oldest)
                   int scan_to_chart_idx = iBarShift(Symbol(), Period(), scan_end_period_time, true); // Find bar index at or after scan end time (newer)
                   int scan_from_chart_idx = iBarShift(Symbol(), Period(), scan_start_period_time, true); // Find bar index at or after scan start time (older)


                   // Ensure indices are valid and ordered correctly for scanning (scan_from_chart_idx should be numerically smaller/newer or equal to scan_to_chart_idx due to how iBarShift works with 'true' parameter and time progression)
                   // Loop from the latest needed bar index back to the earliest needed bar index
                   int latest_relevant_idx = MathMax(0, iBarShift(Symbol(), Period(), TimeCurrent(), true)); // Max is 0 if data not yet present

                    // Define the scan range relative to newest bars (indices closer to 0). Loop from the most recent bar to an older bar.
                   // Scan from closed_bar_index (newest processed bar when bias is determined) back to find OB candidates.
                   // Limit how far back the scan goes. Scan back up to a bar before the Key Time or Observation End.
                   int scan_end_bar_idx_newer = closed_bar_index; // Start scanning from the current bar (Obs End bar or after) backwards
                   int scan_start_bar_idx_older = iBarShift(Symbol(), Period(), g_TodayKeyTime_Server - PeriodSeconds(Period()) * ob_MaxBlockCandles, true); // Scan back to this bar (or earlier if <0)

                    // Ensure valid scan window start
                   if (scan_start_bar_idx_older < 0) scan_start_bar_idx_older = Bars(Symbol(), Period()) -1 - (ob_Lookback_Bars_For_Impulse + ob_MaxBlockCandles) * 5; // Scan back a good distance if time based failed - adjust buffer based on needs

                    // Ensure enough data is available for detection logic even for oldest candidate
                    if (scan_start_bar_idx_older + MathMax(ob_Lookback_Bars_For_Impulse, ob_MaxBlockCandles) >= total_bars)
                    {
                        Print(visual_comment_text + " - Not enough historical bars to scan adequately for OBs in Stage 2 period.");
                        g_order_blocks_scanned_today = true;
                        return;
                    }

                   // Scan from scan_end_bar_idx_newer (more recent index) back to scan_start_bar_idx_older (older index)
                    if (scan_end_bar_idx_newer < scan_start_bar_idx_older) // Ensure range order is correct (newer < older index numerically)
                    {
                         DetectOrderBlocksForToday(scan_end_bar_idx_newer, scan_start_bar_idx_older); // Pass newest, oldest indices
                    }
                    else
                    {
                         // If scan_end_idx_newer is >= scan_start_idx_older, something might be off in range calc or very small period. Print and skip scan.
                         Print(visual_comment_text + " - Skipping OB scan - invalid or very short scan index range calculated.");
                         g_order_blocks_scanned_today = true;
                         return;
                    }
         ...
         {
              Print("Warning: Could not determine relevant bar indices for initial bias or OB scan start. Insufficient data?");
         ...
         if (current_closed_bar_time == g_EntryTiming_Server)
         {
              ...
              if (g_InitialBias == -1) // If initial move was bearish, looking for BUYS
              {
                   ...
                   if (entry_conditions_met)
                   {
                       ...
                       // Print("Placeholder: Buy order placement function called. Trading disabled in V4.");
                   }
              }
              else if (g_InitialBias == 1) // If initial move was bullish, looking for SELLS
              {
                    ...
                    if (entry_conditions_met)
                   {
                       ...
                       // Print("Placeholder: Sell order placement function called. Trading disabled in V4.");
                   }
              }
              // --- END OF ENTRY LOGIC PLACEHOLDER ---
         }

         // Note: The strategy *specifically* mentions the 15th candlestick as the timing.
         // Extending the entry window to bars after the 15th is a potential adaptation,
         // but strict adherence would limit the entry check to just this one bar.
         // The current logic checks exactly on the 15th bar's start time.

          // Check if we have passed the 15th candlestick entry window without alerting -
          // The flag g_entry_timed_window_alerted controls if the *message* is printed again.
          // The core logic check runs only when time is exactly g_EntryTiming_Server.
...
//| Scans historical data arrays to identify OB candles              |
...
bool IsBullishOrderBlockCandidate(int index_in_array, const double &open_arr[], const double &high_arr[], const double &low_arr[], const double &close_arr[], int array_size)
{
    ...
    int needed_future_bars = MathMax(ob_Lookback_Bars_For_Impulse, ob_MaxBlockCandles -1 );
    if (index_in_array + 1 + needed_future_bars >= array_size) return false;
     ...
     for (int i = index_in_array + 1; i < index_in_array + 1 + ob_Lookback_Bars_For_Impulse && i < array_size; i++)
    {
         if (close_arr[i] > open_arr[i]) // Found a bullish candle
         {
              ...
              // Check for the minimum cumulative move within the impulse scan bars
              if ((cumulative_move_after_ob / GetPipValue()) >= ob_MinMovePips)
              {
                   // Now check for consecutive bullish candles within the impulse scan range
                   ...
                   {
                       if(close_arr[k] <= open_arr[k]) // If any non-bullish candle is found
                       {
                           consecutive_bullish_met = false;
                           break; // Not consecutive bullish sequence for this part of scan
                       }
                   }
                   if(consecutive_bullish_met && (i - (index_in_array+1) + 1) >= 2) // Check if at least 2 consecutive bullish candles led to the move
                   {
         ...
         {
              // If a bearish or doji candle is encountered immediately after the OB candidate
              // This version of logic considers ANY consecutive bullish candles after the OB.
              // The previous indicator's `else break` logic here might be key: Stop looking for impulse if non-bullish candle encountered.
              // Let's re-implement that more strictly as in the previous logic's interpretation.
              // If the candle *immediately* after the potential OB candidate (index_in_array + 1) is NOT bullish, then there is no initial bullish sequence
              // If any candle within the lookback breaks the consecutive bullish count before the move is met, stop.
              if (close_arr[i] <= open_arr[i] ) break; // If a non-bullish candle within the lookback breaks the sequence
...
bool IsBearishOrderBlockCandidate(int index_in_array, const double &open_arr[], const double &high_arr[], const double &low_arr[], const double &close_arr[], int array_size)
{
     ...
     for (int i = index_in_array + 1; i < index_in_array + 1 + ob_Lookback_Bars_For_Impulse && i < array_size; i++)
    {
         if (close_arr[i] < open_arr[i]) // Found a bearish candle
         {
              ...
              if ((cumulative_move_after_ob / GetPipValue()) >= ob_MinMovePips)
              {
                   // Now check for consecutive bearish candles within the impulse scan range
                   bool consecutive_bearish_met = true;
                    for(int k = index_in_array + 1; k <= i; k++) // Check bars from directly after OB up to current candle 'i' in impulse scan
                   {
                       if(close_arr[k] >= open_arr[k]) // If any non-bearish candle is found
                       {
                           consecutive_bearish_met = false;
                           break; // Not consecutive sequence
                       }
                   }
                    if(consecutive_bearish_met && (i - (index_in_array+1) + 1) >= 2) // Check if at least 2 consecutive bearish candles led to the move
                   {
                       found_strong_move = true; // Criteria for strong move met
                       break; // Stop scanning
                   }
         ...
         {
              // If a bullish or doji candle is encountered immediately after the OB candidate
               if (close_arr[i] >= open_arr[i] ) break; // If a non-bearish candle within the lookback breaks the sequence
         }
    }
...
void DetectOrderBlocksForToday(int scan_newest_chart_index, int scan_oldest_chart_index)
{
     ...
     if (scan_newest_chart_index < 0 || scan_oldest_chart_index < 0 || scan_newest_chart_index >= total_bars || scan_oldest_chart_index >= total_bars || scan_newest_chart_index >= scan_oldest_chart_index) // Condition check correct based on index values
     {
      ...
      // Determine the number of bars needed to copy data for. This range covers the scan area PLUS the bars needed *ahead* for the impulse/block checks
      // Copy data from the oldest bar in the scan range (`scan_oldest_chart_index`) and add enough future bars from there
      // for any potential OB candidate's check to complete when scanning backwards.
      // The furthest an OB candidate check looks forward in the *array* from its array index is MathMax(ob_Lookback_Bars_For_Impulse + 1, ob_MaxBlockCandles).
      // This needs to be translated to chart indices.
      // Furthest chart index from 'scan_oldest_chart_index' needed is scan_oldest_chart_index - MathMax(ob_Lookback_Bars_For_Impulse + 1, ob_MaxBlockCandles)
      // The total data range needed for copying starts at MathMax(0, scan_oldest_chart_index - MathMax(ob_Lookback_Bars_For_Impulse + 1, ob_MaxBlockCandles) )
      // And goes up to the latest needed bar which is scan_newest_chart_index

       int max_future_bars_needed_by_ob_check = MathMax(ob_Lookback_Bars_For_Impulse + 1, ob_MaxBlockCandles) ; // How many bars are checked including OB candidate candle itself and forward

      int copy_start_chart_index = MathMax(0, scan_oldest_chart_index + max_future_bars_needed_by_ob_check -1 ) ; // Calculate the OLDEST chart index we need data for to cover all future checks FROM oldest scan bar

      int num_bars_to_copy = scan_newest_chart_index - copy_start_chart_index + 1; // Number of bars from copy_start to scan_newest inclusive

       if (num_bars_to_copy <= max_future_bars_needed_by_ob_check) // Ensure enough bars copied to perform checks from newest scan index backwards
     {
          Print(visual_comment_text + " - Not enough copied bars (", num_bars_to_copy, ") to perform OB detection scan in the relevant period based on required lookahead.");
           g_order_blocks_scanned_today = true;
          return;
     }
     ...
     if (CopyOpen(Symbol(), Period(), copy_start_chart_index, num_bars_to_copy, open_arr) < 0 ||
         CopyHigh(Symbol(), Period(), copy_start_chart_index, num_bars_to_copy, high_arr) < 0 ||
         CopyLow(Symbol(), Period(), copy_start_chart_index, num_bars_to_copy, low_arr) < 0 ||
         CopyClose(Symbol(), Period(), copy_start_chart_index, num_bars_to_copy, close_arr) < 0)
     {
     ...
     Print(visual_comment_text + " - Scanning chart indices from ", scan_newest_chart_index, " (newest) back to ", scan_oldest_chart_index, " (oldest) for Order Blocks in Stage 2 period.");
     ...
     for (int chart_idx = scan_newest_chart_index; chart_idx <= scan_oldest_chart_index; chart_idx++) // Loop from newest (lower index) to oldest (higher index)
     {
          ...
          // If scan_before_obs_end_only is true, only consider candidates whose end time is roughly before observation end time.
           datetime ob_end_approx_time = iTime(Symbol(), Period(), chart_idx) + PeriodSeconds(Period()) * (ob_MaxBlockCandles-1); // Approx end time of the OB block period from this candidate
          if(scan_before_obs_end_only && ob_end_approx_time >= g_ObservationEndTime_Server ) continue;


          int arr_idx = chart_idx - copy_start_chart_index; // Get the array index for this chart index


           // --- Check if this candle at array index 'arr_idx' is a Bullish or Bearish Order Block candidate ---
           // Ensure we have enough data in the copied array starting from this candidate's array index onwards
           if (arr_idx + MathMax(ob_Lookback_Bars_For_Impulse, ob_MaxBlockCandles-1) >= num_bars_to_copy -1) // If not enough bars FROM here forward to satisfy furthest check
           {
                // Print("Skipping OB candidate at chart index ", chart_idx, " - not enough future data in copied array.");
                 continue;
           }
          ...
          if (is_bullish_ob)
          {
               if (g_bullishOB_count < 100)
               {
                    ...
                    // Print confirmation if detection is rare/important enough - turn off for high frequency symbols
                    ...
                    if(visual_enabled && visual_order_blocks)
                    {
                        ...
                        } else { Print(visual_comment_text + " - Error creating Bull OB visual object at chart index ", chart_idx, ": ", GetLastError());}
                    }
              ...
              if (g_bearishOB_count < 100)
               {
                    ...
                    if(visual_enabled && visual_order_blocks)
                    {
                        ...
                        } else { Print(visual_comment_text + " - Error creating Bear OB visual object at chart index ", chart_idx, ": ", GetLastError());}
                    }
...
void CalculateAndDrawDailyTimings()
{
    ...
    if (key_bar_idx_for_count >= 0) // Ensure the starting bar for the count exists
    {
        ...
        if(target_entry_bar_idx >= 0 && target_entry_bar_idx < Bars(Symbol(), Period()))
        {
         ...
         {
              Print("Warning: Could not find a bar at or after the calculated Key Time to get the price. Ensure enough M5 history is loaded around ", TimeToString(g_TodayKeyTime_Server, TIME_DATE|TIME_MINUTES), " Server Time.");
    ...
    if (visual_enabled)
    {
        ...
        if (visual_main_timing_lines)
        {
            // Draw Vertical Line at Key Time (13:00 UTC+2 Server Time)
             string key_time_vline_name = "VLine_KeyTime_" + Symbol() + "_" + EnumToString(Period()) + "_" + TimeToString(TimeTradeServer(), TIME_DATE|TIME_MINUTES|TIME_SECONDS); // Unique name
            ...
            // Draw Vertical Line at Observation End Time (Server Time)
             string obs_end_vline_name = "VLine_ObsEnd_" + Symbol() + "_" + EnumToString(Period()) + "_" + TimeToString(TimeTradeServer(), TIME_DATE|TIME_MINUTES|TIME_SECONDS); // Unique name
        ...
        // Draw Horizontal Line at Key Price over Observation Period if enabled
        // Uses OBJ_TREND to draw a time-bound segment
        if (visual_obs_price_line && g_KeyPrice_At_KeyTime > 0 && g_TodayKeyTime_Server > 0 && g_ObservationEndTime_Server > 0 && g_TodayKeyTime_Server < g_ObservationEndTime_Server) // Ensure valid times and price
        {
             string obs_price_line_name = "Trend_ObsPrice_" + Symbol() + "_" + EnumToString(Period()) + "_" + TimeToString(g_TodayKeyTime_Server, TIME_DATE|TIME_MINUTES|TIME_SECONDS); // Unique name
             if(ObjectFind(0, obs_price_line_name) > 0) ObjectDelete(0, obs_price_line_name); // Delete if exists

             // Create a TREND object from Key Time at Key Price to Observation End Time at Key Price
             if(ObjectCreate(0, obs_price_line_name, OBJ_TREND, 0, g_TodayKeyTime_Server, g_KeyPrice_At_KeyTime, g_ObservationEndTime_Server, g_KeyPrice_At_KeyTime))
             {
                  ObjectSetInteger(0, obs_price_line_name, OBJPROP_COLOR, obs_price_line_color);
                  ObjectSetInteger(0, obs_price_line_name, OBJPROP_STYLE, STYLE_SOLID);
                  ObjectSetInteger(0, obs_price_line_name, OBJPROP_WIDTH, 1);
                  ObjectSetInteger(0, obs_price_line_name, OBJPROP_SELECTABLE, false);
                  ObjectSetInteger(0, obs_price_line_name, OBJPROP_BACK, true); // Send to back
                  ObjectSetString(0, obs_price_line_name, OBJPROP_TEXT, visual_comment_text + ": Obs Price Level"); // Add a label
                   ObjectSetInteger(0, obs_price_line_name, OBJPROP_RAY, false); // Ensure it's a segment, not ray
                   ObjectSetInteger(0, obs_price_line_name, OBJPROP_HIDDEN, true); // Hide on lower/higher TFs
                  ChartRedraw(0);
             } else {
                 Print("Warning: Observation Price Trend line not drawn (GetLastError: ", GetLastError(), "). Check times and price.");
             }
        }

         // Optional: Draw horizontal line at key price (if visual_key_price_line is true) - this is potentially redundant now
        // if (visual_enabled && visual_key_price_line && g_KeyPrice_At_KeyTime > 0)
        // { ... drawing logic for hline ... }

        // OBs visuals are drawn when they are detected later
    }
    ...
    g_bearishOB_count = 0;
     ...
     {
           // Define scan range in times
           datetime scan_start_period_time = g_TodayKeyTime_Server - PeriodSeconds(Period()) * ob_MaxBlockCandles * 2 ; // Scan a bit before key time to allow OBs before the main move (Added x2 buffer)
           datetime scan_end_period_time = g_ObservationEndTime_Server + PeriodSeconds(Period()) * ob_Lookback_Bars_For_Impulse * 2; // Scan a bit after observation end to catch impulse (Added x2 buffer)

           // Find the corresponding chart bar indices for these times (scan_to_idx will be the newest index, scan_from_idx the oldest)
           int scan_to_chart_idx = iBarShift(Symbol(), Period(), scan_end_period_time, true); // Find bar index at or after scan end time (newer index)
           int scan_from_chart_idx = iBarShift(Symbol(), Period(), scan_start_period_time, true); // Find bar index at or after scan start time (older index)


           // Ensure valid indices are found
            if(scan_to_chart_idx < 0 || scan_from_chart_idx < 0 || scan_to_chart_idx >= total_bars || scan_from_chart_idx >= total_bars)
           {
               Print(visual_comment_text + " - Warning: Could not establish a valid chart index range for Order Block detection scan based on key times and history.");
                Print(" Calculated scan times: From ", TimeToString(scan_start_period_time), " to ", TimeToString(scan_end_period_time));
                Print(" Calculated scan indices (newer, older): ", scan_to_chart_idx, ", ", scan_from_chart_idx, ". Total bars: ", total_bars);
               g_order_blocks_scanned_today = true; // Avoid trying again today
               return;
           }


           // The DetectOrderBlocksForToday function loops from a NEWER chart index back to an OLDER chart index numerically.
           // So, the parameters need to be (newest index, oldest index).
            if (scan_to_chart_idx < scan_from_chart_idx) // This is the expected scenario: newer index is numerically smaller than older index
            {
                 DetectOrderBlocksForToday(scan_to_chart_idx, scan_from_chart_idx); // Pass newer index, then older index
            }
            else // Should not happen often with the time-based logic, but handle. Means newest calculated time has an older index than oldest calculated time.
            {
                Print(visual_comment_text + " - Warning: Unusual chart index order calculated for OB scan. Scan from newer index ", scan_to_chart_idx, " back to older index ", scan_from_chart_idx);
                DetectOrderBlocksForToday(scan_to_chart_idx, scan_from_chart_idx); // Still attempt with parameters as is
            }
     }
...
void IsNewDayCheckAndReset()
{
    ...
    if(server_midnight_today > g_last_initialized_day_time && g_last_initialized_day_time != 0)
    {
        ...
        g_obs_price_line_drawn_today = false; // Allow observation price line to be drawn on new day
        ...
        // OB detection scan happens within OnTimer after bias is set, OR right after timings on a new day.
        // The DetectOrderBlocksForToday is called conditionally in CalculateAndDrawDailyTimings now if bias is already determined (like during backtest)
        // Otherwise, OnTimer will call it later after the bias is determined live.

    }
     else if (g_last_initialized_day_time == 0) // Special case for the very first time OnInit is called or after EA restart
     {
          g_last_initialized_day_time = server_midnight_today; // Set the initial day
     }
}
...
void RemoveDailyVisualObjects()
{
     ...
     for (int i = ObjectsTotal(chart_id) - 1; i >= 0; i--)
     {
         ...
         if (StringFind(obj_name, "VLine_KeyTime_", 0) == 0 ||
             ...
             StringFind(obj_name, "HLine_KeyPrice_", 0) == 0 || // Still check for this if it was created
          ...
          else if (StringFind(obj_name, "BearOB_", 0) == 0) // Starts with BearOB_ (covers rects)
         {
          ...
          else if (StringFind(obj_name, "_Label", StringLen(obj_name) - StringLen("_Label")) >= 0 ) // Check if name ends with "_Label"
          {
             // Now check if the object this label *belongs* to starts with one of our OB prefixes
              // We can't directly link label to its rectangle parent easily just by name
              // A safer way is to delete the RECTANGLE (OB) object first. If labels are drawn after, deleting by the rectangle's name might also delete its label, or they can be named relationally.
              // Since our labels' names START with the rectangle's name, deleting the rects first should clean up most labels automatically IF drawn that way.
              // But the StringFind check here is also valid: If a name ends in _Label AND starts with our OB prefixes:
...
{
     Print(visual_comment_text + " - Placeholder: PlaceBuyOrder called. Trading disabled in V4.");
...
{
     Print(visual_comment_text + " - Placeholder: PlaceSellOrder called. Trading disabled in V4.");
