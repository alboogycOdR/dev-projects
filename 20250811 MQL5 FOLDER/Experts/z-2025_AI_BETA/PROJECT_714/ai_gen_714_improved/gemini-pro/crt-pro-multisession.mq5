...
#property description "Expert Advisor for the Institutional 1AM, 5AM, and 9AM Candle Range Theory (CRT) Models."
#property strict
...
input ENUM_BIAS_MODE        BiasDetectionMode = AUTOMATIC;                 // Bias Detection Mode
...
input int                   NY_Time_Offset_Hours = 7;                   // Your Broker's Server Time vs NY Time (e.g., -7 for GMT-4 broker)
...
//--- Magic Number
input ulong                 ExpertMagicNumber = 19911;                  // EA Magic Number
...
string g_crt_range_status = "Waiting for range...";
string g_crt_range_values = "";
string g_setup_status     = "No setup identified.";
string g_signal_status    = "No signal generated.";
...
int OnInit()
{
    Print("Multi-Session CRT Expert Advisor Initializing...");
    Print("Selected Model: ", EnumToString(CRTModelSelection));
    Print("NY Time Offset: ", NY_Time_Offset_Hours, " hours");

    SetupDashboard();
    trade.SetExpertMagicNumber(ExpertMagicNumber); // New magic number for updated version
...
void ResetDailyVariables()
{
    ...
    // Reset dashboard strings
    g_crt_range_status = "Waiting for range...";
    g_crt_range_values = "";
    g_setup_status     = "No setup identified.";
    g_signal_status    = "No signal generated.";
}
...
{
    //--- Only determine bias once per day
    if(htfKeyLevelHigh != 0 || htfKeyLevelLow != 0) return;

    ENUM_DAILY_BIAS currentBias = ManualDailyBias;
    if(BiasDetectionMode == AUTOMATIC)
    {
        ...
        if(prevDayClose > (prevDayHigh + prevDayLow) / 2)
            ...
            currentBias = BEARISH;
        // This is now displayed on the dashboard, no need to print to log.
        // Print("Automatic Bias Detected: ", EnumToString(currentBias));
    }
    ...
    if(ny_tm.hour >= range_set_ready_hour)
    {
        ...
        for(int i = ArraySize(h1_rates) - 1; i >= 0; i--)
        {
            ...
            if(candle_ny_tm.hour == range_candle_hour)
            {
                ...
                g_crt_range_status = EnumToString(CRTModelSelection) + " Range Set for " + TimeToString(crtRangeSetTime, TIME_DATE);
                g_crt_range_values = "High: " + DoubleToString(crtHigh, _Digits) + " Low: " + DoubleToString(crtLow, _Digits);
                Print(g_crt_range_status);
                Print(g_crt_range_values);
...
bool IsWithinKillzone()
{
    ...
    {
        case CRT_1AM_ASIA:
            kz_start_str = Asia_Killzone_Start;
            kz_end_str = Asia_Killzone_End;
            break;
        case CRT_5AM_LONDON:
            kz_start_str = London_Killzone_Start;
            kz_end_str = London_Killzone_End;
            break;
        case CRT_9AM_NY:
            kz_start_str = NY_Killzone_Start;
            kz_end_str = NY_Killzone_End;
            break;
    }
...
void CheckForEntry()
{
    ...
    ENUM_DAILY_BIAS activeBias = (BiasDetectionMode == MANUAL) ? ManualDailyBias : GetAutomaticBias();

    if(activeBias == BULLISH)
    {
        if(last_closed_low < crtLow)
        {
            g_setup_status = "Potential bullish setup: CRT Low has been swept.";
            Print(g_setup_status);
            if(EntryLogicModel == CONFIRMATION_MSS)
            {
                entrySignal = "BUY Signal (MSS Confirmation)";
                g_signal_status = entrySignal;
    ...
    else if(activeBias == BEARISH)
    {
        ...
        {
            g_setup_status = "Potential bearish setup: CRT High has been swept.";
            Print(g_setup_status);
            if(EntryLogicModel == CONFIRMATION_MSS)
            {
...
void ExecuteTrade(string direction)
{
    ...
    double entry_price, sl_price, tp1_price, tp2_price, lot_size;
...
double CalculateLotSize(double sl_distance_price)
{
    ...
    {
        Print("Could not get tick value/size for ", _Symbol);
        return 0.0;
    }
...
{
    if(position.SelectByMagic(_Symbol, ExpertMagicNumber))
    {
...
void SetupDashboard()
{
    ...
    ObjectSetInteger(0, "CRTDashboard", OBJPROP_CORNER, CORNER_RIGHT_UPPER);
    ObjectSetString(0, "CRTDashboard", OBJPROP_FONT, "Arial");
    ObjectSetInteger(0, "CRTDashboard", OBJPROP_FONTSIZE, 11);
    ObjectSetInteger(0, "CRTDashboard", OBJPROP_COLOR, clrWhite);
    ObjectSetInteger(0, "CRTDashboard", OBJPROP_BACK, false);
}

void UpdateDashboard()
{
    string dashboard_text = "--- CRT Status Dashboard ---\n";
    string bias_str = (BiasDetectionMode == MANUAL) ? EnumToString(ManualDailyBias) : EnumToString(GetAutomaticBias());
    dashboard_text += "Model: " + EnumToString(CRTModelSelection) + " | Bias: " + bias_str + "\n\n";
    
    // --- CRT Range ---
    dashboard_text += g_crt_range_status + "\n";
    if(g_crt_range_values != "")
    {
        dashboard_text += g_crt_range_values + "\n";
    }
    dashboard_text += "\n";
    
    // --- Live Status ---
    dashboard_text += "Setup: " + g_setup_status + "\n";
    dashboard_text += "Signal: " + g_signal_status + "\n";
...
// Helper to parse strings like "09:30" into hours and minutes
...
// Helper to get the current automatic bias without re-printing
