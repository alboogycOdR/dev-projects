...
void OnStart()
{
    ...
    // Going backwards in case one or more positions are closed during the cycle.
    {
        ...
        // Compare doubles by calculating difference. 
        //If this difference is greater than Point / 2, the position's 
        //profit is below the MinimumProfit parameter.

        if ((MinimumProfit > 0) && 
            ((double)MinimumProfit - MathAbs(PositionGetDouble(POSITION_PRICE_OPEN) - PositionGetDouble(POSITION_PRICE_CURRENT)) / point > point / 2)) 
        continue;
