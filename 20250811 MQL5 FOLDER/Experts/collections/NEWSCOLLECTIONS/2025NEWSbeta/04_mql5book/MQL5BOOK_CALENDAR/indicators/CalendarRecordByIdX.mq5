...
void OnTimer()
{
   ...
   else
   {
      ...
      MqlCalendarEvent event;CalendarEventById(track.id,event);
      ...
      {
         Comment(eventName+" Forthcoming event time: ", track.time,
            ", remaining: ", Timing::stringify((uint)(track.time - TimeCurrent())));
      }
      else
      {
         Comment(track.event_id+" Forthcoming event time: ", track.time,
