...
input group
"==========Strategies=========="
// input bool tradeBoS = true; //Enable BOS Trade
 bool tradeFVG = true; //Enable FVG Trade
input bool tradeFVG_Market = true; //Enable Market Orders | FVG
input bool tradeFVG_Pending = false; //Enable Pending Orders | FVG
input group
"==========Settings control=========="
input group   "fvg"
...
input int           InpStopLoss = 385;     //SL & trailing stop in points
input int           InpTakeProfit = 2800;  //take profit in points  (0=off)
...
//--- tuning for 3 or 5 digits
   ...
   //    //if(handleSMC_MS == INVALID_HANDLE)
   //    //   return (INIT_FAILED);
...
//+------------------------------------------------------------------+
bool IsBoolish(double open, double close)
  {
   return(close > open);
  }
...
//mechanism to only trade during SILVER BULLET HOURS
//    REMOVE IF NOT APPLICABLE TO TRADING STRAT
...
//By the end of the loop, if isSwingHigh is still true, it suggests that current bar curr_bar has a higher high than the surrounding bars within length range, marking a potential swing high.
   ...
   if(isSwingHigh)
     {
      ...
      drawSwingPoint(TimeToString(time(curr_bar)),time(curr_bar),high(curr_bar),77,clrPurple,-1);
     }
   if(isSwingLow)
     {
      swing_L = low(curr_bar);
      //Print("DOWN @ BAR INDEX ",curr_bar," of Low: ",low(curr_bar));
      drawSwingPoint(TimeToString(time(curr_bar)),time(curr_bar),low(curr_bar),77,clrPink,1);
     }
...
//===end of BOS routine
   ...
   */
   ...
   //                      time(0+1),high(swing_H_index),clrPurple,-1);
   ...
   //       //obj_Trade.Buy(InpLots,_Symbol,Ask,sl,tp,"BoS Break Up BUY");
   //       PerformTrade(ORDER_TYPE_BUY, Ask, sl, tp, "BoS Break Up BUY");
   ...
   //                         time(0+1),low(swing_L_index),clrPink,1);
   ...
   //          //obj_Trade.Sell(InpLots,_Symbol,Bid,sl,tp,"BoS Break Down SELL");
   //          PerformTrade(ORDER_TYPE_SELL, Bid, sl, tp, "BoS Break Down SELL");
      ...
      if(FVG_UP || FVG_DOWN)
        {
         ...
         if(ObjectFind(0, fvgNAME) < 0 && tradeFVG)
           {
            CreateRec(fvgNAME, time1, price1, time2, price2, fvgClr);
            if(isDebugging)
               Print("Old ArraySize = ", ArraySize(totalFVGs));
      ...
      if(fvgColor == CLR_DOWN && tradeFVG_Market && Bid > fvgHigh && !signalFVGs[j])
        {
         ...
         if(fvgColor == CLR_UP && tradeFVG_Market && Ask < fvgLow && !signalFVGs[j])
           {
            if(isDebugging)
            ...
            if(fvgColor == CLR_UP && tradeFVG_Pending && !signalFVGs[j])
              {
               if(isDebugging)
                  Print("BUY SIGNAL For (", fvgNAME, ") Now @ ", Ask);
               //if(tradeFVG) {
               ...
               if(fvgColor == CLR_DOWN && tradeFVG_Pending && !signalFVGs[j])
                 {
                  if(isDebugging)
                     Print("SELL SIGNAL For (", fvgNAME, ") Now @ ", Bid);
                  //if(tradeFVG) {
      ...
      if((check_order_type == ORDER_TYPE_BUY_LIMIT) || (check_order_type == ORDER_TYPE_BUY_STOP))
        {
         ...
         if((check_order_type == ORDER_TYPE_SELL_LIMIT) || (check_order_type == ORDER_TYPE_SELL_STOP))
           {
...
//-------------traditional snr
//resistence
if(IsBoolish(mrate[2].open, mrate[2].close)
   &&
   !IsBoolish(mrate[1].open, mrate[1].close))
  {
   CreatePositionLine(mrate[2].time, mrate[2].close, mrate[1].time, mrate[2].close, clrBlack);
//kkjkj
  }
//support
if(IsBearish(mrate[2].open, mrate[2].close) && !IsBearish(mrate[1].open, mrate[1].close))
  {
   CreatePositionLine(mrate[2].time, mrate[2].close, mrate[1].time, mrate[2].close, clrBlue);
  }
//-------------sideways snr
//support
if(IsBoolish(mrate[2].open, mrate[2].close)
