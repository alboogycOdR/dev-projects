...
int ema9_handle_M5;
int vwap_handle_M5;    // Handle for custom VWAP indicator on M5
...
const ENUM_TIMEFRAMES TIMEFRAME_M5 = PERIOD_M5;
...
void OnInit()
{
   ...
   // Create EMA9 indicator handle for M5
   ema9_handle_M5 = iMA(SYMBOL, TIMEFRAME_M5, 9, 0, MODE_EMA, PRICE_CLOSE);
   if(ema9_handle_M5 == INVALID_HANDLE)
   {
      Print("Failed to create EMA9 handle for M5. EA will terminate.");
      ExpertRemove();
      return;
   }

   // Create handle for custom VWAP indicator for M5
   vwap_handle_M5 = iCustom(SYMBOL, TIMEFRAME_M5, "VWAP_UTC_Final");
   if(vwap_handle_M5 == INVALID_HANDLE)
   {
      Print("Failed to create VWAP handle for M5. EA will terminate.");
      ExpertRemove();
      return;
   }

   trade.SetDeviationInPoints(10); // Allow 1 pip slippage
   Print("EA initialized successfully on ", SYMBOL, " (M15 - Main, M5 - Entry)");
}
...
void OnTick()
{
   ...
   //--- M5 Bar Check for Entry ---
   datetime current_bar_time_M5 = iTime(SYMBOL, TIMEFRAME_M5, 0);
   if(current_bar_time_M5 > last_bar_time_M5)
   {
      last_bar_time_M5 = current_bar_time_M5;

      // Get M5 indicator values
      double ema9_M5[];
      ArraySetAsSeries(ema9_M5, true);
      if(CopyBuffer(ema9_handle_M5, 0, 0, 3, ema9_M5) < 3)
      {
      ...
      double vwap_M5[];
      ArraySetAsSeries(vwap_M5, true);
      if(CopyBuffer(vwap_handle_M5, 0, 0, 3, vwap_M5) < 3)
      {
      ...
      // Check Entry Conditions on M5 timeframe
      if(!PositionSelect(SYMBOL)) // Check for no existing position before entry
      {
         CheckEntryConditions_M5(ema9_M5, vwap_M5, vwap_M15_for_SL); // Pass M15 VWAP for SL
      }
   }
}
...
//| Check Entry Conditions on M5 Timeframe                             |
//+------------------------------------------------------------------+
void CheckEntryConditions_M5(double &ema9_M5[], double &vwap_M5[], double &vwap_M15_for_SL[])
{
   int digits = (int)SymbolInfoInteger(SYMBOL, SYMBOL_DIGITS);
   double point = SymbolInfoDouble(SYMBOL, SYMBOL_POINT);

   // Buy condition: M5 EMA9 crosses above M5 VWAP
   if(ema9_M5[2] < vwap_M5[2] && ema9_M5[1] > vwap_M5[1])
   {
      double sl = NormalizeDouble(vwap_M15_for_SL[1] - 3 * point, digits); // SL from M15 VWAP
      if(trade.Buy(FIXED_LOT_SIZE, SYMBOL, 0.0, sl, 0.0))
      {
         Print("Buy trade opened (M5 Entry) with SL: ", sl, " (based on M15 VWAP)");
      }
   }
   // Sell condition: M5 EMA9 crosses below M5 VWAP
   else if(ema9_M5[2] > vwap_M5[2] && ema9_M5[1] < vwap_M5[1])
   {
      double sl = NormalizeDouble(vwap_M15_for_SL[1] + 3 * point, digits); // SL from M15 VWAP
      if(trade.Sell(FIXED_LOT_SIZE, SYMBOL, 0.0, sl, 0.0))
      {
         Print("Sell trade opened (M5 Entry) with SL: ", sl, " (based on M15 VWAP)");
      }
   }
}
...
void OnDeinit(const int reason)
{
   ...
   if(ema9_handle_M5 != INVALID_HANDLE)
      IndicatorRelease(ema9_handle_M5);
   if(vwap_handle_M5 != INVALID_HANDLE)
      IndicatorRelease(vwap_handle_M5);
