...
input double MIN_DISTANCE_M10 = 10.0;              // Minimum distance between EMA9 and VWAP for M10 (in price units)
input double MIN_DISTANCE_M15 = 10.0;              // Minimum distance between EMA9 and VWAP for M15 (in price units)
input bool WaitUntilNextCrossover = false;          // Wait until next crossover for distance confirmation (true) or abandon after MAX_WAIT_BARS (false)
input int MAX_WAIT_BARS = 10;                      // Safety limit: maximum bars to wait if WaitUntilNextCrossover=false
...
void OnInit()
{
   ...
   Print("Minimum EMA-VWAP distance filters: M10=", MIN_DISTANCE_M10, ", M15=", MIN_DISTANCE_M15, 
         " price units. Wait until next crossover: ", WaitUntilNextCrossover ? "Yes" : "No", 
         ", Safety max bars: ", MAX_WAIT_BARS);
}
   ...
   if(current_bar_time_M15 > last_bar_time_M15)
   {
      ...
      if(ema9_M15[2] < vwap_M15[2] && ema9_M15[1] > vwap_M15[1]) // Bullish crossover
      {
         if(last_crossover_m15 != 1)
         {
            ...
            // Clear any bearish pending signals as we now have an opposite crossover
            if(m15_bearish_pending)
            {
               m15_bearish_pending = false;
               Print("M15 Bearish pending signal canceled - new bullish crossover detected");
            }
            
            // Set pending flag for this new signal
            if(!PositionSelect(SYMBOL))
            {
               m15_bullish_pending = true;
               pending_bars_count = 0;
               if(WaitUntilNextCrossover)
                  Print("M15 Bullish signal pending distance confirmation. Will check until next crossover");
               else
      ...
      else if(ema9_M15[2] > vwap_M15[2] && ema9_M15[1] < vwap_M15[1]) // Bearish crossover
      {
         if(last_crossover_m15 != -1)
         {
            ...
            // Clear any bullish pending signals as we now have an opposite crossover
            if(m15_bullish_pending)
            {
               m15_bullish_pending = false;
               Print("M15 Bullish pending signal canceled - new bearish crossover detected");
            }
            ...
            if(!PositionSelect(SYMBOL))
            {
               ...
               pending_bars_count = 0;
               if(WaitUntilNextCrossover)
                  Print("M15 Bearish signal pending distance confirmation. Will check until next crossover");
               else
      ...
      if((m15_bullish_pending || m15_bearish_pending) && !PositionSelect(SYMBOL))
      {
         ...
         else
         {
            ...
            // If we're not waiting until next crossover, check if we've waited too long
            if(!WaitUntilNextCrossover && pending_bars_count >= MAX_WAIT_BARS)
            {
               if(m15_bullish_pending)
            ...
            {
               if(WaitUntilNextCrossover)
                  Print("Waiting for M15 distance confirmation. Current: ", ema_vwap_diff, ", Bar ", pending_bars_count, " (waiting until next crossover)");
               else
   ...
   if(current_bar_time_M10 > last_bar_time_M10)
   {
      ...
      if(ema9_M10[2] < vwap_M10[2] && ema9_M10[1] > vwap_M10[1]) // Bullish crossover
      {
         if(last_crossover_m10 != 1)
         {
            ...
            // Clear any bearish pending signals as we now have an opposite crossover
            if(m10_bearish_pending)
            {
               m10_bearish_pending = false;
               Print("M10 Bearish pending signal canceled - new bullish crossover detected");
            }
            ...
            if(!PositionSelect(SYMBOL))
            {
               ...
               pending_bars_count = 0;
               if(WaitUntilNextCrossover)
                  Print("M10 Bullish signal pending distance confirmation. Will check until next crossover");
               else
      ...
      else if(ema9_M10[2] > vwap_M10[2] && ema9_M10[1] < vwap_M10[1]) // Bearish crossover
      {
         if(last_crossover_m10 != -1)
         {
            ...
            if(m10_bullish_pending)
            {
               ...
               Print("M10 Bullish pending signal canceled - new bearish crossover detected");
            }
            ...
            if(!PositionSelect(SYMBOL))
            {
               ...
               pending_bars_count = 0;
               if(WaitUntilNextCrossover)
                  Print("M10 Bearish signal pending distance confirmation. Will check until next crossover");
               else
      ...
      // If new crossover and we want immediate check (no waiting)
      if(new_crossover && !WaitUntilNextCrossover && MAX_WAIT_BARS == 0 && !PositionSelect(SYMBOL))
      {
      ...
      else if((m10_bullish_pending || m10_bearish_pending) && !PositionSelect(SYMBOL))
      {
         ...
         if(m10_bullish_pending && ema_vwap_diff >= MIN_DISTANCE_M10)
         {
            ...
            {
               Print("Buy trade opened (M10 Entry with distance confirmation) with SL: ", sl, ". Distance: ", ema_vwap_diff, " units");
            }
         }
         // If we have a bearish pending signal
         else if(m10_bearish_pending && ema_vwap_diff >= MIN_DISTANCE_M10)
         {
            ...
            {
               Print("Sell trade opened (M10 Entry with distance confirmation) with SL: ", sl, ". Distance: ", ema_vwap_diff, " units");
            }
         }
         else
         {
            ...
            {
               if(WaitUntilNextCrossover)
                  Print("Waiting for M10 distance confirmation. Current: ", ema_vwap_diff, ", Bar ", pending_bars_count, " (waiting until next crossover)");
               else
...
void CheckEntryConditions_M10(double &ema9_M10[], double &vwap_M10[], double &vwap_M15_for_SL[])
{
   ...
   if(last_crossover_m10 == 1 && ema9_M10[1] > vwap_M10[1])
   {
      ...
      if(ema_vwap_diff >= MIN_DISTANCE_M10)
      {
         ...
         if(trade.Buy(FIXED_LOT_SIZE, SYMBOL, 0.0, sl, 0.0))
         {
            Print("Buy trade opened (M10 Entry) with SL: ", sl, " (based on M15 VWAP). Distance: ", ema_vwap_diff, " units");
         }
      }
      else
      {
   ...
   else if(last_crossover_m10 == -1 && ema9_M10[1] < vwap_M10[1])
   {
      ...
      if(ema_vwap_diff >= MIN_DISTANCE_M10)
      {
         ...
         if(trade.Sell(FIXED_LOT_SIZE, SYMBOL, 0.0, sl, 0.0))
         {
            Print("Sell trade opened (M10 Entry) with SL: ", sl, " (based on M15 VWAP). Distance: ", ema_vwap_diff, " units");
         }
      }
      else
      {
