...
{
    // Initialize log file
    fileHandle = FileOpen("KC_4.7_BE_Improvisation.txt", FILE_WRITE|FILE_TXT);
    ...
    LogMessage("=== KC 4.7 BE Improvisation EA ===");
    ...
    LogMessage("KC 4.7 BE Improvisation EA initialized successfully");
...
bool CheckStrongSignalRecovery()
{
    ...
    // Only run on new candle data (60 second intervals)
    datetime currentTime = TimeCurrent();
    if(currentTime <= lastBreakEvenCheckTime + 60)
        return false;
...
bool IsStopLossHit()
{
    ...
    if(currentStopLossLevel != 0)
    {
           ...
           (!isLong && currentAsk >= currentStopLossLevel))
        {
            ...
            string slType = safetyNetActivated ? "safety net" : (breakEvenActivated ? "break-even" : "VWAP");
    ...
    if(!breakEvenActivated && !safetyNetActivated && UseStopLossAtVWAP &&
...
void OnTick()
{
    ...
    if(UseBreakEven && (inLongPosition || inShortPosition) && !breakEvenActivated && !safetyNetActivated)
    {
        ...
        if(hadStrongSignal)
        {
            bool safetyNetResult = CheckStrongSignalRecovery();
            if(safetyNetResult)
            {
        ...
        {
            bool breakEvenResult = CheckBreakEvenConditions();
            if(breakEvenResult)
            {
...
void OnDeinit(const int reason)
{
    ...
    Print("KC 4.7 BE Improvisation EA deinitialized");
}