...
input group "TRADE SETTING IN POINTS"; // TRADE SETTINGS
    ...
    input double InpMinPriceMovementFactor = 0.1; //TickFilter_Factor of Point for min price movement (0.1 = 10% of a point)
    input int InpMinTimeInterval = 1;          // TickFilter_Minimum time interval in seconds for processing a tick
    input bool InpAutoAdjustParametersForIndices = true; // Auto-adjust Delta, Stop, MaxDistance for Index-like symbols?
    input int InpPendingOrderExpirationMinutes = 60; // Pending Order Expiration in Minutes (0 for GTC)
...
//Fields
    ...
    static double g_atrMain = 0;
    static double g_atrLong = 0; // For regime detection (e.g., period 50)
    static double g_atrMicroBreakout = 0;   // For Micro-Breakout module
    static double g_atrAdaptiveTrail = 0; // For Adaptive Exit module
    static double g_adxValue = 0;
    static datetime g_lastIndicatorCalcTime = 0;
    // Static Indicator Handles (Global)
    static int s_atrHandleMain = INVALID_HANDLE;
    static int s_atrHandleLong = INVALID_HANDLE;
    static int s_atrHandleMicroBreakout = INVALID_HANDLE;
    static int s_atrHandleAdaptiveTrail = INVALID_HANDLE;
    static int s_adxHandle = INVALID_HANDLE;

// Function to cleanup indicator handles (called from OnDeinit)
void CleanupIndicatorHandles() {
    if(s_atrHandleMain != INVALID_HANDLE) {IndicatorRelease(s_atrHandleMain); s_atrHandleMain = INVALID_HANDLE; }
    if(s_atrHandleLong != INVALID_HANDLE) { IndicatorRelease(s_atrHandleLong); s_atrHandleLong = INVALID_HANDLE; }
    if(s_atrHandleMicroBreakout != INVALID_HANDLE) { IndicatorRelease(s_atrHandleMicroBreakout); s_atrHandleMicroBreakout = INVALID_HANDLE; }
    if(s_atrHandleAdaptiveTrail != INVALID_HANDLE) { IndicatorRelease(s_atrHandleAdaptiveTrail); s_atrHandleAdaptiveTrail = INVALID_HANDLE; }
    if(s_adxHandle != INVALID_HANDLE) { IndicatorRelease(s_adxHandle); s_adxHandle = INVALID_HANDLE; }
}
//more fields
    // Working variables for indicator periods (to be initialized in OnInit)
    int workingVolatilityPeriod; // From main input VolatilityPeriod
    int workingMB_MomentumPeriod; // From InpMB_MomentumPeriod (Advanced Modules)
    int workingAE_AdaptiveTrail_VolatilityPeriod; // From InpAE_AdaptiveTrail_VolatilityPeriod (Advanced Modules)
    int workingAdxPeriod = 14; // Default ADX period, can be an input later if needed
    ...
    // Market Regime variables are now defined in HFT_ADVANCED_MODULES.mqh
    ...
    input int InpMaxConsecutiveLosses = 99;  // Max Consecutive Losses Allowed
    ...
    bool workingAutoAdjustParametersForIndices; // Added working variable
    bool workingEnableBaselineEntry; // Added working variable
    // Working variables for Breakeven settings
    bool workingUseBreakeven;
    double workingBreakevenProfitPoints;
    double workingBreakevenPlusPoints;
    bool workingDisableInitialStopLoss;
    ...
    ulong workingPendingOrderExpirationSeconds;
input group "BREAKEVEN SETTINGS"; // Breakeven Settings
    input bool   InpUseBreakeven = true; // Use Breakeven Stop?
    input double InpBreakevenProfitPoints = 10; // Profit in Points to activate Breakeven (e.g. 10 for US30 means $10 profit if 1 lot)
    input double InpBreakevenPlusPoints = 1;  // Set SL this many Points beyond Open Price for Breakeven (e.g. 1 point)
    input bool   InpDisableInitialStopLoss = false; // Disable Initial SL (relies on Breakeven or other exits)?
#include "HFT_ADVANCED_MODULES.mqh"  // Include the new advanced modules
...
double CalculateVolatility(int period) {
    // Dispatch based on the requested period, using globally cached ATR values
    if (period == workingVolatilityPeriod) {
        return (g_atrMain > 0) ? g_atrMain : (AverageSpread > 0 ? AverageSpread : SymbolInfoDouble(_Symbol, SYMBOL_POINT) * 10); // Fallback
    }
    if (period == 50) { // Period for g_atrLong (regime detection)
        return (g_atrLong > 0) ? g_atrLong : g_atrMain; // Fallback to g_atrMain
    }
    if (InpEnableMicroBreakoutEntry && period == workingMB_MomentumPeriod) {
        return (g_atrMicroBreakout > 0) ? g_atrMicroBreakout : g_atrMain; // Fallback
    }
    if (InpEnableAdaptiveExit && period == workingAE_AdaptiveTrail_VolatilityPeriod) {
        return (g_atrAdaptiveTrail > 0) ? g_atrAdaptiveTrail : g_atrMain; // Fallback
    }

    // If no specific global ATR matches, or if a module is disabled, provide a general fallback.
    // This part tries to calculate on-the-fly if a non-cached period is requested.
    // However, the design goal is to pre-calculate all *needed* ATRs in OnTimer.
    // For unexpected periods, it might be better to return a default or g_atrMain.
    if (period <= 0) {
        // PrintFormat("CalculateVolatility: Invalid period %d. Returning g_atrMain or fallback.", period);
        return (g_atrMain > 0) ? g_atrMain : (AverageSpread > 0 ? AverageSpread : SymbolInfoDouble(_Symbol, SYMBOL_POINT) * 10);
    }
    
    // Fallback for un-cached periods (should ideally not be hit if all used periods are cached in OnTimer)
    static int dynamicAtrHandle = INVALID_HANDLE;
    static int lastDynamicPeriod = -1;

    if (period != lastDynamicPeriod) { // Recreate handle if period changed
        if (dynamicAtrHandle != INVALID_HANDLE) IndicatorRelease(dynamicAtrHandle); // Release previous if any
        dynamicAtrHandle = iATR(_Symbol, Period(), period);
        lastDynamicPeriod = period;
    }

    if (dynamicAtrHandle == INVALID_HANDLE) {
        // PrintFormat("CalculateVolatility: Failed to create dynamic ATR handle for period %d. Using g_atrMain or fallback.", period);
        return (g_atrMain > 0) ? g_atrMain : (AverageSpread > 0 ? AverageSpread : SymbolInfoDouble(_Symbol, SYMBOL_POINT) * 10);
    }
    
    double atrValueArr[];
    if (CopyBuffer(dynamicAtrHandle, 0, 0, 1, atrValueArr) > 0 && atrValueArr[0] > 0) {
        return atrValueArr[0];
    }
    
    // PrintFormat("CalculateVolatility: Failed to copy dynamic ATR for period %d. Using g_atrMain or fallback.", period);
    return (g_atrMain > 0) ? g_atrMain : (AverageSpread > 0 ? AverageSpread : SymbolInfoDouble(_Symbol, SYMBOL_POINT) * 10);
}
...
double CalculateTrendStrength(int period=14) { // Default period for ADX is often 14
    // The ADX value is now pre-calculated in OnTimer and stored in g_adxValue.
    // The 'period' argument here is mainly for API compatibility if called from elsewhere,
    // but the returned value will be from the globally managed g_adxValue using workingAdxPeriod.
    if (period != workingAdxPeriod) {
        // Optionally, log a warning if a different period is requested than what's cached.
        // For now, we return the globally cached value regardless of the period argument here,
        // as the design is to centralize calculation with workingAdxPeriod.
        // PrintFormat("CalculateTrendStrength: Requested period %d differs from cached period %d. Returning cached ADX.", period, workingAdxPeriod);
    }

    return (g_adxValue >= 0 && g_adxValue <=100) ? MathMin(g_adxValue / 100.0, 1.0) : 0.5; // Return normalized g_adxValue or neutral
}
...
//---
   ...
   workingAutoAdjustParametersForIndices = InpAutoAdjustParametersForIndices; // Initialize working variable
   workingEnableBaselineEntry = InpEnableBaselineEntry; // Initialize working variable
   workingPendingOrderExpirationSeconds = (InpPendingOrderExpirationMinutes < 0) ? 0 : (ulong)InpPendingOrderExpirationMinutes * 60;

   // Initialize Breakeven working variables
   workingUseBreakeven = InpUseBreakeven;
   workingBreakevenProfitPoints = InpBreakevenProfitPoints;
   workingBreakevenPlusPoints = InpBreakevenPlusPoints;
   workingDisableInitialStopLoss = InpDisableInitialStopLoss;
   ...
   // Initialize working variables for indicator periods
   workingVolatilityPeriod = VolatilityPeriod; // Main EA volatility period
   workingMB_MomentumPeriod = InpMB_MomentumPeriod; // From HFT_ADVANCED_MODULES.mqh inputs
   workingAE_AdaptiveTrail_VolatilityPeriod = InpAE_AdaptiveTrail_VolatilityPeriod; // From HFT_ADVANCED_MODULES.mqh inputs
   // workingAdxPeriod is already defaulted to 14, can be made an input later if needed

   // Initialize indicator handles
   if(workingVolatilityPeriod > 0) {
      s_atrHandleMain = iATR(_Symbol, Period(), workingVolatilityPeriod);
      if(s_atrHandleMain == INVALID_HANDLE) {
         Print("Failed to create ATR Main handle. Error: ", GetLastError());
         return INIT_FAILED;
      }
   }

   s_atrHandleLong = iATR(_Symbol, Period(), 50); // Long-term ATR period
   if(s_atrHandleLong == INVALID_HANDLE) {
      Print("Failed to create ATR Long handle. Error: ", GetLastError());
      return INIT_FAILED;
   }

   if(InpEnableMicroBreakoutEntry && workingMB_MomentumPeriod > 0) {
      s_atrHandleMicroBreakout = iATR(_Symbol, Period(), workingMB_MomentumPeriod);
      if(s_atrHandleMicroBreakout == INVALID_HANDLE) {
         Print("Failed to create ATR MicroBreakout handle. Error: ", GetLastError());
         return INIT_FAILED;
      }
   }

   if(InpEnableAdaptiveExit && workingAE_AdaptiveTrail_VolatilityPeriod > 0) {
      s_atrHandleAdaptiveTrail = iATR(_Symbol, Period(), workingAE_AdaptiveTrail_VolatilityPeriod);
      if(s_atrHandleAdaptiveTrail == INVALID_HANDLE) {
         Print("Failed to create ATR AdaptiveTrail handle. Error: ", GetLastError());
         return INIT_FAILED;
      }
   }

   if(workingAdxPeriod > 0) {
      s_adxHandle = iADX(_Symbol, Period(), workingAdxPeriod);
      if(s_adxHandle == INVALID_HANDLE) {
...
// ... existing code ...
   ...
   // Dynamic Parameter Adjustment for Index-Like Symbols (before initial UpdateMarketAndTradeParameters and Validation)
   if(workingAutoAdjustParametersForIndices) {
      double currentPoint = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
      double currentTickSize = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);
      long stopsLevel = SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
      double currentAsk = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      double currentBid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      double initialSpreadPoints = (currentAsk - currentBid) / currentPoint;
      initialSpreadPoints = MathMax(initialSpreadPoints, 1.0); // Ensure spread is at least 1 point for calculations

      // Heuristic for index-like: high stops_level (e.g. > 50) or point == tick_size (common for indices)
      // And that _Digits indicates it's not a typical FX pair (e.g., _Digits <= 2 for many indices)
      bool isIndexLike = (stopsLevel > 50 || (currentPoint == currentTickSize && _Digits <= 2) );

      if(isIndexLike) {
         PrintFormat("INFO: Symbol %s detected as Index-like. Attempting dynamic parameter adjustment based on initial spread of %.1f points.", _Symbol, initialSpreadPoints);
         PrintFormat("INFO: Original Input Multipliers: Delta=%.1f, Stop=%.1f, MaxDistance=%.1f", Delta, Stop, MaxDistance);

         // Interpret inputs as target points, then derive new multipliers
         // workingDelta will aim for Delta points away
         workingDelta = Delta / initialSpreadPoints;
         // workingStop will aim for Stop points away
         workingStop = Stop / initialSpreadPoints;
         // workingMaxDistance will aim for MaxDistance points away
         workingMaxDistance = MaxDistance / initialSpreadPoints;

         // Sanity checks for derived multipliers
         workingDelta = MathMax(0.5, MathMin(workingDelta, 20.0)); // Multiplier between 0.5x and 20x of spread
         workingStop = MathMax(1.0, MathMin(workingStop, 50.0));   // Multiplier between 1x and 50x of spread
         workingMaxDistance = MathMax(workingDelta + 0.1, MathMin(workingMaxDistance, 100.0)); // Ensure MaxDistance > Delta

         PrintFormat("INFO: Dynamically Adjusted Multipliers: workingDelta=%.2f, workingStop=%.2f, workingMaxDistance=%.2f", 
                     workingDelta, workingStop, workingMaxDistance);

         // Update Base parameters with these new working multipliers
         BaseOrderDistance = workingDelta; 
         BaseStopLoss = workingStop;
         BaseMaxDistance = workingMaxDistance;
      } else {
         PrintFormat("INFO: Symbol %s not treated as Index-like for dynamic adjustment. Using direct input multipliers.", _Symbol);
         // Ensure base parameters are set from the original inputs if no dynamic adjustment
         BaseOrderDistance = Delta;
         BaseStopLoss = Stop;
         BaseMaxDistance = MaxDistance;
      }
   } else {
      // If auto-adjust is off, ensure base parameters are correctly initialized from original inputs
      // (This also covers the case where it's not index-like and auto-adjust was on)
      BaseOrderDistance = Delta;
      BaseStopLoss = Stop;
      BaseMaxDistance = MaxDistance;
   }
...
SpreadMultiplier = 1.0; // Defaulting to 1.0 as per refactoring plan
...
// ... existing code ...
   ...
   InitializeRegimeParameterSets(); // Initialize regime-specific parameter sets
...
//---
   ...
   CleanupIndicatorHandles(); // Cleanup indicator handles
  }
//+------------------------------------------------------------------+
...
void UpdateMarketAndTradeParameters()
{
   ...
   // Calculate average spread using direct symbol information
   double tickValue = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE);
   double tickSize = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);
   double pointValue = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   
   // Calculate average spread without commission
   AverageSpread = MathMax(SpreadMultiplier * pointValue, CurrentSpread);
   ...
   // Safeguard: Prevent CalculatedStopLoss from becoming unreasonably large
   double pipSize = (_Digits == 3 || _Digits == 5) ? pointValue * 10 : pointValue;
   double maxReasonableStopFactor = 5.0; // Allow SL to be up to 5x MinStopDistance
   double reasonablePipsBasedLimit = 500 * pipSize; // 500 "pips" (which are points for indices if _Digits=2)
   double factorComponent = (MinStopDistance > 0) ? MinStopDistance * maxReasonableStopFactor : reasonablePipsBasedLimit;
   double maxReasonableStopLossPriceUnits = MathMax(MathMax(reasonablePipsBasedLimit, factorComponent), MinStopDistance > 0 ? MinStopDistance : reasonablePipsBasedLimit); 
   
   if(CalculatedStopLoss > maxReasonableStopLossPriceUnits && maxReasonableStopLossPriceUnits > 0) { // ensure limit is positive
    //   PrintFormat("WARNING (UpdateMarketParams): CalculatedStopLoss (%.5f) exceeds reasonable limit (%.5f). AvgSpread=%.5f, workStop=%.1f, MinStopDist=%.5f. Using fallback.", 
    //               CalculatedStopLoss, maxReasonableStopLossPriceUnits, AverageSpread, workingStop, MinStopDistance);
      CalculatedStopLoss = MathMax(workingStop * pipSize, MinStopDistance > 0 ? MinStopDistance : (workingStop * pipSize)); 
   }
   
   // Debug logging for first few calculations
   static int debugCount = 0;
   if(debugCount < 3) {
      PrintFormat("UpdateMarketParams DEBUG: AvgSpread=%.5f, workDelta=%.1f, workStop=%.1f, CalcSL=%.5f, MinStopDist=%.5f, AdjOrderDist=%.5f", 
                  AverageSpread, workingDelta, workingStop, CalculatedStopLoss, MinStopDistance, AdjustedOrderDistance);
      debugCount++;
   }
}
...
void ManageOpenPositions()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      // Use enhanced exit strategy manager from advanced modules
      EnhancedManageExitStrategy(ticket, openPrice, type);

      // The existing trailing stop logic as fallback (if advanced exits are disabled)
      if(!InpEnableAdaptiveExit && !InpEnableOpportunityCostExit) {
         double sl = posinfo.StopLoss();
         double tp = posinfo.TakeProfit();

         if(type == POSITION_TYPE_BUY)
         {
            double priceMove = MathMax(currentBid - openPrice, 0);
            ...
            double triggerLevel = openPrice + TrailingStopIncrement;
         ...
         {
            double priceMove = MathMax(openPrice - currentAsk, 0);
            double trailDist = CalculateTrailingStop(priceMove, MinStopDistance, TrailingStopActive, BaseTrailingStop, TrailingStopMax);
...
void OnTimer()
{
   ...
   if (currentTime != g_lastIndicatorCalcTime) {
      double atrVals[];
      
      // --- ATR Main Handle Management ---
      if (s_atrHandleMain == INVALID_HANDLE && workingVolatilityPeriod > 0) {
         s_atrHandleMain = iATR(_Symbol, Period(), workingVolatilityPeriod);
         if(s_atrHandleMain == INVALID_HANDLE) {
            PrintFormat("WARNING: Failed to create ATR Main handle. Error: %d", GetLastError());
         }
      }
      
      // --- ATR Long Handle Management ---
      if (s_atrHandleLong == INVALID_HANDLE) {
         s_atrHandleLong = iATR(_Symbol, Period(), 50); // Long-term ATR period
         if(s_atrHandleLong == INVALID_HANDLE) {
            PrintFormat("WARNING: Failed to create ATR Long handle. Error: %d", GetLastError());
         }
      }
      
      // --- ATR MicroBreakout Handle Management ---
      if (InpEnableMicroBreakoutEntry && s_atrHandleMicroBreakout == INVALID_HANDLE && workingMB_MomentumPeriod > 0) {
         s_atrHandleMicroBreakout = iATR(_Symbol, Period(), workingMB_MomentumPeriod);
         if(s_atrHandleMicroBreakout == INVALID_HANDLE) {
            PrintFormat("WARNING: Failed to create ATR MicroBreakout handle. Error: %d", GetLastError());
         }
      }
      
      // --- ATR Adaptive Trail Handle Management ---
      if (InpEnableAdaptiveExit && s_atrHandleAdaptiveTrail == INVALID_HANDLE && workingAE_AdaptiveTrail_VolatilityPeriod > 0) {
         s_atrHandleAdaptiveTrail = iATR(_Symbol, Period(), workingAE_AdaptiveTrail_VolatilityPeriod);
         if(s_atrHandleAdaptiveTrail == INVALID_HANDLE) {
            PrintFormat("WARNING: Failed to create ATR AdaptiveTrail handle. Error: %d", GetLastError());
         }
      }
      
      // --- ADX Handle Management ---
      if (s_adxHandle == INVALID_HANDLE && workingAdxPeriod > 0) {
         s_adxHandle = iADX(_Symbol, Period(), workingAdxPeriod);
         if(s_adxHandle == INVALID_HANDLE) {
            PrintFormat("WARNING: Failed to create ADX handle. Error: %d", GetLastError());
         }
      }

      // --- Copy Buffer Operations ---
      // Main ATR
      if (s_atrHandleMain != INVALID_HANDLE) {
         if (CopyBuffer(s_atrHandleMain, 0, 0, 1, atrVals) > 0 && atrVals[0] > 0) {
            g_atrMain = atrVals[0];
         } else {
            g_atrMain = AverageSpread > 0 ? AverageSpread : SymbolInfoDouble(_Symbol, SYMBOL_POINT) * 10;
         }
      }

      // Long ATR
      if (s_atrHandleLong != INVALID_HANDLE) {
         if (CopyBuffer(s_atrHandleLong, 0, 0, 1, atrVals) > 0 && atrVals[0] > 0) {
            g_atrLong = atrVals[0];
         } else {
            g_atrLong = g_atrMain; // Fallback to main ATR
         }
      }

      // MicroBreakout ATR
      if (InpEnableMicroBreakoutEntry && s_atrHandleMicroBreakout != INVALID_HANDLE) {
         if (CopyBuffer(s_atrHandleMicroBreakout, 0, 0, 1, atrVals) > 0 && atrVals[0] > 0) {
            g_atrMicroBreakout = atrVals[0];
         } else {
            g_atrMicroBreakout = g_atrMain; // Fallback
         }
      }

      // Adaptive Trail ATR
      if (InpEnableAdaptiveExit && s_atrHandleAdaptiveTrail != INVALID_HANDLE) {
         if (CopyBuffer(s_atrHandleAdaptiveTrail, 0, 0, 1, atrVals) > 0 && atrVals[0] > 0) {
            g_atrAdaptiveTrail = atrVals[0];
         } else {
            g_atrAdaptiveTrail = g_atrMain; // Fallback
         }
      }
      ...
      double adxVals[];
      if (s_adxHandle != INVALID_HANDLE) {
         if (CopyBuffer(s_adxHandle, MAIN_LINE, 0, 1, adxVals) > 0 && adxVals[0] >= 0) {
            g_adxValue = adxVals[0];
         } else {
            g_adxValue = 50.0; // Neutral ADX value as fallback
         }
      }

      g_lastIndicatorCalcTime = currentTime;
   }

   // Update market data, average spread (historical), and less tick-sensitive parameters
   UpdateMarketAndTradeParameters();

   // Detect market regime (this will be used by UpdateAllDynamicParameters)
   currentMarketRegime = DetectMarketRegime(); 

   // Update all dynamic multipliers (_EA variables) from advanced modules
   UpdateAllDynamicParameters(); // This now only sets the _EA multipliers and MinOrderInterval_EA
...
void OnTrade()
{
   ...
   for(int i = dealsTotal - 1; i >= 0; i--)
   {
         ...
         HistoryDealGetInteger(dealTicket, DEAL_ENTRY) == DEAL_ENTRY_OUT)
      {
         ...
         if(CheckPointer(riskManager) == POINTER_DYNAMIC) {
            riskManager.RegisterTradeResult(isProfit);
         }
         
         // Handle trade result for advanced adaptive systems
         HandleTradeResultForAdaptiveSystems(isProfit);
         
         // Remove position state tracking for closed positions
...
void AdaptParametersToRegime(MARKET_REGIME regime) {
    // AdjustedOrderDistance is ALREADY in price units from UpdateMarketAndTradeParameters()
    // (e.g., AverageSpread * workingDelta)
    // BaseOrderDistance is the MULTIPLIER from input (e.g., workingDelta = 0.5)
    // We should scale the already calculated price-unit AdjustedOrderDistance, 
    // or recalculate it using a MODIFIED multiplier.

    // Let's define a regime-specific multiplier for Delta
    double deltaMultiplier = 1.0; // Default: no change to Delta

    switch(regime) {
        case REGIME_TRENDING:
            deltaMultiplier = 1.2; // Make Delta 20% larger
            // TrailingStopActive = (BaseTrailingStopValue * 1.5) * AverageSpread; // Keep this as is for now
            break;
        case REGIME_RANGING:
            deltaMultiplier = 0.8; // Make Delta 20% smaller
            // TrailingStopActive = (BaseTrailingStopValue * 0.7) * AverageSpread;
            break;
        case REGIME_VOLATILE:
            deltaMultiplier = 1.5; // Make Delta 50% larger
            // TrailingStopActive = (BaseTrailingStopValue * 2.0) * AverageSpread;
            break;
        case REGIME_QUIET:
            deltaMultiplier = 0.6; // Make Delta 40% smaller
            // TrailingStopActive = (BaseTrailingStopValue * 0.5) * AverageSpread;
            break;
        case REGIME_UNDEFINED:
        default:
            // deltaMultiplier remains 1.0, no change to Delta from its base value
            break;
    }

    // Recalculate AdjustedOrderDistance using the original base (workingDelta) modified by the regime multiplier
    double regimeAdjustedDelta = workingDelta * deltaMultiplier;
    AdjustedOrderDistance = MathMax(AverageSpread * regimeAdjustedDelta, MinStopDistance);

    // Trailing stop logic can be adjusted similarly if needed, but let's focus on order distance first.
    // For now, ensure TrailingStopActive is based on the current AverageSpread.
    TrailingStopActive = BaseTrailingStopValue * AverageSpread;
    TrailingStopActive = MathMax(TrailingStopActive, MinStopDistance); // Ensure min distance for trailing stop too

    // The final MathMax for AdjustedOrderDistance is redundant here as it's done in the line above,
    // but it's harmless to keep it. If it were removed, ensure the recalculation above includes it.
...
void ScaleParametersByVolatility() {
    ...
    CalculatedStopLoss *= scaleFactor; // Scale the existing price-unit CalculatedStopLoss
...
void ManageExitStrategy(ulong ticket, double entryPrice, ENUM_POSITION_TYPE posType) {
    ...
    if (posType == POSITION_TYPE_BUY) {
        priceMovePoints = (currentPriceBid - entryPrice);
    } else { 
        priceMovePoints = (entryPrice - currentPriceAsk);
    }
    ...
    // Breakeven Logic (New - Placed before Trailing Stop and Partial Close, after TP mod)
    if (workingUseBreakeven && !IsPartialClosed(ticket)) { // Don't move to BE if already partially closed, or let user decide
        double profitInAccountCurrency = posinfo.Profit(); // Includes swap and commission
        ...
        if (posType == POSITION_TYPE_BUY) {
            profitPoints = (currentPriceBid - openPrice) / pointValue; 
        } else { // POSITION_TYPE_SELL
            profitPoints = (openPrice - currentPriceAsk) / pointValue;
        }
        
        // Use workingBreakevenProfitPoints which is already in points from input
        if (profitPoints >= workingBreakevenProfitPoints) {
            double breakevenSLPrice = 0;
            if (posType == POSITION_TYPE_BUY) {
                breakevenSLPrice = NormalizeDouble(openPrice + (workingBreakevenPlusPoints * pointValue), _Digits);
                // Check if current SL is worse than breakevenSL or not set
                if (slToSet == 0 || breakevenSLPrice > slToSet) {
                    // Ensure new SL is valid
                    if (breakevenSLPrice < currentPriceBid - MinStopDistance) {
                        slToSet = breakevenSLPrice;
                        modifyOrder = true;
                        PrintFormat("ManageExitStrategy: Breakeven triggered for BUY #%I64u. Profit: %.2f points. New SL: %.5f", ticket, profitPoints, slToSet);
                    }
                }
            } else { // POSITION_TYPE_SELL
                breakevenSLPrice = NormalizeDouble(openPrice - (workingBreakevenPlusPoints * pointValue), _Digits);
                // Check if current SL is worse than breakevenSL or not set
                if (slToSet == 0 || breakevenSLPrice < slToSet) {
                     // Ensure new SL is valid and positive
                    if (breakevenSLPrice > currentPriceAsk + MinStopDistance && breakevenSLPrice > 0) {
                        slToSet = breakevenSLPrice;
                        modifyOrder = true;
                        PrintFormat("ManageExitStrategy: Breakeven triggered for SELL #%I64u. Profit: %.2f points. New SL: %.5f", ticket, profitPoints, slToSet);
                    }
                }
            }
        }
    ...
    // 3. Modify Position if SL or TP changed (or by breakeven)
    if (modifyOrder) {
...
bool SafeOrderSend(string symbol, ENUM_ORDER_TYPE orderType, double volume, double price, double sl, double tp, string comment)
{
    if(CheckPointer(errorHandler) == POINTER_INVALID) {
        ...
        // Define order expiration parameters for direct send
        ENUM_ORDER_TYPE_TIME directOrderTimeType = (workingPendingOrderExpirationSeconds == 0) ? ORDER_TIME_GTC : ORDER_TIME_SPECIFIED;
        datetime directExpirationTimestamp = (workingPendingOrderExpirationSeconds == 0) ? 0 : TimeCurrent() + workingPendingOrderExpirationSeconds;

        if (orderType == ORDER_TYPE_BUY_STOP) {
            return trade.BuyStop(volume, price, symbol, sl, tp, directOrderTimeType, directExpirationTimestamp, comment);
        } else if (orderType == ORDER_TYPE_SELL_STOP) {
            return trade.SellStop(volume, price, symbol, sl, tp, directOrderTimeType, directExpirationTimestamp, comment);
        } else {
            // Market orders do not use expiration like pending orders
    ...
    // Define order expiration parameters for retriable send
    ENUM_ORDER_TYPE_TIME orderTimeType = (workingPendingOrderExpirationSeconds == 0) ? ORDER_TIME_GTC : ORDER_TIME_SPECIFIED;
    datetime expirationTimestamp = (workingPendingOrderExpirationSeconds == 0) ? 0 : TimeCurrent() + workingPendingOrderExpirationSeconds;
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        if (orderType == ORDER_TYPE_BUY_STOP || orderType == ORDER_TYPE_SELL_STOP) {
            ...
            if (orderType == ORDER_TYPE_BUY_STOP) {
                result = trade.BuyStop(volume, price, symbol, sl, tp, orderTimeType, expirationTimestamp, comment);
            } else {
                result = trade.SellStop(volume, price, symbol, sl, tp, orderTimeType, expirationTimestamp, comment);
            }
        } else {
            // Use OrderOpen for market orders (GTC is standard, no expiration needed here)
...
void BenchmarkCriticalFunctionsEA() {
    ...
    durationMicroseconds = GetMicrosecondCount() - startMicroseconds;
...
bool ValidateInputParameters() {
    ...
    if (InpPendingOrderExpirationMinutes < 0) {
        validationMessage += "WARNING: Pending Order Expiration Minutes (InpPendingOrderExpirationMinutes) is negative. Will be treated as GTC (0 minutes).\\n";
        // OnInit handles the assignment to workingPendingOrderExpirationSeconds
    }
    ...
    // Use workingDelta and workingMaxDistance for validation as these are used by the EA
    if (workingMaxDistance <= workingDelta) {
        validationMessage += "ERROR: Effective Max Order Distance (workingMaxDistance: " + DoubleToString(workingMaxDistance,2) + ") must be greater than effective Delta (workingDelta: " + DoubleToString(workingDelta,2) + "). Check MaxDistance and Delta inputs.\\n";
        isValid = false;
    }
    if (Stop <= 0) {
    ...
    // Use MinStopDistance which is already in price units (points * _Point)
    // The CalculatedStopLoss logic already ensures it's MathMax(AverageSpread * workingStop, MinStopDistance)
    if (MinStopDistance > 0 && (workingStop * AverageSpread) < MinStopDistance && AverageSpread > 0) {
         validationMessage += "INFO: Base Stop Loss (workingStop * AverageSpread = " + DoubleToString(workingStop * AverageSpread, _Digits) + ") is less than broker's MinStopDistance (" + DoubleToString(MinStopDistance, _Digits) + "). EA automatically adjusts to MinStopDistance.\\n";
    }
    if (MaxTrailing <= 0) {
...
//+------------------------------------------------------------------+
//| Update Tick-Specific Parameters                                  |
//+------------------------------------------------------------------+
void UpdateTickSpecificParameters(double currentAsk, double currentBid)
{
   if (workingDelta_EA <= 0 || workingStop_EA <= 0) { // Check if EA multipliers are initialized
      // This might happen if OnTimer hasn't run yet or advanced modules aren't enabled/working
      // Fallback to base input parameters if _EA versions are not ready
      if (BaseOrderDistance > 0 && BaseStopLoss > 0) {
         double instantSpread = MathMax(currentAsk - currentBid, _Point); // Ensure spread is at least 1 point
         AdjustedOrderDistance = MathMax(instantSpread * BaseOrderDistance, MinStopDistance);
         CalculatedStopLoss = MathMax(instantSpread * BaseStopLoss, MinStopDistance);
         TrailingStopActive = MathMax(instantSpread * BaseTrailingStopValue, MinStopDistance); // Use BaseTrailingStopValue
      } else {
         // Further fallback if base parameters are also not set (should not happen in normal operation)
         AdjustedOrderDistance = MinStopDistance * 2;
         CalculatedStopLoss = MinStopDistance * 3;
         TrailingStopActive = MinStopDistance * 2;
         static bool fallbackLogged = false;
         if(!fallbackLogged){
            PrintFormat("CRITICAL WARNING: UpdateTickSpecificParameters using hardcoded fallbacks. Check EA multiplier initialization. Ask=%.5f, Bid=%.5f", currentAsk, currentBid);
            fallbackLogged = true;
         }
      }
   ...
   // Use _EA multipliers set by OnTimer via Advanced Modules
   AdjustedOrderDistance = MathMax(instantSpread * workingDelta_EA, MinStopDistance);
   CalculatedStopLoss = MathMax(instantSpread * workingStop_EA, MinStopDistance);
   // Ensure workingMaxTrailing_EA is used for TrailingStopActive
   TrailingStopActive = MathMax(instantSpread * workingMaxTrailing_EA, MinStopDistance);

   // Optional: Add a debug print here for the first few ticks to verify values
   static int tickParamDebugCount = 0;
   if (tickParamDebugCount < 5) {
      PrintFormat("UpdateTickSpecificParams: Ask=%.5f, Bid=%.5f, Spread=%.5f, workDelta_EA=%.2f, workStop_EA=%.2f, workTrail_EA=%.2f -> AdjOrderDist=%.5f, CalcSL=%.5f, TrailActive=%.5f",
         currentAsk, currentBid, instantSpread, workingDelta_EA, workingStop_EA, workingMaxTrailing_EA,
         AdjustedOrderDistance, CalculatedStopLoss, TrailingStopActive);
      tickParamDebugCount++;
   }
}
...
void OnTick()
{
   ...
   // Check emergency conditions first (from advanced modules)
   CheckEmergencyConditions();
   
   // If emergency pause is active, skip normal trading logic
   ...
   // Update tick-specific parameters using current Ask/Bid and _EA multipliers
   UpdateTickSpecificParameters(Ask, Bid);
   
   // The old simple EMA for AverageSpread is removed.
   // AverageSpread (historical from spreadHistory) is updated in OnTimer by UpdateMarketAndTradeParameters()
   // CalculatedStopLoss is now updated by UpdateTickSpecificParameters based on instant spread.
   // The safeguard for CalculatedStopLoss is still in UpdateMarketAndTradeParameters (for the historical one) 
   // and a similar one might be needed in UpdateTickSpecificParameters or after its call if critical.
   // For now, we assume MinStopDistance and the multipliers handle reasonable bounds for HFT reaction.

   // Safeguard: Prevent CalculatedStopLoss from becoming unreasonably large (using pipSizeForOnTick)
   // This check is now more relevant here after CalculateStopLoss is updated by UpdateTickSpecificParameters
   double pipSizeForOnTick = (_Digits == 3 || _Digits == 5) ? _Point * 10 : _Point;
   double maxReasonableStopFactorOnTick = 5.0; 
   double reasonablePipsBasedLimitOnTick = 500 * pipSizeForOnTick; 
   double factorComponentOnTick = (MinStopDistance > 0) ? MinStopDistance * maxReasonableStopFactorOnTick : reasonablePipsBasedLimitOnTick;
   double maxReasonableStopLossPriceUnitsOnTick = MathMax(MathMax(reasonablePipsBasedLimitOnTick, factorComponentOnTick), MinStopDistance > 0 ? MinStopDistance : reasonablePipsBasedLimitOnTick);

   if(CalculatedStopLoss > maxReasonableStopLossPriceUnitsOnTick && maxReasonableStopLossPriceUnitsOnTick > 0) { // ensure limit is positive
    //   PrintFormat("WARNING (OnTick-PostUpdate): CalculatedStopLoss (%.5f) from UpdateTickSpecificParameters exceeds reasonable limit (%.5f). MinStopDist=%.5f. Using fallback.", 
    //               CalculatedStopLoss, maxReasonableStopLossPriceUnitsOnTick, MinStopDistance);
      // Fallback to a value based on workingStop_EA (if available) or BaseStopLoss times pipSize, ensuring MinStopDistance
      double slMultiplier = (workingStop_EA > 0) ? workingStop_EA : BaseStopLoss;
      CalculatedStopLoss = MathMax(slMultiplier * pipSizeForOnTick, MinStopDistance > 0 ? MinStopDistance : (slMultiplier * pipSizeForOnTick)); 
   }
   ...
   // Define order expiration parameters
   ENUM_ORDER_TYPE_TIME orderTimeType = (workingPendingOrderExpirationSeconds == 0) ? ORDER_TIME_GTC : ORDER_TIME_SPECIFIED;
   datetime expirationTimestamp = (workingPendingOrderExpirationSeconds == 0) ? 0 : TimeCurrent() + workingPendingOrderExpirationSeconds;

   // Check for advanced entry signals first
   ENUM_ORDER_TYPE advancedOrderType;
   double advancedEntryPrice, advancedStopLoss, advancedTakeProfit;
   
   if(CheckAdvancedEntrySignals(advancedOrderType, advancedEntryPrice, advancedStopLoss, advancedTakeProfit)) {
      // Use MinOrderInterval_EA from advanced modules instead of MinOrderInterval
      bool canPlaceAdvancedOrder = false;
      
      if(advancedOrderType == ORDER_TYPE_BUY_STOP && (CurrentTime - LastBuyOrderTime) > MinOrderInterval_EA) {
         canPlaceAdvancedOrder = true;
      } else if(advancedOrderType == ORDER_TYPE_SELL_STOP && (CurrentTime - LastSellOrderTime) > MinOrderInterval_EA) {
         canPlaceAdvancedOrder = true;
      }
      
      if(canPlaceAdvancedOrder) {
         double stopLossDistance = MathAbs(advancedEntryPrice - advancedStopLoss);
         double lotSize = CalculateOptimalLotSize(stopLossDistance);
         
         // Apply lot size diagnostics
         lotSize = ApplyLotSizeDiagnostics(lotSize, stopLossDistance);
         ...
         if(lotSize >= MinLotSize && CheckPointer(riskManager) == POINTER_DYNAMIC) {
            ...
            if(riskManager.IsTradeAllowed(lotSize, stopLossPips)) {
               ...
               if(advancedOrderType == ORDER_TYPE_BUY_STOP) {
                  orderResult = trade.BuyStop(lotSize, advancedEntryPrice, _Symbol, 
                                            workingDisableInitialStopLoss ? 0 : advancedStopLoss, 
                                            advancedTakeProfit, orderTimeType, expirationTimestamp, "HFT_ADV_BUY");
                  if(orderResult) LastBuyOrderTime = CurrentTime;
               } else {
                  orderResult = trade.SellStop(lotSize, advancedEntryPrice, _Symbol, 
                                             workingDisableInitialStopLoss ? 0 : advancedStopLoss, 
                                             advancedTakeProfit, orderTimeType, expirationTimestamp, "HFT_ADV_SELL");
                  if(orderResult) LastSellOrderTime = CurrentTime;
               }
               
               if(orderResult) {
                  riskManager.RegisterTradeRisk(lotSize, stopLossPips);
                  
                  // Add position state tracking for advanced exits
                  ulong ticket = trade.ResultOrder();
                  if(ticket > 0) {
                     AddPositionState(ticket, stopLossDistance, currentMarketRegime, 
                                    CalculateVolatility(VolatilityPeriod), stopLossDistance);
                  }
                  ...
                  // Skip regular order placement logic since we placed an advanced order
                  if(CheckPointer(perfMonitor) == POINTER_DYNAMIC) {
                     perfMonitor.EndTickMeasurement();
                  }
                  return;
   ...
   // Quick order placement logic (original logic with MinOrderInterval_EA)
   if(needsBuyOrder && (CurrentTime - LastBuyOrderTime) > MinOrderInterval_EA && workingEnableBaselineEntry) { // Added workingEnableBaselineEntry condition
      double entryPrice = Ask + AdjustedOrderDistance;
      double stopLoss = entryPrice - CalculatedStopLoss;
      
      if(entryPrice > Ask + MinStopDistance && stopLoss > 0 && CalculatedStopLoss > 0) { // Ensure CalculatedStopLoss is positive
         double lotSize = CalculateOptimalLotSize(CalculatedStopLoss);
         
         // Apply lot size diagnostics
         lotSize = ApplyLotSizeDiagnostics(lotSize, CalculatedStopLoss);
         
         if(lotSize >= MinLotSize && CheckPointer(riskManager) == POINTER_DYNAMIC) {
            double pipSize = (_Digits == 3 || _Digits == 5) ? _Point * 10 : _Point;
            double stopLossPips = CalculatedStopLoss / pipSize; 
            
            // Debug logging for lot calculation parameters
            static int lotCalcDebugCount = 0;
            if (lotCalcDebugCount < 5) {
                PrintFormat("OnTick LotCalc DEBUG: LotSize=%.2f, CalcSL=%.5f (%.1f pips), MinLot=%.2f, RiskAllowsTrade=%s (details below)", 
                            lotSize, CalculatedStopLoss, stopLossPips, MinLotSize, riskManager.IsTradeAllowed(lotSize, stopLossPips) ? "true" : "false");
                lotCalcDebugCount++;
            }

            if(riskManager.IsTradeAllowed(lotSize, stopLossPips)) { // This also updates internal risk calculation for the check
                double marginRequired = 0;
                if(!OrderCalcMargin(ORDER_TYPE_BUY_STOP, _Symbol, lotSize, entryPrice, marginRequired)) {
                    PrintFormat("OnTick: OrderCalcMargin failed for BUY_STOP. Lot: %.2f, Price: %.5f. Error: %d", lotSize, entryPrice, GetLastError());
                } else {
                    double freeMargin = AccountInfoDouble(ACCOUNT_MARGIN_FREE);
                    if (marginRequired > freeMargin) {
                        PrintFormat("OnTick: Insufficient free margin for BUY_STOP. Required: %.2f, Free: %.2f. Lot: %.2f. Skipping trade.", marginRequired, freeMargin, lotSize);
                    } else {
                        double slToSend = workingDisableInitialStopLoss ? 0 : stopLoss;
                        if(trade.BuyStop(lotSize, entryPrice, _Symbol, slToSend, 0, orderTimeType, expirationTimestamp, "HFT_BUY")) {
                            riskManager.RegisterTradeRisk(lotSize, stopLossPips); // Only register risk if order successfully placed
                            LastBuyOrderTime = CurrentTime;
                            
                            // Add position state tracking
                            ulong ticket = trade.ResultOrder();
                            if(ticket > 0) {
                               AddPositionState(ticket, CalculatedStopLoss, currentMarketRegime, 
                                              CalculateVolatility(VolatilityPeriod), CalculatedStopLoss);
                            }
                            ...
                            PrintFormat("OnTick: trade.BuyStop failed. Broker Error: %d, MQL5 Code: %d. Symbol: %s, Lot: %.2f, Price: %.5f, SL: %.5f", 
                                        GetLastError(), trade.ResultRetcode(), _Symbol, lotSize, entryPrice, stopLoss);
                            if(CheckPointer(errorHandler)) errorHandler.LogError(trade.ResultRetcode(), "OnTick trade.BuyStop failed. Lot: "+DoubleToString(lotSize,2)+", Price: "+DoubleToString(entryPrice,_Digits));
                        }
                    }
                }
            }
   ...
   if(needsSellOrder && (CurrentTime - LastSellOrderTime) > MinOrderInterval_EA && workingEnableBaselineEntry) { // Added workingEnableBaselineEntry condition
      double entryPrice = Bid - AdjustedOrderDistance;
      double stopLoss = entryPrice + CalculatedStopLoss;
      
      if(entryPrice < Bid - MinStopDistance && stopLoss > 0 && CalculatedStopLoss > 0) { // Ensure CalculatedStopLoss is positive
         double lotSize = CalculateOptimalLotSize(CalculatedStopLoss);
         ...
         if(lotSize >= MinLotSize && CheckPointer(riskManager) == POINTER_DYNAMIC) {
            ...
            if(riskManager.IsTradeAllowed(lotSize, stopLossPips)) {
                double marginRequired = 0;
                if(!OrderCalcMargin(ORDER_TYPE_SELL_STOP, _Symbol, lotSize, entryPrice, marginRequired)) {
                    PrintFormat("OnTick: OrderCalcMargin failed for SELL_STOP. Lot: %.2f, Price: %.5f. Error: %d", lotSize, entryPrice, GetLastError());
                } else {
                    double freeMargin = AccountInfoDouble(ACCOUNT_MARGIN_FREE);
                    if (marginRequired > freeMargin) {
                        PrintFormat("OnTick: Insufficient free margin for SELL_STOP. Required: %.2f, Free: %.2f. Lot: %.2f. Skipping trade.", marginRequired, freeMargin, lotSize);
                    } else {
                        double slToSend = workingDisableInitialStopLoss ? 0 : stopLoss;
                        if(trade.SellStop(lotSize, entryPrice, _Symbol, slToSend, 0, orderTimeType, expirationTimestamp, "HFT_SELL")) {
                            riskManager.RegisterTradeRisk(lotSize, stopLossPips); // Only register risk if order successfully placed
                            ...
                            PrintFormat("OnTick: trade.SellStop failed. Broker Error: %d, MQL5 Code: %d. Symbol: %s, Lot: %.2f, Price: %.5f, SL: %.5f", 
                                        GetLastError(), trade.ResultRetcode(), _Symbol, lotSize, entryPrice, stopLoss);
                            if(CheckPointer(errorHandler)) errorHandler.LogError(trade.ResultRetcode(), "OnTick trade.SellStop failed. Lot: "+DoubleToString(lotSize,2)+", Price: "+DoubleToString(entryPrice,_Digits));
                        }
                    }
                }
            }
