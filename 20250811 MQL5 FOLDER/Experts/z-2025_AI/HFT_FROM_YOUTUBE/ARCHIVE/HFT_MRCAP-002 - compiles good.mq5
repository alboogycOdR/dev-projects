...
void OnTick()
  {
   ulong ticket = 0; // Add this declaration
   if(CheckPointer(perfMonitor) == POINTER_DYNAMIC) {
      perfMonitor.StartTickMeasurement();
   }

   if(!IsSignificantTick()) { // Filter ticks
      if(CheckPointer(perfMonitor) == POINTER_DYNAMIC) {
         perfMonitor.EndTickMeasurement(); // End measurement even if tick is insignificant
      }
      return;
   }
   ...
   // Check with RiskManager BEFORE attempting to open trade
   bool canTrade = true;
   if(CheckPointer(riskManager) == POINTER_DYNAMIC) {
      double slPips = CalculatedStopLoss / riskManager.PointScaleFactor();
