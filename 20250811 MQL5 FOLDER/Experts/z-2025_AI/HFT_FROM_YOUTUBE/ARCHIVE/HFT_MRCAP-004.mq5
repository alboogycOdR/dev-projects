...
void UpdateMarketAndTradeParameters()
{
   ...
   double pipSize = (_Digits == 3 || _Digits == 5) ? _Point * 10 : _Point;
   double maxReasonableStopLossPriceUnits = 100 * pipSize; // 100 traditional pips in price units
   if(CalculatedStopLoss > maxReasonableStopLossPriceUnits) {
      PrintFormat("WARNING (UpdateMarketParams): CalculatedStopLoss (%.5f) exceeds reasonable limit (%.5f = 100 pips). AverageSpread=%.5f, workingStop=%.1f. Using fallback.", 
                  CalculatedStopLoss, maxReasonableStopLossPriceUnits, AverageSpread, workingStop);
      CalculatedStopLoss = MathMax(workingStop * pipSize, MinStopDistance); // Fallback: workingStop (e.g. 5) * pipSize
   }
   
   // Debug logging for first few calculations
   static int debugCount = 0;
   if(debugCount < 3) {
      PrintFormat("UpdateMarketParams DEBUG: AverageSpread=%.5f, workingStop=%.1f, CalculatedStopLoss=%.5f, MinStopDistance=%.5f", 
                  AverageSpread, workingStop, CalculatedStopLoss, MinStopDistance);
      debugCount++;
   }
}
...
void OnTick()
{
   ...
   double maxReasonableStopLossPriceUnitsOnTick = 100 * pipSizeForOnTick; // 100 traditional pips
   if(CalculatedStopLoss > maxReasonableStopLossPriceUnitsOnTick) {
      PrintFormat("WARNING (OnTick): CalculatedStopLoss (%.5f) exceeds reasonable limit (%.5f = 100 pips). Using fallback.", 
                  CalculatedStopLoss, maxReasonableStopLossPriceUnitsOnTick);
      CalculatedStopLoss = MathMax(workingStop * pipSizeForOnTick, MinStopDistance); // Fallback: workingStop (e.g. 5) * pipSize
   }
   ...
   if(needsBuyOrder && (CurrentTime - LastBuyOrderTime) > MinOrderInterval) {
      ...
      if(entryPrice > Ask + MinStopDistance && stopLoss > 0) {
         ...
         if(lotSize >= MinLotSize && CheckPointer(riskManager) == POINTER_DYNAMIC) {
            ...
            // Debug logging for first few calculations
            static int debugCountOnTick = 0;
            if(debugCountOnTick < 3) {
               PrintFormat("OnTick DEBUG: CalculatedStopLoss=%.5f, pipSize=%.5f, stopLossPips=%.1f, AverageSpread=%.5f, workingStop=%.1f", 
                           CalculatedStopLoss, pipSize, stopLossPips, AverageSpread, workingStop);
               debugCountOnTick++;
            }
