...
void OnTick()
{
   ...
   if(CalculatedStopLoss > maxReasonableStopLossPriceUnitsOnTick && maxReasonableStopLossPriceUnitsOnTick > 0) { // ensure limit is positive
      PrintFormat("WARNING (OnTick): CalculatedStopLoss (%.5f) exceeds reasonable limit (%.5f). MinStopDist=%.5f. Using fallback.", 
                  CalculatedStopLoss, maxReasonableStopLossPriceUnitsOnTick, MinStopDistance);
      CalculatedStopLoss = MathMax(workingStop * pipSizeForOnTick, MinStopDistance > 0 ? MinStopDistance : (workingStop * pipSizeForOnTick)); 
   }
            ...
            if(riskManager.IsTradeAllowed(lotSize, stopLossPips)) { // This also updates internal risk calculation for the check
                ...
                if(!OrderCalcMargin(ORDER_TYPE_BUY_STOP, _Symbol, lotSize, entryPrice, marginRequired)) {
                    ...
                    if (marginRequired > freeMargin) {
                        ...
                        if(trade.BuyStop(lotSize, entryPrice, _Symbol, slToSend, 0, 0, "HFT_BUY")) {
            ...
            if(riskManager.IsTradeAllowed(lotSize, stopLossPips)) {
                ...
                if(!OrderCalcMargin(ORDER_TYPE_SELL_STOP, _Symbol, lotSize, entryPrice, marginRequired)) {
                    ...
                    if (marginRequired > freeMargin) {
                        ...
                        double slToSend = workingDisableInitialStopLoss ? 0 : stopLoss;
                        if(trade.SellStop(lotSize, entryPrice, _Symbol, slToSend, 0, 0, "HFT_SELL")) {
                            riskManager.RegisterTradeRisk(lotSize, stopLossPips); // Only register risk if order successfully placed
