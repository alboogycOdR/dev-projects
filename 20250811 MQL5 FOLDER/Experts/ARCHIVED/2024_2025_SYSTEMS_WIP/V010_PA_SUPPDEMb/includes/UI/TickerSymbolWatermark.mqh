...
TickerSymbolWatermark::TickerSymbolWatermark(color _fontColor, int _fontSize) {
  ...
  string strSymbolSubtitle = AccountInfoString(ACCOUNT_COMPANY);// SymbolInfoString(_Symbol, SYMBOL_DESCRIPTION);
...
void TickerSymbolWatermark::resizeLabel(string objName, float ratio) {
  ObjectSetInteger(0, objName, OBJPROP_FONTSIZE, (long)(this.resizedFont * ratio));
};
