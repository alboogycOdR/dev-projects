   ...
   for (int i = OrdersTotal () - 1; i >= 0; i --)
   {
      if (OrderSelect (i, SELECT_BY_POS) && OrderSymbol () == Symbol () && OrderMagicNumber () >= initial_magic && OrderMagicNumber () <= initial_magic + 100)
      {
         switch (OrderType ())
         {
            ...
            case OP_SELLSTOP:
            /*
               if (OrderSelect (StrToDouble (OrderComment ()), SELECT_BY_TICKET) && OrderCloseTime () > 0)  // PendingRemove rejects calls with OrderCloseTime > 0
               {
                  PendingRemove (OrderTicket()) ;                                                          // PendingRemove rejects calls with OrderCloseTime > 0
               }
            */
               break ;
         }
		     ...
		     while(true)
		       {
			        ...
			        if(GetTickCount() - StartWaitingTime > sec_wait_context * 1000)
			          {
				           Print("Waiting limit exceeded (" + IntegerToString(sec_wait_context) + " sec.)!");
				           contextReturn = (-2);
				           break;
			          }
...
int PendingSend (int type, double lots, double price, double stop, double take, int magic_number, string comm)
{
   ...
   while (attempt <= command_attempts && tick < 0)
   {
      ...
      color orderColor = clrNONE;
      switch (type)
      {
         case OP_BUYLIMIT: str = StringConcatenate (str, "Buy Limit") ; orderColor = clrBlueViolet; break ;
         case OP_SELLLIMIT: str = StringConcatenate (str, "Sell Limit") ; orderColor = clrOrangeRed; break ;
         case OP_BUYSTOP: str = StringConcatenate (str, "Buy Stop") ; orderColor = clrAliceBlue; break ;
         case OP_SELLSTOP: str = StringConcatenate (str, "Sell Stop"); orderColor = clrOrange; break;;
      }
      ...
      tick = OrderSend(Symbol(), type,  lots, price,       0,  use_sl,    use_tp,        comm, magic_number,           0,         orderColor) ;
...
int MarketSend (int dir, double lots, double stop, double take, int magic_number)
{
   ...
   while (attempt <= command_attempts && tick < 0)
   {
      ...
      uint ms = GetTickCount () ;
      //int  OrderSend(symbol,    cmd,volume, price,      slippage,stoploss,takeprofit,comment=NULL,      magic=0,expiration=0,arrow_color=clrNONE)
      tick = OrderSend(Symbol(),  dir,  lots, price, (int) use_slp,  use_sl,    use_tp,          "", magic_number,           0,         orderColor) ;
...
bool ProfitReached ()
{
   ...
   while ( (pos >= 0) && (profitReached == true) )
   {
   ...
   {
      profitReached = true;
   } else if (daily_profit_percent > 0 && sum / AccountBalance () * 100 >= daily_profit_percent)
   {
      profitReached = true;
   } else
   {
...
bool HedgeIsSet (int magic)
{
   ...
   if ( !OrderSelect(tick, SELECT_BY_TICKET) )
   {
      Print("OrderSelect failed for: ",IntegerToString(tick),", in HedgeIsSet.");
   }
