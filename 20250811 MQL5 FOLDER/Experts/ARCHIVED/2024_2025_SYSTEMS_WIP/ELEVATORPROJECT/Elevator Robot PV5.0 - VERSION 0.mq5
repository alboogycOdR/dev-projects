...
void OnTick()
{
   ...
   string zigzagsignal=cntrol();
   string ma_signal=CheckMA();
   ...
   //executed ontick & updated
   total_profit = 
      NormalizeDouble(AccountInfoDouble(ACCOUNT_BALANCE)-curr_ball,2);
...
{
   double arr[];double candle[2];int j=0;
   int zig = iCustom(_Symbol,_Period,"Examples/ZigZagColor.ex5"
                        ,Depth,Deviation,Steps);
...
void drawLabel()
{
   ...
   make_detail("bal",    "BALANCE----------------> "
      +(string)NormalizeDouble(AccountInfoDouble(ACCOUNT_BALANCE),2),165,clrWhite);
   
   make_detail("opp",    "OPEN POSITIONS----------> "
      +(string)PositionsTotal(),145,clrWhite);
   
   make_detail("prof",    "ACCOUNT PROFIT----------> "
      +(string)NormalizeDouble(AccountInfoDouble(ACCOUNT_PROFIT),2),125,clrWhite);
