...
input ushort   InpTrailingStop   = 2000;        // Trailing Stop (in pips)
...
input double          fixed_lots=0.165;
input int             hl_multiplier= 3;
input int             magic=99923;
...
*/
   if(iGetArray(handle_iCustom_TDI,4,0,2,buffer4)){ tdi_red=buffer4[0];
   tdi_red_minus_one=buffer4[1];
   }
/*  
   RSI Price line    GREEN
*/
   if(iGetArray(handle_iCustom_TDI,3,0,1,buffer3)){tdi_green_s2=buffer3[0];
   }
...
//Comment(commentstring);
   ...
   int digits=SymbolInfoInteger(Symbol(),SYMBOL_DIGITS); RefreshRates();
      ...
      if(
         ((entry_method==method1 && sto_one[0]<80) || (entry_method==method2 && sto_one[0]<50)) 
       && 
                  ...
                  if(m_position.StopLoss()<m_position.PriceCurrent()-(ExtTrailingStop+ExtTrailingStep))
                    {
                     if(!m_trade.PositionModify(m_position.Ticket()
                        ...
                        Print("Modify BUY ",m_position.Ticket(), " Position -> false. Result Retcode: ",m_trade.ResultRetcode(), ", description of result: ",m_trade.ResultRetcodeDescription());                    PrintResultModify(m_trade,m_symbol,m_position); continue;
                    }
              }
            else//SELL POSITION
              {
