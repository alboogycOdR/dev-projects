...
ENUM_TIMEFRAMES input TRADING_TIMEFRAME = PERIOD_CURRENT;
...
double HighestHigh(int period, int shift, ENUM_TIMEFRAMES timeframe)
  {
  string ObjectPrefix="H_";
  ObjectsDeleteAll(0, ObjectPrefix);
  
   string name = "H_" + (string)MathRand();
   double hh=iHigh(_Symbol, timeframe, iHighest(_Symbol, timeframe, MODE_HIGH, period, shift));
   DrawLine(name,hh);
...
double LowestLow(int period, int shift, ENUM_TIMEFRAMES timeframe)
  {
  string ObjectPrefix="L_";
  ObjectsDeleteAll(0, ObjectPrefix);
  
   string name = "L_" + (string)MathRand();
   double ll=iLow(_Symbol, timeframe, iLowest(_Symbol, timeframe, MODE_LOW, period, shift));
   DrawLine(name,ll);
               ...
               if(cp < trail_long)
                 {
                  ...
                  if(trail_long > sl + 0.1 * atr && cp - trail_long >= SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL))
                    {
                  ...
                  if(cp > trail_short)
                    {
                     ...
                     if(trail_short < sl - 0.1 * atr && trail_short - cp >= SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL))
                       {
   ...
   if(BarOpen())
     {
      ...
      // Long order
      Print("trigger_long :"+trigger_long);
      Print("ask :"+ask);

      Print("trigger_long - ask  :"+ (trigger_long - ask));

      Print("SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL :"+SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL));

      Print("trigger_long - ask >= SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL)  :"+(trigger_long - ask >= SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL)));
