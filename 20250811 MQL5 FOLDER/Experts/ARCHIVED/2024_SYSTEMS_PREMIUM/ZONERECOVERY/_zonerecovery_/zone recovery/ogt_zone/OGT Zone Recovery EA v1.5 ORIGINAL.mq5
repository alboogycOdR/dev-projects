...
input group "RecoverySettings"
   input long __RecoveryZoneSize__ = 200; // Recovery Zone Size (points)
   long RecoveryZoneSize = __RecoveryZoneSize__;
...
input group "TP SL TRADECOUNT"
   input long __TakeProfit__ = 200; // Take Profit (points)
   long TakeProfit = __TakeProfit__;
...
input group "ATR Dynamic Zone Sizing"
...
input group "MONEY MANAGEMENT SETTINGS"
   ...
   input double InitialLotSize = 1; // Initial Lot Size (if % not used)
...
input group "Visuals"
   ...
   input int Panel_Y = 120; // Panel Y coordinate.
...
void OnInit()
  {
   ...
   MagicNumber = makeMagicNumber(MQLInfoString(MQL_PROGRAM_NAME)
                                 + Symbol()
                                 + (string)Period());
   Comment("automagic: "+(string)MagicNumber);
      ...
      if(gCurrentDirection == 1 && netLotSize != 0)
        {
         ...
         if(!CCBC::ObjectSetDouble(0, PROFIT_LINE, OBJPROP_PRICE, profitPrice))
               ...
               if(MaxTrades == 0 || positions < MaxTrades) // check we've not exceeded the max trades
                 {
                  ...
                  gBuyStopTicket = OpenPendingOrder(Symbol(), OP_BUYSTOP, gLotSize, gBuyOpenPrice, 0, 0,
                                                    CCBC::StringConcatenate("Recovery Buy Stop opened.","."), 0, clrTurquoise); // create an opposite buy stop
               ...
               if(MaxTrades == 0 || positions < MaxTrades) // check we've not exceeded the max trades
                 {
                  ...
                  gSellStopTicket = OpenPendingOrder(Symbol(), OP_SELLSTOP, gLotSize, gSellOpenPrice, 0, 0,
                                                     CCBC::StringConcatenate("Recovery Sell Stop opened.","."), 0, clrPink); // create an opposite sell stop
...
// create the button to start the trade off
   ...
   CCBC::ObjectSetString(0, PENDING_EDIT, OBJPROP_TEXT, DoubleToString(pending));
...
//|                                                                  |
//+------------------------------------------------------------------+
long StringToPips(string text)
  {
   long pips = StringToInteger(text);
   if(pips <= 0)
     {
         ...
         if(CCBC::OrderMagicNumber() == MagicNumber && CCBC::OrderSymbol() == _Symbol && (CCBC::OrderType() == OP_BUY || CCBC::OrderType() == OP_SELL))
           {
            ...
            while(CCBC::IsTradeContextBusy())
               Sleep(10);

            CCBC::RefreshRates();
            double ClosePrice = CCBC::MarketInfo(_Symbol,MODE_BID);
            if(CCBC::OrderType() == OP_SELL)
               ClosePrice = CCBC::MarketInfo(_Symbol, MODE_ASK);
   ...
   for(int order = 0; order <= CCBC::OrdersTotal() - 1; order++)
     {
      ...
      long orderMagicNumber = CCBC::OrderMagicNumber();
   ...
   while(retryCount <= max_attempts)
     {
      ...
      if(pType == OP_BUY)
         orderPrice = CCBC::MarketInfo(pSymbol,MODE_ASK);
      else
         if(pType == OP_SELL)
            orderPrice = CCBC::MarketInfo(pSymbol,MODE_BID);
...
double GetTradeSize(string pSymbol, double pFixedVol, double pPercent, double pStopPoints)
  {
   double tradeSize;
         ...
         if(CCBC::OrderMagicNumber() == MagicNumber && CCBC::OrderSymbol() == _Symbol && CCBC::OrderType() == OP_BUYSTOP)
           {
            ...
            if(CCBC::OrderMagicNumber() == MagicNumber && CCBC::OrderSymbol() == _Symbol && CCBC::OrderType() == OP_SELLSTOP)
              {
      ...
      switch(errorCode)
        {
         ...
         case ERR_STRING_TOO_BIGNUMBER          :
            return 4011; /* ERR_TOO_LONG_STRING */
         ...
         case ERR_ZEROSIZE_ARRAY                :
            return 4016; /* ERR_NOT_INITIALIZED_ARRAY */
         ...
         case ERR_INTERNAL_ERROR                :
            return 4024; /* ERR_INTERNAL_ERROR */
         case ERR_NOT_ENOUGH_MEMORY             :
            return 4025; /* ERR_OUT_OF_MEMORY */
         case ERR_INVALID_POINTER               :
            return 4026; /* ERR_INVALID_POINTER */
         case ERR_TOO_MANY_FORMATTERS           :
            return 4027; /* ERR_FORMAT_TOO_MANY_FORMATTERS */
         case ERR_TOO_MANY_PARAMETERS           :
            return 4028; /* ERR_FORMAT_TOO_MANY_PARAMETERS */
         case ERR_INVALID_ARRAY                 :
            return 4029; /* ERR_ARRAY_INVALID */
         case ERR_CHART_NO_REPLY                :
            return 4030; /* ERR_CHART_NOREPLY */
         ...
         case ERR_RESOURCE_NOT_FOUND            :
            return 4068; /* ERR_RESOURCE_NOT_FOUND */
         case ERR_RESOURCE_UNSUPPOTED_TYPE      :
            return 4069; /* ERR_RESOURCE_NOT_SUPPORTED */
         case ERR_RESOURCE_NAME_DUPLICATED      :
            return 4070; /* ERR_RESOURCE_DUPLICATED */
         case ERR_INDICATOR_CANNOT_CREATE       :
            return 4071; /* ERR_INDICATOR_CANNOT_INIT */
         case ERR_INDICATOR_CANNOT_ADD          :
         case ERR_CHART_INDICATOR_CANNOT_ADD    :
            return 4072; /* ERR_INDICATOR_CANNOT_LOAD */
         case ERR_HISTORY_NOT_FOUND             :
            return 4073; /* ERR_NO_HISTORY_DATA */
         case ERR_HISTORY_LOAD_ERRORS           :
            return 4074; /* ERR_NO_MEMORY_FOR_HISTORY */
         case ERR_BUFFERS_NO_MEMORY             :
            return 4075; /* ERR_NO_MEMORY_FOR_INDICATOR */
         case ERR_FILE_ENDOFFILE                :
            return 4099; /* ERR_END_OF_FILE */
         ...
         case ERR_TRADE_DEAL_NOT_FOUND          :
            return 4105; /* ERR_NO_ORDER_SELECTED */
         case ERR_MARKET_UNKNOWN_SYMBOL         :
         case ERR_INDICATOR_UNKNOWN_SYMBOL      :
            return 4106; /* ERR_UNKNOWN_SYMBOL */
         ...
         case TRADE_RETCODE_SHORT_ONLY          :
            return 4110; /* ERR_LONGS_NOT_ALLOWED */
         case TRADE_RETCODE_LONG_ONLY           :
            return 4111; /* ERR_SHORTS_NOT_ALLOWED */
         case TRADE_RETCODE_SERVER_DISABLES_AT  :
            return 4112; /* ERR_TRADE_EXPERT_DISABLED_BY_SERVER */
         ...
         case ERR_OBJECT_WRONG_PROPERTY         :
            return 4201; /* ERR_UNKNOWN_OBJECT_PROPERTY */
         case ERR_OBJECT_NOT_FOUND              :
            return 4202; /* ERR_OBJECT_DOES_NOT_EXIST */
         ...
         case ERR_OBJECT_ERROR                  :
            return 4207; /* ERR_SOME_OBJECT_ERROR */
         case ERR_CHART_WRONG_PROPERTY          :
            return 4210; /* ERR_CHART_PROP_INVALID */
         case ERR_CHART_NOT_FOUND               :
            return 4211; /* ERR_CHART_NOT_FOUND */
         case ERR_CHART_WINDOW_NOT_FOUND        :
            return 4212; /* ERR_CHARTWINDOW_NOT_FOUND */
         case ERR_CHART_INDICATOR_NOT_FOUND     :
            return 4213; /* ERR_CHARTINDICATOR_NOT_FOUND */
         case ERR_MARKET_NOT_SELECTED           :
            return 4220; /* ERR_SYMBOL_SELECT */
         case ERR_NOTIFICATION_SEND_FAILED      :
            return 4250; /* ERR_NOTIFICATION_ERROR */
         case ERR_NOTIFICATION_WRONG_PARAMETER  :
            return 4251; /* ERR_NOTIFICATION_PARAMETER */
         case ERR_NOTIFICATION_WRONG_SETTINGS   :
            return 4252; /* ERR_NOTIFICATION_SETTINGS */
         case ERR_NOTIFICATION_TOO_FREQUENT     :
            return 4253; /* ERR_NOTIFICATION_TOO_FREQUENT */
         case ERR_FTP_NOSERVER                  :
            return 4260; /* ERR_FTP_NOSERVER */
         case ERR_FTP_NOLOGIN                   :
            return 4261; /* ERR_FTP_NOLOGIN */
         case ERR_FTP_CONNECT_FAILED            :
            return 4262; /* ERR_FTP_CONNECT_FAILED  */
         // ERR_FTP_CLOSED is listed in MQL5's documentation as a value, but it's not defined as a constant
         case 4524                              :
            return 4263; /* ERR_FTP_CLOSED */
         case ERR_FTP_CHANGEDIR                 :
            return 4264; /* ERR_FTP_CHANGEDIR */
         case ERR_FTP_FILE_ERROR                :
            return 4265; /* ERR_FTP_FILE_ERROR */
         case ERR_FTP_SEND_FAILED               :
            return 4266; /* ERR_FTP_ERROR */
         case ERR_TOO_MANY_FILES                :
            return 5001; /* ERR_FILE_TOO_MANY_OPENED */
         case ERR_WRONG_FILENAME                :
            return 5002; /* ERR_FILE_WRONG_FILENAME */
         case ERR_TOO_LONG_FILENAME             :
            return 5003; /* ERR_FILE_TOO_LONG_FILENAME */
         case ERR_CANNOT_OPEN_FILE              :
            return 5004; /* ERR_FILE_CANNOT_OPEN */
         case ERR_FILE_CACHEBUFFER_ERROR        :
            return 5005; /* ERR_FILE_BUFFER_ALLOCATION_ERROR */
         case ERR_CANNOT_DELETE_FILE            :
            return 5006; /* ERR_FILE_CANNOT_DELETE */
         case ERR_INVALID_FILEHANDLE            :
            return 5007; /* ERR_FILE_INVALID_HANDLE */
         case ERR_WRONG_FILEHANDLE              :
            return 5008; /* ERR_FILE_WRONG_HANDLE */
         case ERR_FILE_NOTTOWRITE               :
            return 5009; /* ERR_FILE_NOT_TOWRITE */
         case ERR_FILE_NOTTOREAD                :
            return 5010; /* ERR_FILE_NOT_TOREAD */
         case ERR_FILE_NOTBIN                   :
            return 5011; /* ERR_FILE_NOT_BIN */
         case ERR_FILE_NOTTXT                   :
            return 5012; /* ERR_FILE_NOT_TXT */
         case ERR_FILE_NOTTXTORCSV              :
            return 5013; /* ERR_FILE_NOT_TXTORCSV */
         case ERR_FILE_NOTCSV                   :
            return 5014; /* ERR_FILE_NOT_CSV */
         case ERR_FILE_READERROR                :
            return 5015; /* ERR_FILE_READ_ERROR */
         case ERR_FILE_WRITEERROR               :
            return 5016; /* ERR_FILE_WRITE_ERROR */
         case ERR_FILE_BINSTRINGSIZE            :
            return 5017; /* ERR_FILE_BIN_STRINGSIZE */
         case ERR_INCOMPATIBLE_FILE             :
            return 5018; /* ERR_FILE_INCOMPATIBLE */
         case ERR_FILE_IS_DIRECTORY             :
            return 5019; /* ERR_FILE_IS_DIRECTORY */
         case ERR_FILE_NOT_EXIST                :
            return 5020; /* ERR_FILE_NOT_EXIST */
         case ERR_FILE_CANNOT_REWRITE           :
            return 5021; /* ERR_FILE_CANNOT_REWRITE */
         case ERR_WRONG_DIRECTORYNAME           :
            return 5022; /* ERR_FILE_WRONG_DIRECTORYNAME */
         case ERR_DIRECTORY_NOT_EXIST           :
            return 5023; /* ERR_FILE_DIRECTORY_NOT_EXIST */
         case ERR_FILE_ISNOT_DIRECTORY          :
            return 5024; /* ERR_FILE_NOT_DIRECTORY */
         case ERR_CANNOT_DELETE_DIRECTORY       :
            return 5025; /* ERR_FILE_CANNOT_DELETE_DIRECTORY */
         case ERR_CANNOT_CLEAN_DIRECTORY        :
            return 5026; /* ERR_FILE_CANNOT_CLEAN_DIRECTORY */
         case ERR_ARRAY_RESIZE_ERROR            :
            return 5027; /* ERR_FILE_ARRAYRESIZE_ERROR */
         case ERR_STRING_RESIZE_ERROR           :
            return 5028; /* ERR_FILE_STRINGRESIZE_ERROR */
         case ERR_STRUCT_WITHOBJECTS_ORCLASS    :
            return 5029; /* ERR_FILE_STRUCT_WITH_OBJECTS */
         case ERR_WEBREQUEST_INVALID_ADDRESS    :
            return 5200; /* ERR_WEBREQUEST_INVALID_ADDRESS */
         case ERR_WEBREQUEST_CONNECT_FAILED     :
            return 5201; /* ERR_WEBREQUEST_CONNECT_FAILED */
         case ERR_WEBREQUEST_TIMEOUT            :
            return 5202; /* ERR_WEBREQUEST_TIMEOUT */
         case ERR_WEBREQUEST_REQUEST_FAILED     :
            return 5203; /* ERR_WEBREQUEST_REQUEST_FAILED */
         case ERR_USER_ERROR_FIRST              :
            return 65536; /* ERR_USER_ERROR_FIRST */
      ...
      while(true)
        {
         ...
         if(CCBC_TRADES::IsFillingTypeAllowed(symbol, SYMBOL_FILLING_FOK))
            ...
            if(CCBC_TRADES::IsFillingTypeAllowed(symbol, SYMBOL_FILLING_IOC))
               request.type_filling = ORDER_FILLING_IOC;
            else
               if(CCBC_TRADES::IsFillingTypeAllowed(symbol, ORDER_FILLING_RETURN))  // just in case
   ...
   static double     OrderLots()
     {
      ...
      if(CCBC_TRADES::LoadedType() == 4)
         return ::HistoryOrderGetDouble(CCBC::OrderTicket(), ORDER_VOLUME_CURRENT);
   ...
   static long       OrderMagicNumber()
     {
      ...
      if(CCBC_TRADES::LoadedType() == 4)
         return (long)::HistoryOrderGetInteger(CCBC::OrderTicket(), ORDER_MAGIC);
   ...
   static double     OrderOpenPrice()
     {
      ...
      if(CCBC_TRADES::LoadedType() == 4)
         return ::HistoryOrderGetDouble(CCBC::OrderTicket(), ORDER_PRICE_OPEN);
         ...
         if(isPendingOrder)
           {
            if(CCBC_TRADES::IsFillingTypeAllowed(symbol, ORDER_FILLING_RETURN))
               ...
               if(CCBC_TRADES::IsFillingTypeAllowed(symbol, ORDER_FILLING_FOK))
                  request.type_filling = ORDER_FILLING_FOK;
               else
                  if(CCBC_TRADES::IsFillingTypeAllowed(symbol, ORDER_FILLING_IOC))
                     request.type_filling = ORDER_FILLING_IOC;
           }
         else
           {
   ...
   static string     OrderSymbol()
     {
      ...
      if(CCBC_TRADES::LoadedType() == 4)
         return ::HistoryOrderGetString(CCBC::OrderTicket(), ORDER_SYMBOL);
   ...
   static int        OrderType()
     {
      ...
      if(CCBC_TRADES::LoadedType() == 3)
         return (int)::HistoryDealGetInteger(CCBC::OrderTicket(), DEAL_TYPE);
      if(CCBC_TRADES::LoadedType() == 4)
         return (int)::HistoryOrderGetInteger(CCBC::OrderTicket(), ORDER_TYPE);
