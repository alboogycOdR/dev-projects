...
/*
      ...
      The EA determines the trend direction based on the two latest Highs and two Lows (presence of a higher High and a higher Low — uptrend; 

      accordingly, a lower High and a lower Low — downtrend; Highs and Lows changed in different directions —no trend). 

      The Fibonacci grid is based only on the trend.
      ...
      Take profit at FIBO extension — 
      take profit levels as a Fibonacci extension 
      (for example, value 0.38 means an expansion of 138%);

      Next bar close distance from level (points) — the distance from the level along the trend, 
      at which the EA will open a trade;
      ...
      Pause from close to trade (bars) 
      — the number of bars that must be elapsed after closure of the previous trade for the EA 
      to open a new trade;
...
input double   TakeProfitAt = 0.618;     //Take profit at FIBO extension
               ...
               case  1:
                 ...
                 if(((RatesArray[0].close - Fibo76) > Point() * SafetyBuffer && (Fibo76 - RatesArray[1].close) > Point() * SafetyBuffer) ||
                    ...
                    ((RatesArray[0].close - Fibo23) > Point() * SafetyBuffer && (Fibo23 - RatesArray[1].close) > Point() * SafetyBuffer))
                  {
                      ...
                      if(!TradeCheck(1)) 
                        Print("Wait another bar");
                  }
                 break;
               //DOWNTREND - CHECK FOR SELLS
               case -1:
      ...
      switch(deal)
           {
            case 1: 
               if(trade.Buy( 
               TradeValue,NULL, SymbolInfoDouble(Symbol(),SYMBOL_ASK) - Point() /*0*/, 
               SymbolInfoDouble(Symbol(),SYMBOL_BID) - Point() * StopLossLevel, 
               Fibo00 + TakeProfitAt * FiboBASE)
               )
                  {
                  //Print("Method Buy is completed at Fibo00="+Fibo00);
                  //Print("Method Buy is completed at TakeProfitAt="+TakeProfitAt);
                  //Print("Method Buy is completed at Fibo00 + TakeProfitAt * FiboBASE="+(Fibo00 + TakeProfitAt * FiboBASE));
                  return(true);
                  }
               break;
            case 2:
               if(trade.Sell( TradeValue, 
               NULL, SymbolInfoDouble(Symbol(),SYMBOL_BID) - Point() /*0*/, 
               SymbolInfoDouble(Symbol(), SYMBOL_ASK) + Point() * StopLossLevel, 
               Fibo00 - TakeProfitAt * FiboBASE))
                  {
...
//| CheckTrend function                                              |
//+------------------------------------------------------------------+
int CheckTrend(double hl0,double hl1,double hl2,double hl3)
   {
   int check_trend = 0;
...
bool FiboCreate(datetime time1, double array1, datetime time0, double array0)
{
     ...
     if(ObjectCreate( 0, "MyFIBO", OBJ_FIBO, 0, time1, array1, time0, array0)) ///----- FIBO retracement creation based on last ZZ leg
      ...
      {
        Print("MyFIBO creation is failed"); return (false);
      }
  }
//+------------------------------------------------------------------+
//| Trade function                                                   |
