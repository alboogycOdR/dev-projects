...
static double GoldRatioFibAlpha;

static double GoldRatioFibBeta;
static double GoldRatioFibAlphaInverse;
static double GoldRatioFibBetaInverse,GoldRatioFib236SELL,
       GoldRatioFib236BUY;
...
int OnInit()
  {
   ...
   CopyHigh(_Symbol,_Period,start,end,High);//   [ 0 to 10 candles ]
   ...
   ObjectSetInteger(0,"Highest",OBJPROP_COLOR,clrGreenYellow);
...
//=========
   ...
   ObjectSetInteger(0,"Lowest",OBJPROP_COLOR,clrGreenYellow);
   ObjectSetInteger(0,"Lowest",OBJPROP_WIDTH,3);
//=========
//=========
//=========
   ...
   ObjectSetInteger(0,"Middle",OBJPROP_WIDTH,3);
      ...
      if(direction =="BUY")
        {
         ...
         //SELL AT 50% FIBONACCI
         bool orderSell = trade.SellLimit(Volume
                                          ,PRICE
                                          ,Symbol()
                                          ,SL
                                          ,TP
...
//         bool orderSell2 = trade.SellLimit(Volume
//                                           ,GoldRatioFibAlphaInverse
//                                           ,Symbol()
//                                           ,SL
//                                           ,TP
//                                           ,0,0
//                                           ,"s61");
//         //SELL AT 50% FIBONACCI
         bool orderSell3 = trade.SellLimit(Volume
                                           ,GoldRatioFibBetaInverse
                                           ,Symbol()
                                           ,SL
                                           ,TP
                                           ,0,0
                                           ,"s38");
         bool orderSell4 = trade.SellLimit(Volume
                                           ,GoldRatioFib236SELL
                                           ,Symbol()
                                           ,SL
                                           ,TP
      ...
      if(direction =="SELL")
        {
         ...
         bool orderBuy = trade.BuyLimit(Volume
                                        ,PRICE
                                        ,Symbol()
                                        ,SL
                                        ,TP
                                        ,0,0
                                        ,"b50");
         //bool orderBuy2 = trade.BuyLimit(Volume
         //                                ,GoldRatioFibAlpha
         //                                ,Symbol()
         //                                ,SL
         //                                ,TP
         //                                ,0,0
         //                                ,"b61");
         bool orderBuy3 = trade.BuyLimit(Volume
                                         ,GoldRatioFibBeta
                                         ,Symbol()
                                         ,SL
                                         ,TP
         ...
         bool orderBuy4 = trade.BuyLimit(Volume
                                         ,GoldRatioFib236BUY
                                         ,Symbol()
                                         ,SL
                                         ,TP
...
//
//   double stopslevels=SymbolInfoInteger(Symbol(),SYMBOL_TRADE_STOPS_LEVEL) * Point();
//
//   //double STOPLOSS=sellbid+sl;
//   if(sl>0.0)
//      if(sl-sellask<stopslevels)
//         sl=sellask+stopslevels;
//
//   //double TAKEPROFIT=sellbid-tp;
//   if(tp>0.0)
//      if(sellbid-tp<stopslevels)
//         tp=sellbid-stopslevels;
...
//============================================================
   ...
   Highest = ArrayMaximum(High,start,end);  // highest of the last 10 candles
   CopyLow(_Symbol,_Period,start,end,Low);
   Lowest = ArrayMinimum(Low,start,end);
//===========================================================
   double Mid = iHigh(_Symbol,_Period,Highest)-(0.5*(iHigh(_Symbol,_Period,Highest)-iLow(_Symbol,_Period,Lowest)));
...
//BUY
   GoldRatioFibAlpha= iHigh(_Symbol,_Period,Highest)-(0.618*(iHigh(_Symbol,_Period,Highest)-iLow(_Symbol,_Period,Lowest)));
   GoldRatioFibBeta= iHigh(_Symbol,_Period,Highest)-(0.382*(iHigh(_Symbol,_Period,Highest)-iLow(_Symbol,_Period,Lowest)));
   GoldRatioFib236BUY= iHigh(_Symbol,_Period,Highest)-(0.236*(iHigh(_Symbol,_Period,Highest)-iLow(_Symbol,_Period,Lowest)));
//SELL
   GoldRatioFibAlphaInverse=iLow(_Symbol,_Period,Lowest)+(0.382*(iHigh(_Symbol,_Period,Highest)-iLow(_Symbol,_Period,Lowest)));
   GoldRatioFibBetaInverse= iLow(_Symbol,_Period,Lowest)+(0.618*(iHigh(_Symbol,_Period,Highest)-iLow(_Symbol,_Period,Lowest)));
   GoldRatioFib236SELL= iLow(_Symbol,_Period,Lowest)+(0.236*(iHigh(_Symbol,_Period,Highest)-iLow(_Symbol,_Period,Lowest)));
      ...
      for(int i = 0; i<OrdersTotal(); i++)
        {
         ...
         //Print((string)tkt+" "+OrderGetString(ORDER_COMMENT));
...
//                 {
//                  //SL = NormalizeDouble(TWEN5,_Digits); //change this to LOW
//                  trade.OrderModify(tkt,NormalizeDouble(PRICE,_Digits),LOW,TOP-poits*_Point,NULL,0,0);
//                  //Print("");
//                  //Print("modified ORDER_TYPE_BUY_LIMIT");
//
//                 }
//               if(OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_LIMIT)
//                 {
//                  //SL = NormalizeDouble(SEVEN5,_Digits);//change this to TOP
//
//                  trade.OrderModify(tkt,NormalizeDouble(PRICE,_Digits),TOP,LOW+poits*_Point,NULL,0,0);
//                  //Print("");
//                  //Print("modified ORDER_TYPE_SELL_LIMIT");
//
//                 }
//              }
//           }
...
//
//            //Print("b61 or s61");
//            if(OrderGetDouble(ORDER_PRICE_OPEN)  != NormalizeDouble(GoldRatioFibAlpha,_Digits))
//              {
//               //Print(OrderGetDouble(ORDER_PRICE_OPEN) +" openprice | b68 or s68 | "+NormalizeDouble(GoldRatioFibAlpha,_Digits));
//
//               if(OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_LIMIT)
//                 {
//                  SL = NormalizeDouble(TWEN5,_Digits);
//                  trade.OrderModify(tkt,NormalizeDouble(GoldRatioFibAlpha,_Digits),LOW-10*Point(),TOP-poits*_Point,NULL,0,0);
//                  Print("");
//                  Print("modified ORDER_TYPE_BUY_LIMIT b61");
//
//                 }
//               if(OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_LIMIT)
//                 {
//                  SL = NormalizeDouble(SEVEN5,_Digits);
//                  trade.OrderModify(tkt,NormalizeDouble(GoldRatioFibAlpha,_Digits),TOP,LOW+poits*_Point,NULL,0,0);
//                  Print("");
//                  Print("modified ORDER_TYPE_SELL_LIMIT s61");
//
//                 }
//              }
//           }
         ...
         if(commentflag=="b38" || commentflag=="s38")
           {
            ...
            if(OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_LIMIT)
              {
               if(OrderGetDouble(ORDER_PRICE_OPEN)  != NormalizeDouble(GoldRatioFibBeta,_Digits))
                 {
                  ////SL = NormalizeDouble(TWEN5,_Digits);
                  trade.OrderModify(tkt,NormalizeDouble(GoldRatioFibBeta,_Digits),LOW,TOP-poits*_Point,NULL,0,0);
                 }
              }
            if(OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_LIMIT)
              {
               if(OrderGetDouble(ORDER_PRICE_OPEN)  != NormalizeDouble(GoldRatioFibAlphaInverse,_Digits))
                 {
                  //SL = NormalizeDouble(SEVEN5,_Digits);
                  trade.OrderModify(tkt,NormalizeDouble(GoldRatioFibAlphaInverse,_Digits),TOP,LOW+poits*_Point,NULL,0,0);
                  // Print("");
                  //Print("modified ORDER_TYPE_SELL_LIMIT s61");
         ...
         if(commentflag=="b23" || commentflag=="s23")
           {
            if(OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_LIMIT)
              {
               if(OrderGetDouble(ORDER_PRICE_OPEN)  != NormalizeDouble(GoldRatioFib236BUY,_Digits))
                 {
                  //SL = NormalizeDouble(TWEN5,_Digits);
                  trade.OrderModify(tkt,NormalizeDouble(GoldRatioFib236BUY,_Digits),LOW,TOP-poits*_Point,NULL,0,0);
                 }
              }
            if(OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_LIMIT)
              {
               if(OrderGetDouble(ORDER_PRICE_OPEN)  != NormalizeDouble(GoldRatioFib236SELL,_Digits))
                 {
                  //SL = NormalizeDouble(SEVEN5,_Digits);
                  trade.OrderModify(tkt,NormalizeDouble(GoldRatioFib236SELL,_Digits),TOP,LOW+poits*_Point,NULL,0,0);
                 }
              }
           }//23 retrace
