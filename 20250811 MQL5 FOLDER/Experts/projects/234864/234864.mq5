...
ulong recently_closed_positions[20]; // Store last 20 closed position IDs
int closed_position_index = 0;       // Current index in the circular buffer
...
void PlacePendingOrder(string symbol, ENUM_ORDER_TYPE order_type, double price, double lot_size, double tp, double sl)
{
   ...
   // Calculate ATR-based distance (using period 14)
   ...
   // Use the greater of broker's minimum stop level or 1x ATR
   double min_distance = MathMax(stop_level * point, atr_value);
      ...
      if(!position_exists)   // Position is fully closed
        {
         ...
         if(WasPositionRecentlyClosed(trans.position)) {
            Print("Already processed position ID: ", trans.position);
               ...
               if(HistoryDealGetInteger(deal_ticket, DEAL_ENTRY) == DEAL_ENTRY_IN && HistoryDealGetInteger(deal_ticket, DEAL_POSITION_ID) == trans.position)
                 {
                  ...
                  //Sleep(1000); // Wait 1 second for any previous order to register in the system
...
// Add this function to check if a position was recently processed
bool WasPositionRecentlyClosed(ulong position_id) {
   for(int i = 0; i < 20; i++) {
      if(recently_closed_positions[i] == position_id) {
         return true; // Already processed this position
      }
   }
   // Not found, so add it to our tracking
   recently_closed_positions[closed_position_index] = position_id;
   closed_position_index = (closed_position_index + 1) % 20; // Circular buffer
   return false;
}
