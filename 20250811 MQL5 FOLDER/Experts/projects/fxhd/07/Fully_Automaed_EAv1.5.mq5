...
//=========================================================================
bool              EnableWallpaper=true;//Wallpaper on chart (branding)
#include          "includes\\C_WallPaper.mqh"
...
//|                                                                  |
//+------------------------------------------------------------------+
char     user01 = 30;              //transparency ( 0 to 100 )
string   user02 = "Lates_Logo";          //name of wallpaper (placed in \MQL5\Files\WallPapers)
//(placed in \MQL5\Files\WallPapers)

eType    user03 = IMAGEM;           //Tipo de fundo do grafico
...
int OnInit()
  {
   if(EnableWallpaper)
      WallPaper.Init("WallPapers\\" + user02, (char)(100 - user01));
...
//================================ Check for expiration=========================================
    if (isExpired())
    {
        Alert("EA has expired. Please contact the developer.");
        // Take appropriate action, such as deinitializing the EA
        return(INIT_FAILED);
    }

    Alert("EA is valid. Starting...");
    // Initialize your EA here
//========================== Check the current account number===================================//
    long account_number = AccountInfoInteger(ACCOUNT_LOGIN);

    // If the account number matches the allowed account number, proceed
    if (account_number == ALLOWED_ACCOUNT)
    {
        Alert("Account number is valid. EA is starting.");
        // Initialize your EA here
    }
    else
    {
        Alert("Invalid account number. EA will not run.");
        // Take appropriate action, such as deinitializing the EA
        return(INIT_FAILED);                                                  
    }
...
void OnTick()
  {
//========== Check for expiration on each tick=====================
    if (isExpired())
    {
