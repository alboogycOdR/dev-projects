...
input ENUM_TIMEFRAMES LTF = PERIOD_M30;
input color LTF_Fibo_Color = clrRed;
 ...
 bool SuperTrend_Show_Filling = true;


 input string Trade_Properties = "================ Trade Properties ================"; 
 int LookHL_XBars = 200;
 input double RetraceBreak_XLevel = 30.9; 
 double MinorBreakLimitOrderPrice_XLevel = 38.2;
 double MajorBreakLimitOrderPrice_XLevel = 66.4;
 double TakeProfit_XLevel = 161.8;
 double StopLoss_XLevel = 27.2;
 double MinorBreakLimitOrderPrice_XLevel_BOS = 30.5;
 double MajorBreakLimitOrderPrice_XLevel_BOS = 11.5;
...
input EnumPosLotType LotType = UseRiskPercentage;
...
void OnTick()
{
   ...
   if (IsNewCandle_HTF) LastTime_HTF = iTime(NULL,HTF,0);

   static datetime LastTime_LTF = 0;
   bool IsNewCandle_LTF = LastTime_LTF != iTime(NULL,LTF,0);
   if (IsNewCandle_LTF) LastTime_LTF = iTime(NULL,LTF,0);
...
)
{
    ...
    {
        if (iTime(NULL,TF,i) < Fibo_Time2) break;
        ...
        else if (NumOfBars_Minor == 2) 
        {
            BreakType = 1;
            break;
        }
        else NumOfBars_Minor = 0;
    }
      ...
      //   if (!OrderExist(_Symbol, type, comment) && !OrderExist(_Symbol, type, "Major Break"))
         {
            ...
            if (ticket <= 0 && -ticket != TRADE_RETCODE_INVALID_STOPS && - ticket != TRADE_RETCODE_INVALID_PRICE) Alert("Error: Order not opened, ErrorCode: "+(string)(-ticket) + " :: " + symbol + " "+OrderTypeToStr(type)+" " + DoubleToString(lot, 2) + " lots @ "+ DoubleToString(price, digits) + " SL: " + DoubleToString(sl, digits) + " TP: " + DoubleToString(tp, digits) + " Comment: "+comment);
            else if (ticket > 0)
            {
        ...
        // if (!OrderExist(_Symbol, type, comment) && !OrderExist(_Symbol, type, "Major Break"))
         {
            ...
            if (ticket <= 0 && -ticket != TRADE_RETCODE_INVALID_STOPS && - ticket != TRADE_RETCODE_INVALID_PRICE) Alert("Error: Order not opened, ErrorCode: "+(string)(-ticket) + " :: " + symbol + " "+OrderTypeToStr(type)+" " + DoubleToString(lot, 2) + " lots @ "+ DoubleToString(price, digits) + " SL: " + DoubleToString(slLTF, digits) + " TP: " + DoubleToString(tpLTF, digits) + " Comment: "+comment);
            else if (ticket > 0)
            {
...
void Check_LTF()
{
   ...
   if (Fibo_HTF_Level100 != Fibo_HTF_Level0 && Fibo_HTF_Level0 > 0 && Fibo_HTF_Level100 > 0)
   {
      ...
      int BuyOrSell;
      if (Fibo_HTF_Level0 > Fibo_HTF_Level100) BuyOrSell = 1;
      else BuyOrSell = 0;
...
void Check_HTF()
{
   ...
   if (LastTime == iTime(NULL,HTF,1)) return;
   ...
   if (Button_UpOrDown < 0)
   {
      ...
      if (Filling1 == EMPTY_VALUE || Filling1 <= 0 || Filling2 == EMPTY_VALUE || Filling2 <= 0 || Filling1 == Filling2) return;
...
if (TimeCurrent() >= D'2023.3.1 17:15')
{
      ...
      )
   {
      ...
      DeleteOrders(_Symbol);*/
        ...
        /* if (Fibo_HTF_Level100_Price > Fibo_HTF_Level0_Price
            ...
            )*/
         {
            ...
            NewFibo = true;
            if (IsHTF_Fibo_Updated || (!PositionExist(_Symbol))) Check_LTF();
      ...
      else
      {
         ...
         if (ShowFibo) ObjectDelete(0, ObjPref+"HTF_Fibo");
      }
   }
}
...
bool LastClosedPositionIsFromSameHTFFibo(string ThirdPart)
{
   ...
   for (int i = HistoryDealsTotal()-1; i >= 0; i--)
   {
      ...
      if (tick_out <= 0 || HistoryDealGetString(tick_out, DEAL_SYMBOL) != _Symbol || HistoryDealGetInteger(tick_out, DEAL_ENTRY) != DEAL_ENTRY_OUT) continue;
      ...
      for (int j = i-1; j >= 0; j--)
      {
         ...
         if (tick_in <= 0 || HistoryDealGetString(tick_in, DEAL_SYMBOL) != _Symbol || HistoryDealGetInteger(tick_in, DEAL_ENTRY) != DEAL_ENTRY_IN || PosID != HistoryDealGetInteger(tick_in, DEAL_POSITION_ID)) continue;
...
void CheckToCloseOppositeTradesOnHTFTrigger()
{
   ...
   {
      if (PositionGetTicket(i) <= 0 || PositionGetString(POSITION_SYMBOL) != _Symbol || PositionGetInteger(POSITION_MAGIC) != MagicNumber) continue;
      
      string comment = PositionGetString(POSITION_COMMENT);
      if (StringFind(comment, "N-HTF-") != 0 && StringFind(comment, "J-HTF-") != 0) continue;
