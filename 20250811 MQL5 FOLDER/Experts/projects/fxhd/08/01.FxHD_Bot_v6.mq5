...
input color LTF_Fibo_Color = clrOrange;
...
void OnTick()
{
      ...
      )
   {
      ...
      for (int i = 1; i < bars; i++)
      {
         ...
         else if (NumOfBars == 2) break;
         else NumOfBars = 0;
         
         if (NumOfBars == 3) break;
      }
      ...
      if (NumOfBars > 1)
      {
         ...
         string comment = (NumOfBars == 2 ? "N-" : "J-")+(TF == HTF ? "HTF-" : "LTF-")+DoubleToString(FiboHTF.Price1,_Digits)+","+DoubleToString(FiboHTF.Price2,_Digits);
         string comment_other = (NumOfBars == 2 ? "J-" : "N-")+(TF == HTF ? "HTF-" : "LTF-")+DoubleToString(FiboHTF.Price1,_Digits)+","+DoubleToString(FiboHTF.Price2,_Digits);
...
//&& -ticket != TRADE_RETCODE_INVALID_STOPS && -ticket != TRADE_RETCODE_INVALID_PRICE
            ...
            if (ticket <= 0 ) Alert("Error: Order not opened, ErrorCode: "+(string)(-ticket) + " :: " + symbol + " "+OrderTypeToStr(type)+" " + DoubleToString(lot, 2) + " lots @ "+ DoubleToString(price, digits) + " SL: " + DoubleToString(sl, digits) + " TP: " + DoubleToString(tp, digits) + " Comment: "+comment);
            else if (ticket > 0)
            {
