...
//+------------------------------------------------------------------+
#property copyright "Copyright © 2023, [kingdom financier]"
#property link      "https://t.me/AlisterFx/"
#property version   "1.00"
#property description "\n\nFeature Set:"
#property description "\n____________"
#property description "\nOpens a position (buy or sell) at the specified time."
#property description "\nCloses the position at a set time"
#property description "\nSLTP configurable"
#property description "\nAlerts configurable"

bool                  RUNMODE_EXPIRY=true;    //Enable expiry
datetime              expiryDate = D'2024.09.14 11:00'; //change as per your requirement



/*

      reference version  Timed Order    earnforex
      --
      ==modes of operation===
...
input string                  DailyTime = "16:00:00"; // Time for daily trades in HH:MM:SS format.
input int                     DailyTradeDuration = 0; // Duration of daily trade in minutes (0 = no auto-close)
...
input int                     Magic = 84684544;
input string                  OrderCommentary = "ea_ez_timed";
/*input*/ bool                    Silent = false; // Silent: If true, does not display any output via chart comment.
/*input*/ bool                    Logging = false; // Logging: If true, errors will be logged to a file.
...
int OnInit()
{
   if(RUNMODE_EXPIRY) Print(" Demo : Expires at "+(string)expiryDate);
   ResetVariables();
...
{
   if (!EnablePositionClosing) return;
   if (!DailyMode) return;
   //if (PositionClosed) return;
   datetime current_time = (CloseTimeType == TIME_TYPE_SERVER) ? TimeCurrent() : TimeLocal();
   
   if (DailyMode && DailyTradeDuration > 0) {
      if (PositionSelect(Symbol())) {
         datetime position_time = (datetime)PositionGetInteger(POSITION_TIME);
         if (current_time >= position_time + DailyTradeDuration * 60) {
            if (Trade.PositionClose(Symbol())) {
               Output("Daily mode position closed after " + IntegerToString(DailyTradeDuration) + " minutes.");
               PositionClosed = true;
            }
            else {
               Output("Failed to close daily mode position: " + Trade.ResultRetcodeDescription());
            }
            return;
         }
      }
   ...
   if (EnablePositionClosing && current_time >= NextCloseTime) {
      if (PositionSelect(Symbol())) {
         if (Trade.PositionClose(Symbol())) {
...
// Time is due. At this point, if something fails, the EA won't try again until it is reloaded.
   ...
   else if (SLType == SLTP_TYPE_SPREADS)
      SL = SymbolInfoInteger(Symbol(), SYMBOL_SPREAD) * StopLoss * Point();
   else if (SLType == SLTP_TYPE_DISTANCE)
      SL = StopLoss * Point();
   ...
   else if (TPType == SLTP_TYPE_SPREADS)
      TP = SymbolInfoInteger(Symbol(), SYMBOL_SPREAD) * TakeProfit * Point();
   else if (TPType == SLTP_TYPE_DISTANCE)
      TP = TakeProfit * Point();
   else
      TP = TakeProfit; // Level.
...
void ShowStatus()
{
   ...
   if (DailyMode) 
   {
      ...
      if (DailyTradeDuration > 0) {
         s += "\nDaily trade duration: " + IntegerToString(DailyTradeDuration) + " minutes";
         if (PositionSelect(Symbol())) {
            datetime position_time = (datetime)PositionGetInteger(POSITION_TIME);
            datetime close_time = position_time + DailyTradeDuration * 60;
            datetime current_time = (TimeType == TIME_TYPE_SERVER) ? TimeCurrent() : TimeLocal();
            int remaining_time = (int)(close_time - current_time);
            if (remaining_time > 0) {
               s += "\nTime until position close: " + TimeDistance(remaining_time);
            }
         }
      }
   }
