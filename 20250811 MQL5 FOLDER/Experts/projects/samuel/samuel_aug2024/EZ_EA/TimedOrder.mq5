...
input group "Position Closing"
   input bool                    EnablePositionClosing = false; // Enable position closing at specified time
   input datetime                CloseTime = __DATETIME__; // Date/time to close the position
   input ENUM_TIME_TYPE          CloseTimeType = TIME_TYPE_SERVER; // Close time type
...
{
    if (!EnablePositionClosing || PositionClosed) return;
