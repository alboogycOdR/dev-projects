...
Single trade mode (default)
      opens a single trade at the time specified by the trader.
Daily mode
      open trades at the specified time on each of the enabled days of the week.
      For example, it can open a trade for you at 12:00 every day or at 15:00 every Monday and Thursday, and so on.

time can be given either as the server time or as the local time. The time can be specified down to the seconds.
...
--
set the maximum allowed spread to avoid executing the trade when spreads are too wide
...
input group "Trading"
    datetime                OrderTime = __DATETIME__; // Date/time (server) to open order
   input ENUM_BETTER_ORDER_TYPE  OrderType = BETTER_ORDER_TYPE_BUY; // Order type
    double                  Entry = 0; // Entry price (optional)
    int                     EntryDistancePoints = 0; // Entry distance in points (for pending)
    double                  StopPrice = 0; // Stop price (for Stop Limit orders)
...
input group "Position Closing"
input bool                    EnablePositionClosing = true; // Enable position closing at specified time
 datetime                CloseTime = __DATETIME__; // Date/time to close the position
 ENUM_TIME_TYPE          CloseTimeType = TIME_TYPE_SERVER; // Close time type
bool PositionClosed = false;
datetime NextCloseTime;

input group "Control"
 datetime                Expires = 0; // Expires on (if pending)
...
input int                     MaxSpread = 3000; // Maximum spread in points
input bool                    RetryUntilMaxSpread = false; // Retry until spread falls below MaxSpread?
 int                     Slippage = 3000; // Maximum slippage in points
 ENUM_TIMEFRAMES         ATR_Timeframe = PERIOD_CURRENT; // ATR Timeframe
 int                     ATR_Period = 14; // ATR Period

input group "Daily mode"
input bool                    DailyMode = true; // Daily mode: if true, will trade every given day.
input string                  DailyTime = "11:00:00"; // Time for daily trades in HH:MM:SS format.
input int                     DailyTradeDuration = 20; // Duration of daily trade in minutes
...
int OnInit()
  {
   ...
   if(Error != "")
     {
      if(!Silent)
         Comment("Wrong input parameters!\n" + Error);
...
//check and close the position
void CheckAndClosePosition()
  {
// if(!EnablePositionClosing || PositionClosed)
//    return;
   if(!EnablePositionClosing && !DailyMode)
      return;
   datetime current_time = (CloseTimeType == TIME_TYPE_SERVER) ? TimeCurrent() : TimeLocal();
   if(DailyMode)
     {
      if(PositionSelect(Symbol()))
        {
         datetime position_time = (datetime)PositionGetInteger(POSITION_TIME);
         datetime close_time = position_time + (DailyTradeDuration * 60); // Convert minutes to seconds
         //Print("position_time: ", position_time, " close_time: ", close_time, " current_time: ", current_time);

         if(current_time >= close_time)
           {
            if(Trade.PositionClose(Symbol()))
              {
               Output("Daily trade position closed after " + IntegerToString(DailyTradeDuration) + " minutes.");
               PositionClosed = true;
              }
            else
              {
               Output("Failed to close daily trade position: " + Trade.ResultRetcodeDescription());
              }
           }
        }
     }
// if(current_time >= NextCloseTime)
//   {
//    if(PositionSelect(Symbol()))
//      {
//       if(Trade.PositionClose(Symbol()))
//         {
//          Output("Position closed at specified time.");
//          PositionClosed = true;
//         }
//       else
//         {
//          Output("Failed to close position: " + Trade.ResultRetcodeDescription());
//         }
//      }
//    else
//      {
//       Output("No open position to close at specified time.");
//       PositionClosed = true;
//      }
//   }
  }
...
void OnTick()
  {
   if(!CanWork)
      return; // Don't operate if there was some initialization problem.
   if(!Silent)
      ShowStatus();
   if(!WillNoLongerTryOpeningTrade)
      DoTrading();
...
// If account currency == pair's base currency, adjust UnitCost to future rate (SL). Works only for Forex pairs.
   ...
   if((SL != 0) && (UnitCost != 0) && (TickSize != 0))
      PositionSize = NormalizeDouble(RiskMoney / (SL * UnitCost / TickSize), LotStep_digits);
   if(PositionSize < SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN))
      PositionSize = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
   else
      if(PositionSize > SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MAX))
         PositionSize = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MAX);
   double steps = PositionSize / SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
   if(MathFloor(steps) < steps)
      PositionSize = MathFloor(steps) * SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
   return PositionSize;
  }
...
void Output(string s)
  {
   ...
   if(file == -1)
      Print("Failed to create an error log file: ", GetLastError(), ".");
   else
     {
      ...
      if((TerminalInfoInteger(TERMINAL_TRADE_ALLOWED) == false) && (Terminal_Trade_Allowed == true))
        {
         ...
         if((TerminalInfoInteger(TERMINAL_TRADE_ALLOWED) == true) && (Terminal_Trade_Allowed == false))
           {
...
// Do nothing if it is too early.
   ...
   if(TimeType == TIME_TYPE_SERVER)
      current_time = TimeCurrent();
   else
      current_time = TimeLocal();
   datetime order_time = OrderTime;
   if(DailyMode)
      order_time = GetOrderTimeForDailyMode(current_time);
   if(current_time < order_time)
      return;
// Time is due. At this point, if something fails,
//the EA won't try again until it is reloaded.
   WillNoLongerTryOpeningTrade = true;
   double SL = 0, TP = 0;
   if(SLType == SLTP_TYPE_ATR)
     {
      double ATR[];
      while(CopyBuffer(ATR_handle, 0, 0, 1, ATR) != 1)
         return;
      SL = ATR[0] * StopLoss;
     }
   else
      if(SLType == SLTP_TYPE_SPREADS)
         SL = SymbolInfoInteger(Symbol(), SYMBOL_SPREAD) * StopLoss * Point();
      else
         if(SLType == SLTP_TYPE_DISTANCE)
            SL = StopLoss * Point();
         else
            SL = StopLoss; // Level.
   if(TPType == SLTP_TYPE_ATR)
     {
      ...
      if(TPType == SLTP_TYPE_SPREADS)
         TP = SymbolInfoInteger(Symbol(), SYMBOL_SPREAD) * TakeProfit * Point();
      else
         if(TPType == SLTP_TYPE_DISTANCE)
            TP = TakeProfit * Point();
         else
            TP = TakeProfit; // Level.
   ...
   if((MaxSpread > 0) && (spread > MaxSpread))
     {
      ...
      if(RetryUntilMaxSpread)
         explanation += " Waiting for spread to go below the MaxSpread setting.";
      Output(explanation);
      if(RetryUntilMaxSpread)
        {
      ...
      if(AlertsOnFailure)
        {
         ...
         if(EnableEmailAlerts)
            SendMail("Timed Order Alert - " + Symbol() + " @ " + StringSubstr(EnumToString((ENUM_TIMEFRAMES)Period()), 7), Text);
         if(EnablePushAlerts)
   ...
   if((TradeType == ORDER_TYPE_BUY) || (TradeType == ORDER_TYPE_SELL))  // Market
     {
      ...
      if(!PositionSelectByTicket(ticket))
      ...
      else
        {
         ...
         if(SL != 0)
            PostOrderText += " SL = " + DoubleToString(PositionGetDouble(POSITION_SL), _Digits);
         if(TP != 0)
            PostOrderText += " TP = " + DoubleToString(PositionGetDouble(POSITION_TP), _Digits);
        }
     }
   else // Pending
     {
      ...
      double price = SymbolInfoDouble(Symbol(), SYMBOL_ASK);
      double entry = Entry;
      if((TradeType == ORDER_TYPE_SELL_STOP) || (TradeType == ORDER_TYPE_SELL_LIMIT) || (TradeType == ORDER_TYPE_SELL_STOP_LIMIT))
         price = SymbolInfoDouble(Symbol(), SYMBOL_BID);
      if((EntryDistancePoints > 0) && (TradeType != ORDER_TYPE_BUY_STOP_LIMIT) && (TradeType != ORDER_TYPE_SELL_STOP_LIMIT))  // Doesn't work with Stop Limit orders.
        {
         ...
         if((order_type == ORDER_TYPE_BUY_STOP) || (order_type == ORDER_TYPE_SELL_LIMIT))
            entry = price + EntryDistancePoints * _Point;
         else
            if((order_type == ORDER_TYPE_SELL_STOP) || (order_type == ORDER_TYPE_BUY_LIMIT))
               entry = price - EntryDistancePoints * _Point;
        }
      else
        {
         if((TradeType == ORDER_TYPE_BUY_STOP) && (Entry < price))
            order_type = ORDER_TYPE_BUY_LIMIT;
         else
            if((TradeType == ORDER_TYPE_BUY_LIMIT) && (Entry > price))
               order_type = ORDER_TYPE_BUY_STOP;
            else
               if((TradeType == ORDER_TYPE_SELL_STOP) && (Entry > price))
                  order_type = ORDER_TYPE_SELL_LIMIT;
               else
                  if((TradeType == ORDER_TYPE_SELL_LIMIT) && (Entry < price))
                     order_type = ORDER_TYPE_SELL_STOP;
        }
      if((TradeType == ORDER_TYPE_BUY_STOP_LIMIT) && (StopPrice < price))
         order_type = ORDER_TYPE_SELL_STOP_LIMIT;
      else
         if((TradeType == ORDER_TYPE_SELL_STOP_LIMIT) && (StopPrice > price))
      ...
      if((SLType != SLTP_TYPE_PRICELEVEL) && (SL != 0))
        {
         ...
         if((order_type == ORDER_TYPE_BUY_STOP) || (order_type == ORDER_TYPE_BUY_LIMIT) || (order_type == ORDER_TYPE_BUY_STOP_LIMIT))
            ...
            if((order_type == ORDER_TYPE_SELL_STOP) || (order_type == ORDER_TYPE_SELL_LIMIT) || (order_type == ORDER_TYPE_SELL_STOP_LIMIT))
               SL = entry + SL;
        }
      if((TPType != SLTP_TYPE_PRICELEVEL) && (TP != 0))
        {
      ...
      if(!OrderSelect(ticket))
      ...
      else
        {
         ...
         if(SL != 0)
            PostOrderText += " SL = " + DoubleToString(OrderGetDouble(ORDER_SL), _Digits);
         if(TP != 0)
            PostOrderText += " TP = " + DoubleToString(OrderGetDouble(ORDER_TP), _Digits);
         if((order_type == ORDER_TYPE_BUY_STOP_LIMIT) || (order_type == ORDER_TYPE_SELL_STOP_LIMIT))
            PostOrderText += " StopPrice = " + DoubleToString(OrderGetDouble(ORDER_PRICE_STOPLIMIT), _Digits);
         if(OrderGetInteger(ORDER_TIME_EXPIRATION) > 0)
            PostOrderText += " Expiration = " + TimeToString(OrderGetInteger(ORDER_TIME_EXPIRATION));
        }
     }
   if(ticket > 0)
     {
      global_ticket = ticket;
      PositionClosed = false; //new - alister
      if(AlertsOnSuccess)
        {
   ...
   if(!DailyMode)  // Normal mode (one-time fixed-date trade).
     {
      ...
      if(((TimeType == TIME_TYPE_SERVER) && (OrderTime <= TimeCurrent())) ||
         ((TimeType == TIME_TYPE_LOCAL) && (OrderTime <= TimeLocal())))
         return "Order time has already passed.";
     }
   else // Daily mode (trades every set day at a given time).
     {
      if(!CheckTime(DailyTime))
         return "DailyTime should be in correct HH:MM:SS format.";
      if((!Monday) && (!Tuesday) && (!Wednesday) && (!Thursday) && (!Friday) && (!Saturday) && (!Sunday))
         return "At least one day of the week should be selected.";
     }
// Pending order with zero entry.
   if((TradeType != ORDER_TYPE_BUY) && (TradeType != ORDER_TYPE_SELL) && (Entry == 0) && (EntryDistancePoints <= 0))
      return "Entry price and distance cannot be both zero for pending orders.";
// Stop limit order with zero Stop price.
   if((TradeType == ORDER_TYPE_BUY_STOP_LIMIT) && (TradeType == ORDER_TYPE_SELL_STOP_LIMIT) && (StopPrice == 0))
      return "Stop price cannot be zero for Stop Limit orders.";
      ...
      if(StopLoss >= Entry)
        {
         if(TradeType == ORDER_TYPE_BUY_STOP)
            return "Stop-loss cannot be above entry for a Buy Stop.";
         if(TradeType == ORDER_TYPE_BUY_LIMIT)
            return "Stop-loss cannot be above entry for a Buy Limit.";
        }
      if(StopLoss <= Entry)
        {
         if(TradeType == ORDER_TYPE_SELL_STOP)
            return "Stop-loss cannot be below entry for a Sell Stop.";
         if(TradeType == ORDER_TYPE_SELL_LIMIT)
            return "Stop-loss cannot be below entry for a Sell Limit.";
        }
     }
   if((TakeProfit != 0) && (TPType == SLTP_TYPE_PRICELEVEL) && (EntryDistancePoints <= 0))
     {
      if(TakeProfit <= Entry)
        {
         if(TradeType == ORDER_TYPE_BUY_STOP)
            return "Take-profit cannot be below entry for a Buy Stop.";
         if(TradeType == ORDER_TYPE_BUY_LIMIT)
            return "Take-profit cannot be below entry for a Buy Limit.";
        }
      if(TakeProfit >= Entry)
        {
         if(TradeType == ORDER_TYPE_SELL_STOP)
            return "Take-profit cannot be above entry for a Sell Stop.";
         if(TradeType == ORDER_TYPE_SELL_LIMIT)
            return "Take-profit cannot be above entry for a Sell Limit.";
        }
     }
   if(!CalculatePositionSize)
     {
      if(FixedPositionSize < SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MIN))
         return "Position size " + DoubleToString(FixedPositionSize, CountDecimalPlaces(FixedPositionSize)) + " < minimum volume " + DoubleToString(SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MIN), CountDecimalPlaces(SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MIN)));
      if(FixedPositionSize > SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MAX))
         return "Position size " + DoubleToString(FixedPositionSize, CountDecimalPlaces(FixedPositionSize)) + " > maximum volume " + DoubleToString(SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MAX), CountDecimalPlaces(SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MAX)));
      double steps = FixedPositionSize / SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_STEP);
      if(MathFloor(steps) < steps)
         return "Position size " + DoubleToString(FixedPositionSize, CountDecimalPlaces(FixedPositionSize)) + " is not a multiple of lot step " + DoubleToString(SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_STEP), CountDecimalPlaces(SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_STEP)));
     }
   else
     {
      if(StopLoss == 0)
         return "Cannot calculate position size based on zero stop-loss.";
     }
   if((Expires > 0) && (Expires < OrderTime))
      return "Expiration cannot be earlier than order time.";
   if(Retries < 1)
      return "Retries cannot be < 1.";
   if(MaxDifference < 0)
      return "MaxDifference cannot be negative.";
   if(MaxSpread < 0)
      return "MaxSpread cannot be negative.";
   if(Slippage < 0)
      return "Slippage cannot be negative.";
   if(ATR_Period < 0)
      return "ATR_Period cannot be negative.";
   return "";
  }
...
void ShowStatus()
  {
   string s = "EZE EA _ Timed Order EA\n";
   ...
   if((TradeType == ORDER_TYPE_BUY_STOP) || (TradeType == ORDER_TYPE_BUY_LIMIT) || (TradeType == ORDER_TYPE_SELL_STOP) || (TradeType == ORDER_TYPE_SELL_LIMIT))
     {
      if(EntryDistancePoints <= 0)
         ...
         s += " @ " + IntegerToString(EntryDistancePoints) + " pts.";
     }
   else
      if((TradeType == ORDER_TYPE_BUY_STOP_LIMIT) || (TradeType == ORDER_TYPE_SELL_STOP_LIMIT))
        {
         s += " @ " + DoubleToString(Entry, _Digits);
        }
   if((TradeType == ORDER_TYPE_BUY_STOP_LIMIT) || (TradeType == ORDER_TYPE_SELL_STOP_LIMIT))
      s += " via " + DoubleToString(StopPrice, _Digits);
   if((!DailyMode) && (global_ticket != 0))  // Order already executed or tried to execute.
     ...
     {
      s += "\n";
      if(SLType == SLTP_TYPE_PRICELEVEL)
         s += "SL = " + DoubleToString(StopLoss, _Digits);
      else
         if(SLType == SLTP_TYPE_DISTANCE)
            s += "SL = " + IntegerToString((int)StopLoss) + " pts.";
         else
            if(SLType == SLTP_TYPE_ATR)
               s += "SL = " + DoubleToString(StopLoss, CountDecimalPlaces(StopLoss)) + " x ATR(" + IntegerToString(ATR_Period) + ") @ " + EnumToString(ATR_Timeframe);
            else
               if(SLType == SLTP_TYPE_SPREADS)
                  s += "SL = " + DoubleToString(StopLoss, CountDecimalPlaces(StopLoss)) + " spreads";
      s += "\n";
      if(TPType == SLTP_TYPE_PRICELEVEL)
         s += "TP = " + DoubleToString(TakeProfit, _Digits);
      else
         if(TPType == SLTP_TYPE_DISTANCE)
            s += "TP = " + IntegerToString((int)TakeProfit) + " pts.";
         else
            if(TPType == SLTP_TYPE_ATR)
               s += "TP = " + DoubleToString(TakeProfit, CountDecimalPlaces(TakeProfit)) + " x ATR(" + IntegerToString(ATR_Period) + ") @ " + EnumToString(ATR_Timeframe);
            else
               if(TPType == SLTP_TYPE_SPREADS)
                  s += "TP = " + DoubleToString(TakeProfit, CountDecimalPlaces(TakeProfit)) + " spreads";
      s += "\n";
      if(!CalculatePositionSize)
         s += "Pos Size = " + DoubleToString(FixedPositionSize, CountDecimalPlaces(FixedPositionSize)) + " lots";
      else
        {
         if(UseMoneyInsteadOfPercentage)
            s += "Risk = " + DoubleToString(MoneyRisk, CountDecimalPlaces(AccountInfoDouble(ACCOUNT_BALANCE))) + " " + AccountInfoString(ACCOUNT_CURRENCY);
         else
            if(UseEquityInsteadOfBalance)
               s += "Risk = " + DoubleToString(Risk, CountDecimalPlaces(Risk)) + "% of Equity (" + DoubleToString(AccountInfoDouble(ACCOUNT_EQUITY), 2) + " " + AccountInfoString(ACCOUNT_CURRENCY) + ")";
            else
               if(FixedBalance > 0)
                  s += "Risk = " + DoubleToString(Risk, CountDecimalPlaces(Risk)) + "% of " + DoubleToString(FixedBalance, CountDecimalPlaces(AccountInfoDouble(ACCOUNT_BALANCE))) + " " + AccountInfoString(ACCOUNT_CURRENCY);
               else
                  s += "Risk = " + DoubleToString(Risk, CountDecimalPlaces(Risk)) + "% of Balance (" + DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE), CountDecimalPlaces(AccountInfoDouble(ACCOUNT_BALANCE))) + " " + AccountInfoString(ACCOUNT_CURRENCY) + ")";
        }
     }
   s += "\n";
   datetime current_time;
   ...
   if(difference <= 0)
      s += "\nTime to next order: " + TimeDistance(-difference);
   else
      s += "\nTime after order: " + TimeDistance(difference);
   if(DailyMode)
     {
      if(difference < -10)
         WillNoLongerTryOpeningTrade = false; // Reset for further order taking.
      s += " (daily mode)";
     }
   s += ".";
   if(EnablePositionClosing)
     {
      s += "\nENABLED: Position closing ";
      if(PositionClosed)
         s += "completed.";
      else
        {
        //alister    todo
         //datetime current_time = (CloseTimeType == TIME_TYPE_SERVER) ? TimeCurrent() : TimeLocal();
         //int difference = (int)NextCloseTime - (int)current_time;
         //if(difference > 0)
         //   s += "in: " + TimeDistance(difference);
         //else
         //   s += "overdue by: " + TimeDistance(-difference);
        }
     }
   Comment(s);
  }
...
string OrderToString(ENUM_ORDER_TYPE ot)
  {
   ...
   if(ot == ORDER_TYPE_BUY_STOP)
      return "Buy Stop";
   if(ot == ORDER_TYPE_SELL_LIMIT)
      return "Sell Limit";
   if(ot == ORDER_TYPE_SELL_STOP)
      return "Sell Stop";
   if(ot == ORDER_TYPE_BUY_STOP_LIMIT)
      return "Buy Stop Limit";
   if(ot == ORDER_TYPE_SELL_STOP_LIMIT)
...
datetime GetOrderTimeForDailyMode(datetime current_time)
  {
   ...
   if(current_time > target_time + 10)
      skip_current_day = true; // Skip the current day. Give a 10 seconds buffer.
   ...
   for(int i = 0; i <= 7; i++, target_time += 24 * 60 * 60)  // <= because the next day could be the same day of the week.
     {
      ...
      if(EnabledDays[target_time_struct.day_of_week])
         break;
     }
   return target_time;
  }
...
MaxDifference (default = 0) — if OrderType is set to Buy or Sell and some non-zero Entry is given, the EA will only execute the order if the difference between the current price and the Entry value is less or equal to the MaxDifference value. Its value is set in points.
