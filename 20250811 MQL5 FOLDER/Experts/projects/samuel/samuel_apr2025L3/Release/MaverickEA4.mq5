...
/*
 V4.1  revert back to more simple TIME BASED ENTRY based on Server Time
 v3.1
...
//|                                                                  |
//+------------------------------------------------------------------+
input int      InpTradeServerHour = 0;            //SERVER Hour for Main Trade Entry (0-23)
input int      InpTradeServerMinute = 0;         //SERVER Minute for Main Trade Entry (0-59)
...
// --- 3. Parameter Validation ---
   ...
   if(InpLotSize <= 0 || InpSLPointsMain <= 0 || InpTPPointsMain <= 0 || InpMagicMain == 0 || InpMagicMain == InpMagicRecovery || (UseRecoveryTrade && (InpLotSizeRecovery <= 0 || InpSLPointsRecovery <= 0 || InpTPPointsRecovery <= 0 || InpMagicRecovery == 0)))
     {
      string errorMsg = "OnInit Error: Invalid input parameters detected: ";
      if(InpLotSize <= 0) errorMsg += "Main Lot Size must be > 0. ";
      if(InpSLPointsMain <= 0) errorMsg += "Main SL Points must be > 0. ";
      if(InpTPPointsMain <= 0) errorMsg += "Main TP Points must be > 0. ";
      if(InpMagicMain == 0) errorMsg += "Main Magic Number cannot be 0. ";
      if(InpMagicMain == InpMagicRecovery) errorMsg += "Main and Recovery Magic Numbers must be different. ";
      if(UseRecoveryTrade) {
         if(InpLotSizeRecovery <= 0) errorMsg += "Recovery Lot Size must be > 0. ";
         if(InpSLPointsRecovery <= 0) errorMsg += "Recovery SL Points must be > 0. ";
         if(InpTPPointsRecovery <= 0) errorMsg += "Recovery TP Points must be > 0. ";
         if(InpMagicRecovery == 0) errorMsg += "Recovery Magic Number cannot be 0. ";
      }
      Print(errorMsg);
      Alert(errorMsg);
      return INIT_PARAMETERS_INCORRECT;
   ...
   if(!ValidateAdjustLotSize(InpLotSize, adjustedLotSize, "Main") || (UseRecoveryTrade && !ValidateAdjustLotSize(InpLotSizeRecovery, adjustedLotSizeRecovery, "Recovery")))
    {
        ...
        bool mainLotValid = ValidateAdjustLotSize(InpLotSize, adjustedLotSize, "Main");
        bool recoveryLotValid = !UseRecoveryTrade || ValidateAdjustLotSize(InpLotSizeRecovery, adjustedLotSizeRecovery, "Recovery");
        
        string errorMsg = "OnInit Error: Lot size validation failed - ";
        if(!mainLotValid)
            errorMsg += "Main trade lot size ("+DoubleToString(InpLotSize, 2)+") is invalid for "+_Symbol+". ";
        if(UseRecoveryTrade && !recoveryLotValid)
            errorMsg += "Recovery trade lot size ("+DoubleToString(InpLotSizeRecovery, 2)+") is invalid for "+_Symbol+". ";
            
        double minLot = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
        double maxLot = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MAX);
        double stepLot = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
        errorMsg += "Symbol requirements: Min="+DoubleToString(minLot, 2)+", Max="+DoubleToString(maxLot, 2)+", Step="+DoubleToString(stepLot, 2);
...
//    lastClosedPair.evaluated = false;
 ...
 // --- 6. Init Runtime State ---
    serverTimeOnInit = TimeCurrent(); // Need this for date/line marker
...
// Status Line - updated
   ...
   if (dailyTradingEnded) // Check this FIRST
   {
       stateStr = "** DAILY TRADING ENDED **";
   }
   else if(recoveryTradeActive)
   {
   ...
   else  if(mainCount > 0 ) //|| recCount > 0)
     {
      ...
      else// If not ended, no recovery active, no main trades active... must be Idle/Awaiting
      {
stateStr = StringFormat("Idle / Awaiting Server %02d:%02d", InpTradeServerHour, InpTradeServerMinute);
