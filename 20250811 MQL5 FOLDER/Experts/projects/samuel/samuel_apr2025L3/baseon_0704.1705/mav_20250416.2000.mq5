...
16 april
   22 - 27 march is now running fine
   multiple reocovery
   multiple main trades
...
Condition Removed: In both the "Buy Survivor" (A3) and "Sell Survivor" (B3) blocks, the condition !recoveryTradeActive && !recoveryUsedToday has been removed from the if statement that checks for the price hitting the breakeven stop (pair.buySLAtBE or pair.sellSLAtBE).
Remaining Conditions: The checks UseRecoveryTrade (is recovery enabled globally?) and !dailyTradingEnded (has trading stopped for the day?) are kept.
recoveryUsedToday Check Added: Crucially, I've added a nested check if (!recoveryUsedToday) immediately before calling OpenRecoveryTrade(). This prevents the EA from opening potentially multiple new recovery trades within the same day specifically triggered by this BE-hit mechanism. The original SL-hit recovery trigger (OnStopLossHit) already implicitly handles the recoveryUsedToday flag because it won't call OpenRecoveryTrade if the flag is true. This new check ensures the BE-hit trigger also respects the "one recovery sequence per day" rule.


Important Caveat (Requires Future Refactoring): MAVERICK0705_
While this fix allows the opening of a second recovery trade pair as requested, the current management logic for recovery trades (TrailSLRecoveryTrade and ModifySLToEntry(InpMagicRecovery)) is not designed to handle multiple concurrent recovery pairs correctly.
ModifySLToEntry(InpMagicRecovery) assumes only one recovery survivor exists.
TrailSLRecoveryTrade iterates all recovery positions without distinguishing which recovery pair they belong to.
To fully support multiple concurrent recovery pairs, you will eventually need to:
Implement a RecoveryPairInfo struct and an activeRecoveryPairs[] array, similar to how main pairs are tracked.
Modify OpenRecoveryTrade to add new pairs to this array.
Rewrite the recovery management section in ManagePositions to iterate through activeRecoveryPairs and manage each recovery pair independently.
Update ModifySLToEntry(int magic) and TrailSLRecoveryTrade to handle the context of a specific recovery pair, likely using GUIDs embedded in the recovery position comments (e.g., "RB-GUID", "RS-GUID").
...
void OnStopLossHit(int magicNumber, ulong positionTicket, ulong dealTicket, string guid)
  {
   ...
   if(lastClosedPair.guid == guid && lastClosedPair.buyClosed && lastClosedPair.sellClosed && !lastClosedPair.evaluated)
   {
       ...
       // +++ DEBUG START +++
       if(InpEnableDebug) PrintFormat("OnStopLossHit Check Start (%s): doNotRecover=%s | skipRecovery=%s | UseRecoveryTrade=%s | TotalProfit=%.2f", // Removed dailyTradingEnded
                                     guid, // Include GUID for clarity
                                     BoolToString(doNotRecover),
                                     BoolToString(skipRecovery),
                                     // BoolToString(dailyTradingEnded), // Commented out
       ...
       if (!doNotRecover && !UseRecoveryTrade) {
          ...
          // No separate Print here as it's just input setting
       }
       // +++ DEBUG START +++
       if(InpEnableDebug) PrintFormat("OnStopLossHit Check After Disabled (%s): doNotRecover=%s", guid, BoolToString(doNotRecover)); // Corrected typo doNowRecover -> doNotRecover
       // +++ DEBUG END +++


       // 4. Day Ended?  (NOTE: dailyTradingEnded check is commented out in the 'if' but included in debug)
       if (!doNotRecover /*&& dailyTradingEnded*/) { // Check dailyTradingEnded here if intended
             // This condition is tricky. If dailyTradingEnded is meant to block, the condition is just 'dailyTradingEnded' if !doNotRecover is true
             // Let's assume the commented intention was correct:
             // if (dailyTradingEnded) { // Only set if day has actually ended // Commented out dailyTradingEnded usage
             //      doNotRecover = true; reason += "DayEnded ";
                  // if (InpEnableDebug) PrintFormat("Pair %s: Recovery Decision - SKIPPED (Daily Trading Ended).", guid); // Specific log if needed
             // }
       }
        // +++ DEBUG START +++
        if(InpEnableDebug) PrintFormat("OnStopLossHit Check After DayEnded Check (%s): doNotRecover=%s | reason='%s'", guid, BoolToString(doNotRecover), reason); // Print reason here too
        // +++ DEBUG END +++
       ...
       if (!doNotRecover) {
           ...
           // OpenRecoveryTrade NO LONGER sets recoveryUsedToday flag
       } else {
            // Recovery denied for one or more reasons
            // The Reason string should now be populated correctly if a condition was met
            if(InpEnableDebug) PrintFormat("Pair %s: Recovery Decision - DENIED. Final State: doNotRecover=%s. Reasons: [%s]", guid, BoolToString(doNotRecover), StringTrimRight(reason)); // Enhanced Denial log
       }

       // Reset global skip flag AFTER evaluation for THIS SL event is complete.
       // This allows the next independent SL event (if any) to be evaluated correctly.
       if (skipRecovery && InpEnableDebug) PrintFormat("OnStopLossHit: Resetting skipRecovery flag for GUID %s", guid); // Log reset
       skipRecovery = false;
   ...
   if(startOfDay > currentTradeDate) // Check if the calculated start of day is later than our tracked date
     {
      ...
      if (recoveryPositions > 0) {
          ...
          if (InpEnableDebug) Print("Daily Reset: No recovery positions found. Recovery active flag reset."); // Adjusted log
      }
...
bool OpenRecoveryTrade() // Return true if successful
  {
   ...
   if (buyRecOpenedOK && sellRecOpenedOK) {
        ...
        // !!! You NEED to add code HERE to add this new pair to a tracking structure (like activeRecoveryPairs) for correct management LATER !!!
...
void PrintTradeStatus(string context = "")
  {
     ...
     status += StringFormat("\nDate: %s | Rec Active: %s | Skip Rec Flag: %s", // Removed Day Ended, Rec Used
...
void UpdateRunningTotal()
  {
   ...
   string dashboard = StringFormat("%s==== MAVERICK EA (v%.2f) ====\r\n\r\n%sSymbol: %s | Time: %s\r\n",
    ...
    // -- Status Line --
    string stateStr = "";
    if (recoveryTradeActive) stateStr = "[RECOVERY ACTIVE]";
