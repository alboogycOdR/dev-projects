...
void ManagePositions()
{
    ...
    // Assumes only one recovery sequence is possible/active at a time.
    ...
    if (recoveryTradeActive) { // Only manage if the recovery sequence *should* be active
        if (recoveryOpenCount == 2) {
             // Both recovery positions open: Check if SL should be moved to BE based on profit
             TrailSLRecoveryTrade();
        } else if (recoveryOpenCount == 1) {
             // One recovery pos closed: Move survivor's SL to entry
             ModifySLToEntry(InpMagicRecovery); // Uses the simple overload
        } else if (recoveryOpenCount == 0) {
            // Both recovery positions are now closed. Recovery phase is over.
             ...
             // PrintTradeStatus("After Recovery Close"); // Log state change
        }
    } else if(recoveryOpenCount > 0 && !recoveryTradeActive) {
         // Safety Check: Found recovery positions open, but recoveryTradeActive was false. Correct state.
    ...
    // --- 2. Main Position Management (Only if Recovery is NOT Active) ---
    if (!recoveryTradeActive)
    {
        ...
        {
             // Removed: HedgePairInfo &pair = activeMainPairs[i]; // Get reference
            ...
            // ---- Case A: Only BUY Survivor Open ----
             if (isBuyOpen && !isSellOpen) {
                 ...
                 // A1. Ensure SL at Breakeven (updates pair.buySLAtBE)
                 ModifySLToEntry(InpMagicMain, activeMainPairs[i].buyTicket, activeMainPairs[i]); // Pass struct by value/copy
                 // A2. Check Stage 2 Trailing (requires pair.buySLAtBE == true)
                 if (activeMainPairs[i].buySLAtBE) { TrailSLMainTrade(activeMainPairs[i].buyTicket, activeMainPairs[i]); } // Pass struct by value/copy
                 ...
                 if (activeMainPairs[i].buySLAtBE && !recoveryTradeActive && !recoveryUsedToday && UseRecoveryTrade && !dailyTradingEnded) {
                     MqlTick tick;
                     if(SymbolInfoTick(_Symbol, tick) && tick.bid <= activeMainPairs[i].buyEntry) { // Check Bid <= Buy Entry
            ...
            // ---- Case B: Only SELL Survivor Open ----
                  ...
                  // B1. Ensure SL at Breakeven (updates pair.sellSLAtBE)
                 ModifySLToEntry(InpMagicMain, activeMainPairs[i].sellTicket, activeMainPairs[i]); // Pass struct by value/copy
                 // B2. Check Stage 2 Trailing (requires pair.sellSLAtBE == true)
                 if (activeMainPairs[i].sellSLAtBE) { TrailSLMainTrade(activeMainPairs[i].sellTicket, activeMainPairs[i]); } // Pass struct by value/copy
                 // B3. Check for Price hitting BE stop -> Trigger Recovery?
                  if (activeMainPairs[i].sellSLAtBE && !recoveryTradeActive && !recoveryUsedToday && UseRecoveryTrade && !dailyTradingEnded) {
                     MqlTick tick;
                     if(SymbolInfoTick(_Symbol, tick) && tick.ask >= activeMainPairs[i].sellEntry) { // Check Ask >= Sell Entry
            ...
            // ---- Case C: BOTH sides are OPEN ----
                  ...
                  // No BE or Trailing logic needed here yet. One must close first.
                  // Could add logic for initial trailing based on price movement if required by strategy.
             }
    ...
    } // End Main Pair Management block
} // End ManagePositions
