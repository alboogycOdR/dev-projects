...
#property version   "1.9" // Added Hedge Status Dashboard
#property description "\n\nMaverick EA"
#property description "\n____________"
#property description "\nHedging System"
#property description "\nMain and Recovery Trades - Multiple Pair Handling"
#property description "\nVERSION 1.9: Added second dashboard for detailed hedge pair status."
...
bool           InpEnableDebug        = false;     // Enable detailed debug output
int            UpdateFrequency = 1000;       // Dashboard update frequency in milliseconds (1 sec = 1000)
bool           InpIsLogging=false;          // Verbose logging for OnTradeTransaction raw events
    ...
    for (int i = 0; i < mainPairCount; i++) // Loop forward is fine here, no removal
    {
        ...
        if (activeMainPairs[i].buyTicket > 0 && activeMainPairs[i].sellTicket == 0 && activeMainPairs[i].buySLAtBE)
        {
            ...
            if (tick.bid <= activeMainPairs[i].buyEntry)
            {
                ...
                if (!alreadyTriggered) {
                     if(InpEnableDebug) PrintFormat("CheckBEHits: Price %.5f touched/crossed Buy Entry %.5f (SL at BE) for Pair %s. Attempting Recovery.",
        ...
        else if (activeMainPairs[i].sellTicket > 0 && activeMainPairs[i].buyTicket == 0 && activeMainPairs[i].sellSLAtBE)
        {
             ...
             if (tick.ask >= activeMainPairs[i].sellEntry)
             {
                 ...
                 if (!alreadyTriggered) {
                      if(InpEnableDebug) PrintFormat("CheckBEHits: Price %.5f touched/crossed Sell Entry %.5f (SL at BE) for Pair %s. Attempting Recovery.",
            ...
            if (sl > 0.0) {
                if (pType == POSITION_TYPE_BUY) {
                    if (sl > entryPrice + tolerance) {
                        slStatus = "trailed";
                    } else if (sl < entryPrice - tolerance) {
                        slStatus = "unchanged";
                    } else {
                        slStatus = "breakeven";
                    }
      ...
      if(magic == InpMagicMain && closeReason == DEAL_REASON_SL && guid != "") {
         ...
         for(int i = 0; i < ArraySize(trackedPositions); i++) {
            ...
            if(trackedPositions[i].ticket == position_id && trackedPositions[i].guid == guid) {
               ...
               else if(trackedPositions[i].posType == POSITION_TYPE_SELL && trackedPositions[i].trailedSL < trackedPositions[i].entryPrice - point) {
                  slWasTrulyTrailed = true;
               }
   ...
   for(int i = 0; i < count; i++) {
      ...
      for(int j = i + 1; j < count; j++) {  // Look for a partner
         ...
         if(foundPositions[i].guid == foundPositions[j].guid && foundPositions[i].type != foundPositions[j].type) {
            ...
            else if(PositionSelectByTicket(activeMainPairs[index].sellTicket))
               activeMainPairs[index].openTime = (datetime)PositionGetInteger(POSITION_TIME);
            else
   ...
   for(int i = 0; i < count; i++) {
      ...
      for(int j = i + 1; j < count; j++) {  // Look for a partner
         ...
         if(foundPositions[i].guid == foundPositions[j].guid && foundPositions[i].type != foundPositions[j].type) {
            ...
            else if(PositionSelectByTicket(activeRecoveryPairs[index].sellTicket))
               activeRecoveryPairs[index].openTime = (datetime)PositionGetInteger(POSITION_TIME);
            else
...
void OnTick()
{
   ...
   // *** NEW: Check for BE hits on every tick ***
   CheckBEHits();
...
// It's crucial ManagePositions handles closed pairs correctly by updating activeMainPairs array
   ...
   if(!NewBar())
      return;
   DrawPositionLabels();         // *** NEW: Update position labels ***
...
//bool isTradingAllowed = !dailyTradingEnded;  // Allow new main trades regardless of recovery state
   if(isTradeTime) {
      if(InpEnableDebug)
         PrintFormat("Trade Time (%s) Reached. Checking if new MAIN pair needed.", InpTradeTime);
      ...
      if(currentTime - lastMainOpenAttemptTime >= 55) { // Throttle to ~1 attempt/min
         if(OpenMainTrade()) {  // Attempt to open a pair (this adds it to tracking on success)
            ...
            if(InpEnableDebug)
               Print("==> New Main Pair Opened & Tracked. <==");
         }
         else {
...
// Resize the array to remove the last slot (which is now a duplicate or the one to remove)
   ...
   if (foundIndex != -1) {
      if (InpEnableDebug) PrintFormat("RemoveActivePair: Removing GUID %s from triggeredBEGuids list.", guidToRemove);
...
void ManagePositions()
{
    ...
    for(int i = active_recovery_count - 1; i >= 0; i--)
    {
        ...
        {
            if(InpEnableDebug) PrintFormat("ManagePos Info (Rec): Pair %s correcting stale Buy Ticket %I64u.", activeRecoveryPairs[i].guid, activeRecoveryPairs[i].buyTicket);
        ...
        {
            if(InpEnableDebug) PrintFormat("ManagePos Info (Rec): Pair %s correcting stale Sell Ticket %I64u.", activeRecoveryPairs[i].guid, activeRecoveryPairs[i].sellTicket);
        ...
        if( (isRecBuyOpen && !isRecSellOpen) || (!isRecBuyOpen && isRecSellOpen) )
        {
            ...
            if(modifyResult)
            {
                if (isRecBuyOpen && !activeRecoveryPairs[i].buySLAtBE) // If Buy is survivor AND flag is not set
                {
                    ...
                    if(InpEnableDebug) PrintFormat("ManagePos: Explicitly set RecovPair %s buySLAtBE = true", activeRecoveryPairs[i].guid);
                }
                else if (isRecSellOpen && !activeRecoveryPairs[i].sellSLAtBE) // If Sell is survivor AND flag is not set
                {
                    activeRecoveryPairs[i].sellSLAtBE = true;
                    if(InpEnableDebug) PrintFormat("ManagePos: Explicitly set RecovPair %s sellSLAtBE = true", activeRecoveryPairs[i].guid);
                }
        ...
        else if(!isRecBuyOpen && !isRecSellOpen)
        {
            ...
            if(InpEnableDebug) PrintFormat("ManagePos: RecovPair %s BOTH sides confirmed closed. Removing from active list.", activeRecoveryPairs[i].guid);
    ...
    if (wasRecoveryActiveAtStart && ArraySize(activeRecoveryPairs) == 0)
    {
        if (!dailyTradingEnded) { // Only set if not already set (avoids repetitive logs)
           dailyTradingEnded = true;
           if(InpEnableDebug) Print("ManagePositions: All recovery pairs closed. Setting dailyTradingEnded=TRUE.");
        }
    }
    ...
    for(int i = active_main_count - 1; i >= 0; i--)
    {
        ...
        {
             if(InpEnableDebug) PrintFormat("ManagePos Info (Main): Pair %s correcting stale Buy Ticket %I64u.", activeMainPairs[i].guid, activeMainPairs[i].buyTicket);
        ...
        {
             if(InpEnableDebug) PrintFormat("ManagePos Info (Main): Pair %s correcting stale Sell Ticket %I64u.", activeMainPairs[i].guid, activeMainPairs[i].sellTicket);
        ...
        {
             // Debug Before Call
             if(InpEnableDebug) PrintFormat("ManagePos Case A (GUID %s): BEFORE ModifySL - buySLAtBE=%s", activeMainPairs[i].guid, BoolToString(activeMainPairs[i].buySLAtBE));
             ...
             // Debug After Call
             if(InpEnableDebug) PrintFormat("ManagePos Case A (GUID %s): AFTER ModifySL result=%s / buySLAtBE was %s", activeMainPairs[i].guid, BoolToString(modifyResult), BoolToString(activeMainPairs[i].buySLAtBE));
             ...
             if(modifyResult && !activeMainPairs[i].buySLAtBE) // Only set if needed
             {
                ...
                if(InpEnableDebug) PrintFormat("ManagePos: Explicitly set MainPair %s buySLAtBE = true", activeMainPairs[i].guid);
             }

             // Perform trailing only IF the BE flag is now confirmed true
             if(activeMainPairs[i].buySLAtBE)
             {
                 if(InpEnableDebug) PrintFormat("ManagePos Case A (GUID %s): Attempting Trail - buySLAtBE=%s", activeMainPairs[i].guid, BoolToString(activeMainPairs[i].buySLAtBE));
        ...
        {
             // Debug Before Call
             if(InpEnableDebug) PrintFormat("ManagePos Case B (GUID %s): BEFORE ModifySL - sellSLAtBE=%s", activeMainPairs[i].guid, BoolToString(activeMainPairs[i].sellSLAtBE));

             // Attempt to move SL to entry / confirm state
             bool modifyResult = ModifySLToEntry(InpMagicMain, activeMainPairs[i].sellTicket, activeMainPairs[i]); // Pass by reference

             // Debug After Call
             if(InpEnableDebug) PrintFormat("ManagePos Case B (GUID %s): AFTER ModifySL result=%s / sellSLAtBE was %s", activeMainPairs[i].guid, BoolToString(modifyResult), BoolToString(activeMainPairs[i].sellSLAtBE));

             // **Explicitly set the BE flag in the array if modify succeeded/state confirmed**
             if(modifyResult && !activeMainPairs[i].sellSLAtBE) // Only set if needed
             {
                activeMainPairs[i].sellSLAtBE = true;
                if(InpEnableDebug) PrintFormat("ManagePos: Explicitly set MainPair %s sellSLAtBE = true", activeMainPairs[i].guid);
             }

             // Perform trailing only IF the BE flag is now confirmed true
             if(activeMainPairs[i].sellSLAtBE)
             {
                 if(InpEnableDebug) PrintFormat("ManagePos Case B (GUID %s): Attempting Trail - sellSLAtBE=%s", activeMainPairs[i].guid, BoolToString(activeMainPairs[i].sellSLAtBE));
        ...
        else if (!isMainBuyOpen && !isMainSellOpen)
        {
            ...
            if(InpEnableDebug) PrintFormat("ManagePos: MainPair %s BOTH sides confirmed closed. Removing from active list.", activeMainPairs[i].guid);
...
{
   if(magic != InpMagicMain) return false; // Only Main
   ...
   else if(pair.sellTicket == survivorTicket && pair.buyTicket == 0) {
   ...
   if (InpEnableDebug) PrintFormat("ModifySLToEntry: MainPair %s, Pos %I64u (%s), Moving SL from %.5f to Entry %.5f",
   ...
   if (!trade.PositionModify(survivorTicket, entry, currentTP)) {
      ...
      if (InpEnableDebug) PrintFormat("ModifySLToEntry OK: MainPair %s, Pos %I64u SL set to entry.", pair.guid, survivorTicket);
...
// Status Line - updated
   ...
   else if(mainCount > 0 || recCount > 0)
