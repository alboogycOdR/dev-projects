...
try:
     with open('config.yaml', 'r') as f:
          ...
          config = cfg_data.get('sources',{}).get('telegram_fxscalping') # Adjust if source ID changes
          if not config or 'credentials' not in config:
              print("ERROR: Cannot find credentials for 'telegram_fxscalping' in config.yaml")
              exit(1)
...
session_name = config.get('session_name', 'mt5_signal_session')
