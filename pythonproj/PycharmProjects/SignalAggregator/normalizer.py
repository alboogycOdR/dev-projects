...
def normalize_signal(parsed_data: dict, symbol_map: dict = None) -> dict | None:
    if not isinstance(parsed_data, dict):
        logger.warning("Normalizer: Invalid Input (not a dict.")
        return None
    # Basic check if parsed_data is empty
    if not parsed_data:
         logger.warning("Norm: Input dict is empty.")
         return None
    ...
    try:
        normalized = {}
        # --- IDs ---
        msg_id = parsed_data.get('message_id', '0') # Get ID for logging early
        normalized['original_message_id'] = str(msg_id) #str(parsed_data.get('message_id', '0'))
        
        # --- Timestamp ---
        timestamp_dt = parsed_data.get('timestamp')
        if not isinstance(timestamp_dt, datetime):
            logger.warning(f"Norm: TS is not datetime obj for {msg_id}.")
            return None
        ...
        if action not in ["BUY", "SELL", "CLOSE"]:
             logger.warning(f"Norm: Unsupported final action '{action}' for {msg_id}. Discarding.")
             return None

        # ===> ASSIGN the final validated action to the dictionary <===
        logger.debug(f"NORMALIZE ACTION: 4. Assigning final '{action}'") # <-- DEBUG 3
        ...
        if not symbol: 
            logger.warning(f"Norm Symbol: Missing symbol for {msg_id}.")
            return None # Guard
        
        #normalized['symbol'] = symbol
        logger.debug(f"Norm Symbol: Start symbol='{symbol}', Map provided='{bool(symbol_map)}'")
        
        
        # ====> APPLY MAPPING LOGIC (This was missing) <====
        if symbol_map and symbol in symbol_map:
            original_symbol = symbol
            #symbol = str(symbol_map[symbol]).upper()
            symbol = str(symbol_map[symbol]) # <<< Keep original case from config value
            
            logger.info(f"Norm Symbol: Mapped '{original_symbol_for_log}' to '{symbol}' (Preserving Case)")
        else:
             logger.debug(f"Norm Symbol: No mapping defined for '{symbol}' or map is None.")
        # ====> END MAPPING LOGIC <====
        # 3. Clean the potentially mapped symbol
        # Allow A-Z, 0-9, dot, underscore, dash, whitespace
        cleaned_symbol = re.sub(r'[^A-Z0-9._\s-]', '', symbol,flags=re.IGNORECASE)# ADD IGNORECASE FLAG? Maybe not needed if map preserved case.
        logger.debug(f"Norm Symbol: 3. After cleaning regex ('[^A-Z0-9._\\s-]'), result = '{cleaned_symbol}'")

        if not cleaned_symbol: # Check if empty after cleaning
            logger.warning(f"Norm Symbol: Symbol became empty after cleaning! Original was '{parsed_data.get('symbol','')}', mapped intermediate was '{symbol}'.")
            return None

        # 4. Assign the FINAL potentially mapped and cleaned symbol
        normalized['symbol'] = cleaned_symbol
        logger.debug(f"Norm Symbol: 4. Final Assigned Symbol = '{normalized['symbol']}'")
        # --- End Symbol Normalization ---
        ...
        if normalized['action'] == "CLOSE":
             ...
             if original_id_str:
                  ...
                  except (ValueError, TypeError): pass # Ignore if invalid format, stays 0
        else: # BUY or SELL
            try:
