...
//https://www.mql5.com/en/code/21936
...
//--- input parameters
input color smartlinecolor = clrBlack; //DynamicTrendColor
...
class CTrendlineCrossAlertExpert {
 ...
 protected:
   ...
   string              FormatPrice(double price) {
      return StringFormat("%." + (string)m_symbol.Digits() + "f", price);
   };
   bool                IsNewBar() {
      return iVolume(NULL, 0, 0) == 1;
   };
   ...
   for(int x = 0; x < ObjectsTotal(ChartID(), 0, -1); x++) {
      if(ObjectGetInteger(ChartID(), ObjectName(ChartID(), x, -1, -1), OBJPROP_TYPE, 0) == OBJ_TREND) {
         if(ObjectGetInteger(ChartID(), ObjectName(ChartID(), x, -1, -1), OBJPROP_COLOR, 0) == smartlinecolor) 
         {
            ...
            ObjectSetInteger(ChartID(), ObjectName(ChartID(), x, -1, -1), OBJPROP_COLOR, clrYellow) ;
