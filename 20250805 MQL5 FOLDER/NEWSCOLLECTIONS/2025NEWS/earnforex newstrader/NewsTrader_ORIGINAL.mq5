   ...
   if(UseATR)
     {
      ...
      if(SL <= (SymbolInfoInteger(Symbol(), SYMBOL_TRADE_STOPS_LEVEL) + SymbolInfoInteger(Symbol(), SYMBOL_SPREAD)) * Point())
         SL = (SymbolInfoInteger(Symbol(), SYMBOL_TRADE_STOPS_LEVEL) + SymbolInfoInteger(Symbol(), SYMBOL_SPREAD)) * Point();
      TP = ATR * ATR_Multiplier_TP;
      if(TP <= (SymbolInfoInteger(Symbol(), SYMBOL_TRADE_STOPS_LEVEL) + SymbolInfoInteger(Symbol(), SYMBOL_SPREAD)) * Point())
         TP = (SymbolInfoInteger(Symbol(), SYMBOL_TRADE_STOPS_LEVEL) + SymbolInfoInteger(Symbol(), SYMBOL_SPREAD)) * Point();
      SL /= Point();
      TP /= Point();
     }
...
// In the netting mode: Adjust SL and TP of the current position
      ...
      if((PositionSelect(Symbol())) && (!Hedging_Mode))
         ControlPosition();
      else
        {
                     ...
                     if(Hedging_Mode)
                       {
                        ...
                        if((!HaveBuyPending) || (!HaveSellPending))
                          {
                           if(!BuyOrderAccepted)
                              fBuy_Pending();
                           if(!SellOrderAccepted)
   ...
   for(int cnt = total - 1; cnt >= 0; cnt--)
     {
      if(!Hedging_Mode)
        {
         if(!PositionSelect(PositionGetSymbol(cnt)))
            continue;
         if(PositionGetInteger(POSITION_MAGIC) != Magic)
            continue;
        }
         ...
         if((TrailingStop == Breakeven) && ((((PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) && (Ask - PositionGetDouble(POSITION_PRICE_OPEN) >= SL * Point())) || ((PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL) && (PositionGetDouble(POSITION_PRICE_OPEN) - Bid >= SL * Point())))))
           {
            ...
            if((TrailingStop == Full) && ((((PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) && (Ask - PositionGetDouble(POSITION_SL) >= SL * Point())) || ((PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL) && (PositionGetDouble(POSITION_SL) - Bid >= SL * Point())))))
              {
               double new_sl = 0;
               if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
                  new_sl = NormalizeDouble(Ask - SL * Point(), Digits());
   ...
   if(PositionSize < SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MIN))
     {
      ...
      if(PositionSize > SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MAX))
        {
