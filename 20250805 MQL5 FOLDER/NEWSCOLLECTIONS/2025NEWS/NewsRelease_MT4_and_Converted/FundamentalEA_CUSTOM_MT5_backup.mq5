...
input TradeWay EUR_TradeInNewsRelease  = Not_Trade;//Set Mode Trade On EUR Events
...
input bool   EUR_Trade_EURGBP          = false;//Trade EURGBP On EUR Events
input bool   EUR_Trade_EURAUD          = false;//Trade EURAUD On EUR Events
input bool   EUR_Trade_EURNZD          = false;//Trade EURNZD On EUR Events
input bool   EUR_Trade_EURUSD          = false;//Trade EURUSD On EUR Events
input bool   EUR_Trade_EURCAD          = false;//Trade EURCAD On EUR Events
input bool   EUR_Trade_EURCHF          = false;//Trade EURCHF On EUR Events
input bool   EUR_Trade_EURJPY          = false;//Trade EURJPY On EUR Events



input group "||====== GBP News Release Settings ======||";
input TradeWay GBP_TradeInNewsRelease  = Not_Trade;//Set Mode Trade On GBP Events
...
input bool   GBP_TradeIn_EURGBP        = false;//Trade EURGBP On GBP Events
input bool   GBP_TradeIn_GBPAUD        = false;//Trade GBPAUD On GBP Events
input bool   GBP_TradeIn_GBPNZD        = false;//Trade GBPNZD On GBP Events
input bool   GBP_TradeIn_GBPUSD        = false;//Trade GBPUSD On GBP Events
input bool   GBP_TradeIn_GBPCAD        = false;//Trade GBPCAD On GBP Events
input bool   GBP_TradeIn_GBPCHF        = false;//Trade GBPCHF On GBP Events
input bool   GBP_TradeIn_GBPJPY        = false;//Trade GBPJPY On GBP Events



input group  "||====== AUD News Release Settings ======||";
input TradeWay AUD_TradeInNewsRelease  = Not_Trade;//Set Mode Trade On AUD Events
...
input bool   AUD_TradeIn_EURAUD        = false;//Trade EURAUD On AUD Events
input bool   AUD_TradeIn_GBPAUD        = false;//Trade GBPAUD On AUD Events
input bool   AUD_TradeIn_AUDNZD        = false;//Trade AUDNZD On AUD Events
input bool   AUD_TradeIn_AUDUSD        = false;//Trade AUDUSD On AUD Events
input bool   AUD_TradeIn_AUDCAD        = false;//Trade AUDCAD On AUD Events
input bool   AUD_TradeIn_AUDCHF        = false;//Trade AUDCHF On AUD Events
input bool   AUD_TradeIn_AUDJPY        = false;//Trade AUDJPY On AUD Events



input group  "||====== NZD News Release Settings ======||";
input TradeWay NZD_TradeInNewsRelease  = Not_Trade;//Set Mode Trade On NZD Events
...
input bool   NZD_TradeIn_EURNZD        = false;//Trade EURNZD On NZD Events
input bool   NZD_TradeIn_GBPNZD        = false;//Trade GBPNZD On NZD Events
input bool   NZD_TradeIn_AUDNZD        = false;//Trade AUDNZD On NZD Events
input bool   NZD_TradeIn_NZDUSD        = false;//Trade NZDUSD On NZD Events
input bool   NZD_TradeIn_NZDCAD        = false;//Trade NZDCAD On NZD Events
input bool   NZD_TradeIn_NZDCHF        = false;//Trade NZDCHF On NZD Events
input bool   NZD_TradeIn_NZDJPY        = false;//Trade NZDJPY On NZD Events
...
input TradeWay CAD_TradeInNewsRelease  = Not_Trade;//Set Mode Trade On CAD Events
...
input bool   CAD_TradeIn_EURCAD        = false;//Trade EURCAD On CAD Events
input bool   CAD_TradeIn_GBPCAD        = false;//Trade GBPCAD On CAD Events
input bool   CAD_TradeIn_AUDCAD        = false;//Trade AUDCAD On CAD Events
input bool   CAD_TradeIn_NZDCAD        = false;//Trade NZDCAD On CAD Events
input bool   CAD_TradeIn_USDCAD        = false;//Trade USDCAD On CAD Events
input bool   CAD_TradeIn_CADCHF        = false;//Trade CADCHF On CAD Events
input bool   CAD_TradeIn_CADJPY        = false;//Trade CADJPY On CAD Events



input group  "||====== CHF News Release Settings ======||";
input TradeWay CHF_TradeInNewsRelease  = Not_Trade;//Set Mode Trade On CHF Events
...
input bool   CHF_TradeIn_EURCHF        = false;//Trade EURCHF On CHF Events
input bool   CHF_TradeIn_GBPCHF        = false;//Trade GBPCHF On CHF Events
input bool   CHF_TradeIn_AUDCHF        = false;//Trade AUDCHF On CHF Events
input bool   CHF_TradeIn_NZDCHF        = false;//Trade NZDCHF On CHF Events
input bool   CHF_TradeIn_USDCHF        = false;//Trade USDCHF On CHF Events
input bool   CHF_TradeIn_CADCHF        = false;//Trade CADCHF On CHF Events
input bool   CHF_TradeIn_CHFJPY        = false;//Trade CHFJPY On CHF Events



input group  "||====== JPY News Release Settings ======||";
input TradeWay JPY_TradeInNewsRelease  = Not_Trade;//Set Mode Trade On JPY Events
...
input bool   JPY_TradeIn_EURJPY        = false;//Trade EURJPY On JPY Events
input bool   JPY_TradeIn_GBPJPY        = false;//Trade GBPJPY On JPY Events
input bool   JPY_TradeIn_AUDJPY        = false;//Trade AUDJPY On JPY Events
input bool   JPY_TradeIn_NZDJPY        = false;//Trade NZDJPY On JPY Events
input bool   JPY_TradeIn_USDJPY        = false;//Trade USDJPY On JPY Events
input bool   JPY_TradeIn_CADJPY        = false;//Trade CADJPY On JPY Events
input bool   JPY_TradeIn_CHFJPY        = false;//Trade CHFJPY On JPY Events
...
input TimeInfoEnum ShowInfoTime        = Format_D_H_M;//Show Time's Format
...
//---------------------------------------------------------------------
//NEW
   string cookie=NULL,referer=NULL,headersInv, headers;
   char post[],result[];
      ...
      if(res==200)//--- Successful download
        {
         ...
         if(StringFind(info,"No Events Scheduled") != -1)
           ...
           {
            //    we have events to be processed
...
//       BoEvent=NextEvent;
//       //---
//       BeginWeek=0;SkipeEvent=false;
//       //---
//       for(I_INDEXER=0; I_INDEXER<7; I_INDEXER++)
//         {
//          mainData[NewsIDx][I_INDEXER]="";
//          NextEvent=StringFind(MyEvent,sTags[I_INDEXER],BeginWeek);
//          //---------------------------------------------------------------------
//          //Within this event,if tag not found, then it must be missing; skip it
//          if(NextEvent==-1)
//             continue;
...
//             //---------------------------------------------------------------------
//             //We must have found the sTag okay...
//             BeginWeek=NextEvent+StringLen(sTags[I_INDEXER]);//Advance past the start tag
//             EndWeek=StringFind(MyEvent,eTags[I_INDEXER],BeginWeek);//Find start of end tag
//             //---------------------------------------------------------------------
//             //Get data between start and end tag
//             if((EndWeek>BeginWeek) && (EndWeek!=-1))
//               {
//                mainData[NewsIDx][I_INDEXER]=StringSubstr(MyEvent,BeginWeek,EndWeek-BeginWeek);
//               }
//            }
//         }
//       //---------------------------------------------------------------------
...
//    ExtBufferSeconds[CountrySelect][0]=-9999;
//    ExtBufferSeconds[CountrySelect][1]=9999;
//    ExtBufferImpact[CountrySelect][0]=-1;//0;
//    ExtBufferImpact[CountrySelect][1]=-1;//0;
// //---------------------------------------------------------------------
// //Set buffers
//    for(I_INDEXER=0; I_INDEXER<NewsIDx; I_INDEXER++)
//      {
//       //---------------------------------------------------------------------
//       //Seconds UNTIL
//       if((ExtMapBuffer0[CountrySelect][I_INDEXER]>=0) && (mainData[I_INDEXER][COUNTRY]==CountryCheck))
//         {
//          if(ExtBufferSeconds[CountrySelect][1]==9999)
//            {
//             ExtBufferSeconds[CountrySelect][1]=ExtMapBuffer0[CountrySelect][I_INDEXER];
//             ExtBufferImpact[CountrySelect][1]=ImpactToNumber(mainData[I_INDEXER][IMPACT]);
//            }
//         }
//       //---------------------------------------------------------------------
//       //Seconds SINCE
...
//          //if(ExtBufferSeconds[CountrySelect][0]==-9999)
//          //{
//          ExtBufferSeconds[CountrySelect][0]=MathAbs(ExtMapBuffer0[CountrySelect][I_INDEXER]);
//          ExtBufferImpact[CountrySelect][0]=ImpactToNumber(mainData[I_INDEXER][IMPACT]);
//          //}
//         }
//      }
// //---------------------------------------------------------------------
...
//|                                                                  |
//+------------------------------------------------------------------+
void ReadNewsBACKUP(int CountrySelect,string CountryCheck)
  {
//---------------------------------------------------------------------
   string Cookie=NULL,Headers;
   char Post[],Result[];
...
//Read file
   ...
   if((SizeOfFile==0) ||
      ...
      (((GlobalVariableCheck("LastUpdateTime: ")==false) ||
       ...
       && (!IsTesting()) && (!IsVisualMode()) && (!IsOptimization())))
     {
      //---------------------------------------------------------------------
      //Get file
      ResetLastError();
      ...
      if(res==-1)
      ...
      else
        {
         ...
         if(xmlHandle<0)
           {
            Print("TRIED TO OPEN XML FILE: FAILED ");
            FileIsOk=false;
            return;
           }
         //---Checking errors
      ...
      if(xmlHandle<0)
      ...
      else
        {
         ...
         Print("xml handle is: "+(string)xmlHandle);
        }
      //---
      ulong    str_size=0;    // file size in bytes
      str_size=FileSize(xmlHandle);//added
      sData=FileReadString(xmlHandle,str_size);//INT_VALUE);
      bool pnt=FileSeek(xmlHandle,0,0);

      SizeOfFile=FileSize(xmlHandle);
      Print("SIZE OF FILE: "+(string)SizeOfFile);
      SizeOfData=StringLen(sData);
      Print("SIZE OF DATA: "+(string)SizeOfData);

      if(SizeOfData<SizeOfFile)
        {
         Print("SizeOfData<SizeOfFile  TRYING TO READ AGAIN");
         Print("xml handle is: "+(string)xmlHandle);
         int str_size=FileReadInteger(xmlHandle,INT_VALUE);
         sData=sData+FileReadString(xmlHandle,str_size+1);//(int)SizeOfFile);
         SizeOfData=StringLen(sData);
         Print("SIZE OF DATA after update: "+(string)SizeOfData);
        }
      ...
      //---------------------------------------------------------------------
      //Check if get Whole file
      EndWeek=StringFind(sData,"</weeklyevents>",0);

      Print("size of stringfind:  "+(string)EndWeek);
   ...
   while(true)
     {
      ...
      //Set skip switch
         ...
         SkipeEvent=true;
      //---------------------------------------------------------------------
      //SKIPEVENT PROCEDURES
         if((!IncludeLow) && (mainData[NewsIDx][IMPACT]=="Low")) SkipeEvent=true;
         if((!IncludeMedium) && (mainData[NewsIDx][IMPACT]=="Medium")) SkipeEvent=true;
         if((!IncludeHigh) && (mainData[NewsIDx][IMPACT]=="High")) SkipeEvent=true;
         if((mainData[NewsIDx][IMPACT]=="Holiday") || (mainData[NewsIDx][IMPACT]=="holiday")) SkipeEvent=true;
         if((mainData[NewsIDx][TIME]=="All Day") || (mainData[NewsIDx][TIME]=="Tentative") || (mainData[NewsIDx][TIME]=="")) SkipeEvent=true;
         if((!IncludeSpeaks) && ((StringFind(mainData[NewsIDx][TITLE],"speaks")!=-1) || (StringFind(mainData[NewsIDx][TITLE],"Speaks")!=-1))) SkipeEvent=true;
      //---------------------------------------------------------------------
      //Get unskip
      if(!SkipeEvent)
        {
         ImpactLastNews=mainData[NewsIDx][IMPACT];//Get impact
         NewsTime=StringToTime(MakeDateTime(mainData[NewsIDx][DATE],mainData[NewsIDx][TIME]));//First, convert the announcement time to seconds (in GMT)
         minsTillNews=NewsTime-CurrentTime;//Now calculate the Seconds until this announcement (may be negative)
         if((minsTillNews<0) || (MathAbs(tmpMins)>minsTillNews))
            tmpMins=minsTillNews;
         ExtMapBuffer0[CountrySelect][NewsIDx]=(int)minsTillNews;
         NewsIDx++;
