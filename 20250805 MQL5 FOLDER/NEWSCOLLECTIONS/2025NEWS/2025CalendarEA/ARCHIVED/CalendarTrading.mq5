...
#property tester_file "calendar2024.cal"
...
input string CalendarCacheFile = "calendar2024.cal";
...
int OnInit()
  ...
  Print(__FUNCTION__);
   ...
   if(!f.isLoaded())
     {
      Print("Calendar filter pointer issue ");
      return INIT_FAILED;
     }
...
void OnTimer()
  {
   ...
   string s=(string)(TimeTradeServer() - SCOPE_DAY);
   string t=(string)(TimeTradeServer() + SCOPE_DAY);
   Print(s);
   Print(t);
         ...
         if(FCAL_TBL_PTR.update(records)) // find changes that match filters
           {
            ...
            if(trackID == FCAL_TBL_PTR.getChangeID())
              {
