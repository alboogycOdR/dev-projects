...
//Print("before cache = new CalendarCache(xyz.cal, true);");
   cache = new CalendarCache("xyz.cal", true);
//Print("after cache = new CalendarCache(xyz.cal, true);");
//Print("_____________________");

   if(cache[].isLoaded())
     {
      fptr = new CalendarFilterCached(cache[]);
      // Print("  if(cache[].isLoaded()) ");
     }
   else
     {
   ...
   if(!f.isLoaded())
     {
      // Print("!f.isLoaded() - GOING TO RETURN INIT_FAILED");
      return INIT_FAILED;
     }
   /*
...
// setup timer for periodic trade by news
   ...
   EventSetTimer(100);//todo :   was EventSetTimer(1)
   ...
   if(f.update(records)) // find changes that match filters
     {
      ...
      request.comment = "newstrade impact:";
      ...
      if(impact > 0)
        {
         request.comment += impact;
         ticket = request.buy(Lot, 0,Distance2SLTP ? ask - point * Distance2SLTP : 0,Distance2SLTP ? ask + point * Distance2SLTP : 0);
        }
      else
         if(impact < 0)
           {
           request.comment += impact;
            ticket = request.sell(Lot, 0,Distance2SLTP ? bid + point * Distance2SLTP : 0,Distance2SLTP ? bid - point * Distance2SLTP : 0);
           }
      //configure trailing points
