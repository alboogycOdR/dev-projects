   ...
   for(int i = calendar.Total() - 1; i >= 0; i--)
     {
      ...
      else if(entry.value_time >= todayStart && entry.value_time < todayEnd)
        {
   ...
   for(int i = 0; i < calendar.Total(); i++)
     {
      ...
      else if(entry.event_importance == CALENDAR_IMPORTANCE_LOW)
         line.Color(clrGray);
      else if(entry.event_importance == CALENDAR_IMPORTANCE_NONE)
