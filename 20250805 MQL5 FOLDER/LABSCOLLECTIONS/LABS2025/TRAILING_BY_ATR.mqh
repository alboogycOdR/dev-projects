// Need to track the best prices reached for each position
struct TrailingData {
    ulong ticket;
...
// Global array to store trailing data
TrailingData trailData[];

double GetStopLossBuy(string symbol ,ulong ticket, double currentPrice)
{
    return iClose(symbol, PERIOD_CURRENT, 1) - GetATR(symbol) * ATRMultiplier;
}

double GetStopLossSell(string symbol, ulong ticket, double currentPrice)
{
    return iClose(symbol, PERIOD_CURRENT, 1) + GetATR(symbol) * ATRMultiplier;
}
/*
...
double GetStopLossBuy(string symbol, ulong ticket, double currentPrice)
{
    // Find position in tracking array or add it
    int idx = FindOrAddTrailData(ticket, currentPrice);
    
    // Update highest price if current price is better
    if(currentPrice > trailData[idx].highestPrice)
        trailData[idx].highestPrice = currentPrice;
    
    // Calculate new stop based on highest price seen
    double atrValue = GetATR(symbol) * ATRMultiplier;
    double newStopLoss = trailData[idx].highestPrice - atrValue;
    
    // Only return new stop if it's higher than current stop (true trailing)
    if(newStopLoss > trailData[idx].currentStop) {
        trailData[idx].currentStop = newStopLoss;
        return newStopLoss;
    }
    
    // Otherwise keep existing stop
    return trailData[idx].currentStop;
}
...
double GetStopLossSell(string symbol, ulong ticket, double currentPrice)
{
    ...
    // Update lowest price if current price is better
    if(currentPrice < trailData[idx].lowestPrice || trailData[idx].lowestPrice == 0)
        trailData[idx].lowestPrice = currentPrice;
    
    // Calculate new stop based on lowest price seen
    double atrValue = GetATR(symbol) * ATRMultiplier;
    double newStopLoss = trailData[idx].lowestPrice + atrValue;
    
    // Only return new stop if it's lower than current stop (true trailing)
    if(newStopLoss < trailData[idx].currentStop || trailData[idx].currentStop == 0) {
        trailData[idx].currentStop = newStopLoss;
        return newStopLoss;
    }
...
// Helper function to find or add position to tracking array
int FindOrAddTrailData(ulong ticket, double price)
{
    ...
    int newIdx = ArraySize(trailData);
    ArrayResize(trailData, newIdx + 1);
    trailData[newIdx].ticket = ticket;
    trailData[newIdx].highestPrice = price;  // Initial price
    trailData[newIdx].lowestPrice = price;   // Initial price
    trailData[newIdx].currentStop = 0;       // No stop yet
...
void TrailingStop(ulong magik)
{
    for (int i = 0; i < PositionsTotal(); i++)
    {
        ...
        if ((UseMagic) && (PositionGetInteger(POSITION_MAGIC) != magik)) continue;
        ...
        double NewSL = 0;double NewTP = 0;
        string Instrument = PositionGetString(POSITION_SYMBOL);
        double SLBuy = GetStopLossBuy(Instrument, ticket, PositionGetDouble(POSITION_SL));double SLSell = GetStopLossSell(Instrument, ticket, PositionGetDouble(POSITION_SL));
