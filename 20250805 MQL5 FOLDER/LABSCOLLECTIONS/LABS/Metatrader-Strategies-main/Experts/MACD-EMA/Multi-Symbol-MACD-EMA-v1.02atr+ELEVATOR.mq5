//info
//+------------------------------------------------------------------+
//|                                  Multi-Symbol-MACD-EMA-v1.02.mq5 |
...
//+------------------------------------------------------------------+
//|                                                  Version Notes 1 |
//| This file uses multi-symbol functionality
//to create an EA that   |
...
//+------------------------------------------------------------------+
//|                                                    Patch Notes 2 |
//| Fixed errors from v1. [Invalid Volume] error was occuring on     |
//| some pairs due to fixed lot size of 0.01. Changed to 0.1.        |
//| [Invalid Stops] error was occuring on some pairs due to fixed    |
//| stop loss of 0.01 away from entry. Changed to 0.1.               |
//+------------------------------------------------------------------+
//|                                              Strategy Conditions |
...
string AllTradableSymbols   = "GBPNZD|GBPAUD|EURAUD|EURGBP";
//-----------------------------------------------------------
...
int    TicksReceived  =  0;
//Risk Metrics
input bool   ATRMM = true;//ATR based tp and sl
input double LotSize = 0.1;//Volume
input double StopLossSize    = 600;//SL
input double TakeProfitSize  = 200;//TP
...
//double MaxLossPrc        = 0.02;   //Percent Risk Per Trade
...
//Set Up Multi-Symbol Handles for Indicators
   if(!MacdHandleMultiSymbol() ||!ATRHandleMultiSymbol() || !EmaHandleMultiSymbol())
      return(INIT_FAILED);
      ...
      if(IsNewCandle == true)
        {
         ...
         ATRSignal = GetATRValue(SymbolLoop);

         //Enter Trades
         if(IndicatorSignal1 == "Long" && IndicatorSignal2 == "Long")
            TicketNumber=ProcessTradeOpen(CurrentSymbol, SymbolLoop, ORDER_TYPE_BUY,ATRSignal);
         else
            if(IndicatorSignal1 == "Short" && IndicatorSignal2 == "Short")
               TicketNumber=ProcessTradeOpen(CurrentSymbol, SymbolLoop, ORDER_TYPE_SELL,ATRSignal);


         //Adjust Open Positions - Trailing Stop Loss
         if(TslCheck == true)
            AdjustTsl(CurrentSymbol,TicketNumber, ATRSignal, AtrLossMulti);
...
bool ATRHandleMultiSymbol()
  {
   for(int SymbolLoop=0; SymbolLoop < NumberOfTradeableSymbols; SymbolLoop++)
     {
      ResetLastError();
      HandleAtr[SymbolLoop] =  iATR(SymbolArray[SymbolLoop]
                                    ,Period()
                                    ,AtrPeriod);
...
//Define Macd and Signal lines, from not confirmed candle 0, for 3 candles, and store results. NOTE:[prior,current confirmed,not confirmed]
   ...
   if(FillMacd==false || FillSignal==false)
      return "Fill Error"; //If buffers are not completely filled, return to end onTick
...
//Populate buffers for ATR Value; check errors
   bool FillAtr = CopyBuffer(HandleAtr[SymbolLoop]
                             ,IndexAtr
                             ,StartCandle
                             ,RequiredCandles
                             ,BufferAtr); //Copy buffer uses oldest as 0 (reversed)

   if(FillAtr==false)
      return(0);

//Find ATR Value for Candle '1' Only
//todo - atr digit value based on SYMBOL specific
   int    SymbolDigits    = (int) SymbolInfoInteger(CurrentSymbol,SYMBOL_DIGITS); //note - typecast required to remove error
   double CurrentAtr   = NormalizeDouble(BufferAtr[1]
                                         ,SymbolDigits);
...
bool ProcessTradeOpen(string CurrentSymbol, int SymbolLoop, ENUM_ORDER_TYPE OrderType, double CurrentAtr)
  {
//Set symbol string and variables
   int    SymbolDigits    = (int) SymbolInfoInteger(CurrentSymbol,SYMBOL_DIGITS); //note - typecast required to remove error
   double    SymbolPoints    =  SymbolInfoDouble(CurrentSymbol,SYMBOL_POINT); //note - typecast required to remove error
   ...
   if(ATRMM)
      ...
      if(OrderType == ORDER_TYPE_BUY)
        {
         ...
         StopLossPrice   = NormalizeDouble(Price - CurrentAtr*AtrLossMulti, SymbolDigits);
         TakeProfitPrice = NormalizeDouble(Price + CurrentAtr*AtrProfitMulti, SymbolDigits);
        }
      else
   ...
   else
     {
      if(OrderType == ORDER_TYPE_BUY)
        {
         Price           = NormalizeDouble(SymbolInfoDouble(CurrentSymbol, SYMBOL_ASK), SymbolDigits);
         StopLossPrice   = NormalizeDouble(Price - StopLossSize*SymbolPoints, SymbolDigits);
         TakeProfitPrice = NormalizeDouble(Price + TakeProfitSize*SymbolPoints, SymbolDigits);
        }
      else
         if(OrderType == ORDER_TYPE_SELL)
           {
...
//Close any current positions and open new position
   ...
   Trade.SetExpertMagicNumber(MagicNumber);
   Trade.PositionOpen(CurrentSymbol
                      ,OrderType
                      ,LotSize
                      ,Price
                      ,StopLossPrice
...
//Print successful
   Print("Trade Processed For ", CurrentSymbol
         ," OrderType ",OrderType
         , " Lot Size ", LotSize
         , " Ticket ", TicketNumber
        );
