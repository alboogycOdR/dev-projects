...
static input int    Max_Open_Positions =     1; // Max Open Positions
...
void ManageOrderSend(int command, double lots, double stopLoss, double takeProfit, ulong ticket, int magicNumber)
  {
   //int posTotal = PositionsTotal();
   //if(posTotal >= Max_Open_Positions)
   //   return;
      ...
      if(IsTradeContextFree())
        {
         ...
         double atr_multiplier = 2.0;  // Multiplier for ATR
         double atr_value = iATR(Symbol(), PERIOD_CURRENT, atr_period);  // Calculate ATR value
         double take_profit_price=0.0;
         //=====
         if(atr_value > 0)
            if(command==OP_BUY)
               take_profit_price = Bid() + (atr_value * atr_multiplier)*Point();
         if(command==OP_SELL)
            take_profit_price = Ask() - (atr_value * atr_multiplier)*Point();
         ...
         request.sl           = stopLoss;
         request.tp           = take_profit_price;//takeProfit;
...
// Envelopes (Close, Simple, 25, 0.88)
   double ind3buffer0[];
   CopyBuffer(indHandlers[0][3][0], 0, 1, 2, ind3buffer0);
   double ind3buffer1[];
   CopyBuffer(indHandlers[0][3][0], 1, 1, 2, ind3buffer1);
...
// Standard Deviation (Close, Simple, 35), Level: 0.0045
   double ind2buffer[];
   CopyBuffer(indHandlers[1][2][0], 0, 1, 3, ind2buffer);
   double ind2val1  = ind2buffer[2];
...
// Accelerator Oscillator
   double ind2buffer[];
   CopyBuffer(indHandlers[2][2][0], 0, 1, 3, ind2buffer);
   double ind2val1  = ind2buffer[2];
   double ind2val2  = ind2buffer[1];
...
// Stochastic Signal (4, 1, 1)
   double ind4buffer0[];
   CopyBuffer(indHandlers[2][4][0], MAIN_LINE,   1, 2, ind4buffer0);
   double ind4buffer1[];
   CopyBuffer(indHandlers[2][4][0], SIGNAL_LINE, 1, 2, ind4buffer1);
...
// Alligator (Smoothed, Median, 43, 29, 29, 14, 14, 4)
   double ind1buffer0[];
   CopyBuffer(indHandlers[3][1][0], 0, 1, 2, ind1buffer0);
   double ind1buffer1[];
   CopyBuffer(indHandlers[3][1][0], 1, 1, 2, ind1buffer1);
   double ind1buffer2[];
   CopyBuffer(indHandlers[3][1][0], 2, 1, 2, ind1buffer2);
...
// Standard Deviation (Close, Simple, 26), Level: 0.0052
   double ind2buffer[];
   CopyBuffer(indHandlers[3][2][0], 0, 1, 3, ind2buffer);
   double ind2val1  = ind2buffer[2];
   double ind2val2  = ind2buffer[1];
...
// Directional Indicators (37)
   double ind1buffer0[];
   CopyBuffer(indHandlers[4][1][0], 1, 1, 2, ind1buffer0);
   double ind1buffer1[];
   CopyBuffer(indHandlers[4][1][0], 2, 1, 2, ind1buffer1);
...
// Envelopes (Close, Simple, 23, 0.66)
   double ind2buffer0[];
   CopyBuffer(indHandlers[4][2][0], 0, 1, 2, ind2buffer0);
   double ind2buffer1[];
   CopyBuffer(indHandlers[4][2][0], 1, 1, 2, ind2buffer1);
...
// Stochastic (17, 17, 15), Level: 20.0
   double ind0buffer[];
   CopyBuffer(indHandlers[5][0][0], MAIN_LINE, 1, 3, ind0buffer);
   double ind0val1  = ind0buffer[2];
   double ind0val2  = ind0buffer[1];
...
// Williams' Percent Range (24)
   double ind3buffer[];
   CopyBuffer(indHandlers[5][3][0], 0, 1, 3, ind3buffer);
   double ind3val1  = ind3buffer[2];
   double ind3val2  = ind3buffer[1];
   bool   ind3long  = ind3val1 < ind3val2 - sigma;
   bool   ind3short = ind3val1 > ind3val2 + sigma;
...
// ADX (43), Level: 38.0
   double ind4buffer[];
   CopyBuffer(indHandlers[5][4][0], 0, 1, 3, ind4buffer);
   double ind4val1  = ind4buffer[2];
...
// ADX (38), Level: 21.0
   double ind1buffer[];
   CopyBuffer(indHandlers[6][1][0], 0, 1, 3, ind1buffer);
   double ind1val1  = ind1buffer[2];
   bool   ind1long  = ind1val1 > 21.0 + sigma;
   bool   ind1short = ind1long;
// Bollinger Bands (Close, 17, 2.49)
   double ind2buffer0[];
   CopyBuffer(indHandlers[6][2][0], 1, 1, 2, ind2buffer0);
   double ind2buffer1[];
   CopyBuffer(indHandlers[6][2][0], 2, 1, 2, ind2buffer1);
   double ind2upBand1 = ind2buffer0[1];
   double ind2dnBand1 = ind2buffer1[1];
   double ind2upBand2 = ind2buffer0[0];
   double ind2dnBand2 = ind2buffer1[0];
...
// Alligator (Smoothed, Median, 20, 4, 4, 3, 3, 2)
   double ind0buffer0[];
   CopyBuffer(indHandlers[7][0][0], 0, 1, 2, ind0buffer0);
   double ind0buffer1[];
   CopyBuffer(indHandlers[7][0][0], 1, 1, 2, ind0buffer1);
   double ind0buffer2[];
   CopyBuffer(indHandlers[7][0][0], 2, 1, 2, ind0buffer2);
...
// Directional Indicators (23)
   double ind1buffer0[];
   CopyBuffer(indHandlers[7][1][0], 1, 1, 2, ind1buffer0);
   double ind1buffer1[];
   CopyBuffer(indHandlers[7][1][0], 2, 1, 2, ind1buffer1);
   double ind1val1  = ind1buffer0[1];
   double ind1val2  = ind1buffer1[1];
   double ind1val3  = ind1buffer0[0];
   double ind1val4  = ind1buffer1[0];
...
// Stochastic Signal (7, 2, 1)
   double ind2buffer0[];
   CopyBuffer(indHandlers[7][2][0], MAIN_LINE,   1, 2, ind2buffer0);
   double ind2buffer1[];
   CopyBuffer(indHandlers[7][2][0], SIGNAL_LINE, 1, 2, ind2buffer1);
...
// Envelopes (Close, Simple, 23, 0.16)
   double ind3buffer0[];
   CopyBuffer(indHandlers[7][3][0], 0, 1, 2, ind3buffer0);
   double ind3buffer1[];
   CopyBuffer(indHandlers[7][3][0], 1, 1, 2, ind3buffer1);
   double ind3upBand1 = ind3buffer0[1];
   double ind3dnBand1 = ind3buffer1[1];
...
// Envelopes (Close, Simple, 37, 0.64)
   double ind2buffer0[];
   CopyBuffer(indHandlers[8][2][0], 0, 1, 2, ind2buffer0);
   double ind2buffer1[];
   CopyBuffer(indHandlers[8][2][0], 1, 1, 2, ind2buffer1);
   double ind2upBand1 = ind2buffer0[1];
   double ind2dnBand1 = ind2buffer1[1];
   double ind2upBand2 = ind2buffer0[0];
   double ind2dnBand2 = ind2buffer1[0];
...
// Alligator (Smoothed, Median, 30, 18, 18, 6, 6, 5)
   double ind0buffer0[];
   CopyBuffer(indHandlers[9][0][0], 0, 1, 2, ind0buffer0);
   double ind0buffer1[];
   CopyBuffer(indHandlers[9][0][0], 1, 1, 2, ind0buffer1);
   double ind0buffer2[];
   CopyBuffer(indHandlers[9][0][0], 2, 1, 2, ind0buffer2);
...
// Moving Averages Crossover (Simple, Simple, 4, 23)
   double ind1buffer0[];
   CopyBuffer(indHandlers[9][1][0], 0, 1, 2, ind1buffer0);
   double ind1buffer1[];
   CopyBuffer(indHandlers[9][1][1], 0, 1, 2, ind1buffer1);
   double ind1val1  = ind1buffer0[1];
   double ind1val2  = ind1buffer1[1];
   bool   ind1long  = ind1val1 < ind1val2 - sigma;
   bool   ind1short = ind1val1 > ind1val2 + sigma;
