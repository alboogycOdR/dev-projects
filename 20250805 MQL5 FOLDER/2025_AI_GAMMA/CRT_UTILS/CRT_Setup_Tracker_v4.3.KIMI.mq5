...
//--- NEW GLOBAL INPUT VARIABLES ---
input group         "Dashboard Layout & Dimensions";
input int           dashboard_base_x = 150;      // Base X position for dashboard
input int           col_width_asset = 120;       // Asset column width
input int           col_width_bias = 120;        // Bias column width
input int           col_width_status = 120;      // Status column width
input int           group_padding = 0;          // Padding between column groups
input int           row_height = 60;            // Height per row
input int           base_y_offset = 80;         // Base Y position offset

input group         "Visual Colors & Styling";
input color         color_bg = clrWhiteSmoke;           // Background color
input color         color_header = clrBlack;          // Header text color
input color         color_text = clrBlack;           // General text color
input color         color_bull = C'38,166,154';      // Bullish bias color
input color         color_bear = C'239,83,80';       // Bearish bias color
input color         color_neutral = C'67,70,81';     // Neutral bias color
input color         color_sweep = clrDarkOrange;     // Sweep state color
input color         color_mss = clrDodgerBlue;       // MSS state color
input color         color_entry = C'0,128,0';        // Entry state color

input group         "Icons & Symbols";
input string        icon_bull = "▲";               // Bullish icon symbol
input string        icon_bear = "▼";               // Bearish icon symbol
input string        icon_neutral = "↔";            // Neutral icon symbol
input string        icon_wait = "—";               // Wait icon symbol
...
void SetThemeColors()
{
    ...
    {
        case THEME_LIGHT:
            c_bg           = color_bg;    c_header  = color_header;
            c_text         = color_text;   c_bull_bias = color_bull;
            c_bear_bias    = color_bear;  c_neutral_bias = color_neutral;
            c_state_sweep  = color_sweep;  c_state_mss = color_mss;
            c_state_entry  = color_entry;
            break;
 ...
 void CreateDashboard()
{
    ...
    int base_x          = dashboard_base_x;

    // Use global input variables directly, do not redeclare
    // int col_width_asset = col_width_asset;
    // int col_width_bias  = col_width_bias;
    // int col_width_status= col_width_status;
    // int group_padding= group_padding;
    // int row_height      = row_height;
    int base_y= base_y_offset;
    // int group_padding= group_padding;
    // int row_height      = row_height;
    ...
    int total_height = row_height * 9 + 30;
    ...
    // --- Create all 16 rows of data labels (initialized as empty) ---
    for(int i = 0; i < 16; i++) {
        ...
        CreateTextLabel(object_prefix+"Sym_"+(string)i,    "", current_asset_x + 5,  row_y, c_text, 8, corner, ANCHOR_LEFT);
        CreateTextLabel(object_prefix+"Bias_"+(string)i,   "—", current_bias_x + 20,  row_y, c_text, 10, corner, ANCHOR_CENTER);
        CreateTextLabel(object_prefix+"Status_"+(string)i, "", current_status_x + 5, row_y, c_text, 8, corner, ANCHOR_LEFT);
    }
}
//+------------------------------------------------------------------+
 void UpdateDashboard()
{
    for(int i=0; i<16; i++) {
        ...
        string bias_icon = (bias==BULLISH)?icon_bull:(bias==BEARISH)?icon_bear:(bias==NEUTRAL)?icon_neutral:icon_wait;
        ...
        ObjectSetString(0,object_prefix+"Sym_"+(string)i,OBJPROP_TEXT,symbol_states[i].symbol_name);
