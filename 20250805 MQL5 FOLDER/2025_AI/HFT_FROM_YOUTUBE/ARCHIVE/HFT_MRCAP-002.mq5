...
public:
   ...
   double CalculateTradeRiskAmount(double lotSize, double stopLossPips)
   {
      ...
      double riskAmount = (stopLossPips * _Point) / tickSize * tickValue * lotSize;
   ...
   bool IsTradeAllowed(double lotSize, double stopLossPips) // stopLossPips is in pips, not points
   {
      ...
      double potentialRiskAmount = CalculateTradeRiskAmount(lotSize, stopLossPips * PointScaleFactor()); // Convert pips to points for calculation if necessary or ensure consistency
   ...
   void RegisterTradeRisk(double lotSize, double stopLossPips)
   {
      ...
      double riskAmount = CalculateTradeRiskAmount(lotSize, stopLossPips * PointScaleFactor());
...
// Updated CalculateVolatility using ATR
double CalculateVolatility(int period) {
    if (period <= 0) {
        PrintFormat("CalculateVolatility: Invalid period %d. Returning 0.", period);
        return 0.0;
    }
    double atrValues[];
    // Request ATR data for the current symbol and period
    // Using current chart timeframe. Could be made an input if multi-timeframe analysis is desired here.
    if (CopyBuffer(iATR(_Symbol, Period(), period), 0, 0, 1, atrValues) <= 0) {
        PrintFormat("CalculateVolatility: Failed to copy ATR buffer for period %d. Error: %d", period, GetLastError());
        // Fallback: return average spread as a basic volatility measure if ATR fails
        return AverageSpread > 0 ? AverageSpread : SymbolInfoDouble(_Symbol, SYMBOL_POINT) * 10; // Default to 10 points if avg spread is 0
    }
    // atrValues[0] contains the most recent ATR value
    return atrValues[0]; 
}

// double CalculateTrendStrength() { /* Placeholder */ return 0.5; } // Example placeholder
// Updated CalculateTrendStrength using ADX
double CalculateTrendStrength(int period=14) { // Default period for ADX is often 14
    if (period <= 0) {
        PrintFormat("CalculateTrendStrength: Invalid period %d. Returning 0.", period);
        return 0.0;
    }
    double adxValues[];
    // Request ADX data for the current symbol and period
    // Using current chart timeframe.
    // The iADX function returns values for MAIN_LINE, PLUSDI_LINE, MINUSDI_LINE.
    // We are interested in the MAIN_LINE for trend strength.
    if (CopyBuffer(iADX(_Symbol, Period(), period), MAIN_LINE, 0, 1, adxValues) <= 0) {
        PrintFormat("CalculateTrendStrength: Failed to copy ADX buffer for period %d. Error: %d", period, GetLastError());
        return 0.0; // Return 0 to indicate no trend or error
    }
    // adxValues[0] contains the most recent ADX MAIN_LINE value.
    // ADX values typically range from 0 to 100.
    // To match the plan's expectation of trendStrength > 0.7 for trending regime,
    // we can normalize this value by dividing by 100.
    return adxValues[0] / 100.0;
}
...
void UpdateMarketAndTradeParameters()
{
   ...
   // Calculate order distances and other parameters
...
void OnTick()
{
   ...
   // Calculate PriceToPipRatio if not set (original logic)
   ...
   // Update market parameters (this replaces the original spread calculation with our enhanced version)
   UpdateMarketAndTradeParameters();
   ...
   for(int i = OrdersTotal()-1; i >= 0; i--)
   {
      if(!ordinfo.SelectByIndex(i))
         continue;
      if(ordinfo.Symbol() != _Symbol || ordinfo.Magic() != workingMagic)
         continue;
      ...
      if(type == ORDER_TYPE_BUY_STOP)
      {
         ...
         if(needsModification == true)
         {
            ...
            {
               if(!trade.OrderModify(ticket, modifiedPrice, modifiedSL, tp, 0, 0))
               {
                  if(CheckPointer(errorHandler) == POINTER_DYNAMIC) {
                     errorHandler.LogError(trade.ResultRetcode(), "OrderModify BUY_STOP failed for ticket #" + (string)ticket);
                  }
               }
      ...
      else if(type == ORDER_TYPE_SELL_STOP)
      {
         ...
         if(needsModification==true)
         {
               ...
               (Bid - openPrice) > MinFreezeDistance)
            {
               ...
               {
                  if(CheckPointer(errorHandler) == POINTER_DYNAMIC) {
                     errorHandler.LogError(trade.ResultRetcode(), "OrderModify SELL_STOP failed for ticket #" + (string)ticket);
                  }
               }
   ...
   // Process open positions (this is handled by ManageOpenPositions in OnTimer, but keep original logic here too)
   for(int i = PositionsTotal()-1; i >= 0; i--)
   {
      if(!posinfo.SelectByIndex(i))
         continue;
      if(posinfo.Symbol() != _Symbol || posinfo.Magic() != workingMagic)
         continue;
   ...
   if((OrderModificationFactor > 1 && TotalBuyCount < 1) || OpenBuyCount < 1)
   {
      if(PendingBuyCount < 1)
      {
         ...
         bool timeOK = (BrokerTime.hour >= workingStartHour && BrokerTime.hour <= workingEndHour);

         if(spreadOK && timeOK && (CurrentTime - LastOrderTime) > MinOrderInterval && EAModeFlag == 0)
         {
            // Lot size calculation - use enhanced version if available
            if(workingLotType == Fixed_Lots)
            {
               CalculatedLotSize = MathCeil(workingFixedLot / LotStepSize) * LotStepSize;
               CalculatedLotSize = MathMax(CalculatedLotSize, MinLotSize);
            }
            else if(workingLotType > Fixed_Lots)
            {
               // Use enhanced lot calculation if available, otherwise fallback to original
               CalculatedLotSize = CalculateOptimalLotSize(CalculatedStopLoss);
               if(CalculatedLotSize <= 0) // Fallback to original if enhanced version fails
            ...
            if(CheckPointer(riskManager) == POINTER_DYNAMIC) {
               double slPips = CalculatedStopLoss / riskManager.PointScaleFactor();
               if (!riskManager.IsTradeAllowed(CalculatedLotSize, slPips)) {
                  canTrade = false;
                  PrintFormat("Trade blocked by RiskManager for BUY. Lots: %.2f, SL Pips: %.1f", CalculatedLotSize, slPips);
               }
            }
            ...
            if(canTrade)
            {
               ...
               {
                  // Calculate order price and SL using enhanced methods if available
                  double orderDist = MathMax(MathMax(AdjustedOrderDistance, MinFreezeDistance), MinStopDistance);
                  double orderPrice = CalculateOptimalEntryPoint(ORDER_TYPE_BUY_STOP);
                  if(orderPrice <= Ask) // Fallback to original calculation if enhanced version fails
                     orderPrice = NormalizeDouble(Ask + orderDist, _Digits);
                  
                  double orderSL = (OpenBuyCount > 0) ? CurrentBuySL : NormalizeDouble(orderPrice - CalculatedStopLoss, _Digits);

                  if(SafeOrderSend(_Symbol, ORDER_TYPE_BUY_STOP, CalculatedLotSize, orderPrice, orderSL, NewOrderTakeProfit, OrderCommentText))
                  {
                     ...
                     if(CheckPointer(riskManager) == POINTER_DYNAMIC) {
                        double scaleFactor = riskManager.PointScaleFactor();
                        riskManager.RegisterTradeRisk(CalculatedLotSize, CalculatedStopLoss / scaleFactor);
                     }
      ...
      if(PendingSellCount < 1)
      {
         ...
         if(spreadOK && timeOK && (CurrentTime - LastOrderTime) > MinOrderInterval && EAModeFlag == 0)
         {
            ...
            if(CheckPointer(riskManager) == POINTER_DYNAMIC) {
               ...
               if (!riskManager.IsTradeAllowed(CalculatedLotSize, slPips)) {
                  canTrade = false;
                  PrintFormat("Trade blocked by RiskManager for SELL. Lots: %.2f, SL Pips: %.1f", CalculatedLotSize, slPips);
               }
            }
            ...
            if(canTrade)
            {
                  ...
                  AccountInfoDouble(ACCOUNT_MARGIN_FREE) > marginRequired)
               {
                  // Calculate order price and SL using enhanced methods if available
                  double orderDist = MathMax(MathMax(AdjustedOrderDistance, MinFreezeDistance), MinStopDistance);
                  double orderPrice = CalculateOptimalEntryPoint(ORDER_TYPE_SELL_STOP);
                  if(orderPrice >= Bid) // Fallback to original calculation if enhanced version fails
                     orderPrice = NormalizeDouble(Bid - orderDist, _Digits);
                  
                  double orderSL = (OpenSellCount > 0) ? CurrentSellSL : NormalizeDouble(orderPrice + CalculatedStopLoss, _Digits);

                  if(SafeOrderSend(_Symbol, ORDER_TYPE_SELL_STOP, CalculatedLotSize, orderPrice, orderSL, NewOrderTakeProfit, OrderCommentText))
                  {
...
MARKET_REGIME DetectMarketRegime() {
    ...
    if (shortTermVol > 1.5 * longTermVol && longTermVol > 0) return REGIME_VOLATILE; // ensure longTermVol is not zero
    if (longTermVol > 0 && shortTermVol < 0.5 * longTermVol) return REGIME_QUIET; // ensure longTermVol is not zero
...
double CalculateOptimalEntryPoint(ENUM_ORDER_TYPE orderType) {
    ...
    double optimalDistance = MathMax(recentVolatility * 0.5 * AverageSpread, MinStopDistance); // Scale by AverageSpread for points
...
void ManageExitStrategy(ulong ticket, double entryPrice, ENUM_POSITION_TYPE posType) {
    ...
    // Determine the relevant price for SL calculation based on position type
    double relevantPriceForSL = (posType == POSITION_TYPE_BUY) ? currentPriceBid : currentPriceAsk;
    
    double priceMovePoints = 0; // Price movement in points, including commission
    if (posType == POSITION_TYPE_BUY) {
        priceMovePoints = (currentPriceBid - entryPrice) - (CommissionPerPip /*already in price units*/);
    }
    else { // POSITION_TYPE_SELL
        priceMovePoints = (entryPrice - currentPriceAsk) - (CommissionPerPip /*already in price units*/);
    }

    double currentSL = PositionGetDouble(POSITION_SL);
    double currentTP = PositionGetDouble(POSITION_TP);

    // 1. Dynamic Trailing Stop using the existing CalculateTrailingStop function
    // Ensure priceMovePoints is non-negative for CalculateTrailingStop logic as it expects profit magnitude
    double positivePriceMovePoints = MathMax(0, priceMovePoints);
    
    // BaseTrailingStop comes from TrailingStopBuffer input (often 0)
    // TrailingStopActive is from MaxTrailing input, scaled by AvgSpread & regime/volatility
    // TrailingStopMax is from MaxTrailingLimit input, scaled by AvgSpread
    ...
    trailDistancePoints = MathMax(trailDistancePoints, MinStopDistance); // Ensure it's at least MinStopDistance

    double newSL = 0;
    if (posType == POSITION_TYPE_BUY) {
        newSL = NormalizeDouble(relevantPriceForSL - trailDistancePoints, _Digits);
        // Condition to modify SL: 
        // 1. New SL is profitable (above entry price) or at least breakeven.
        // 2. New SL is tighter than current SL (or current SL is not set).
        // 3. New SL is different from current SL.
        // 4. Distance from current price to new SL is at least MinStopDistance.
        if (newSL > entryPrice && (newSL > currentSL || currentSL == 0) && newSL != currentSL && (relevantPriceForSL - newSL) >= MinStopDistance) {
            if(!trade.PositionModify(ticket, newSL, currentTP)){
                if(CheckPointer(errorHandler)) errorHandler.LogError(trade.ResultRetcode(), "ManageExitStrategy: PositionModify BUY SL failed for #" + (string)ticket);
            }
            // currentSL = newSL; // Update currentSL for subsequent checks - not strictly needed here as we re-fetch if loop continues
        }
    } else { // POSITION_TYPE_SELL
        newSL = NormalizeDouble(relevantPriceForSL + trailDistancePoints, _Digits);
        // Conditions similar to BUY
        if (newSL < entryPrice && (newSL < currentSL || currentSL == 0) && newSL != currentSL && (newSL - relevantPriceForSL) >= MinStopDistance) {
            if(!trade.PositionModify(ticket, newSL, currentTP)){
                if(CheckPointer(errorHandler)) errorHandler.LogError(trade.ResultRetcode(), "ManageExitStrategy: PositionModify SELL SL failed for #" + (string)ticket);
            }
            // currentSL = newSL;
        }
    }
    ...
    // 3. Time-based exit (MaxHoldingTime should be an input or a defined constant)
...
bool SafeOrderSend(string symbol, ENUM_ORDER_TYPE orderType, double volume, double price, double sl, double tp, string comment)
{
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        if (trade.OrderOpen(symbol, orderType, volume, price, 0, sl, tp, ORDER_TIME_GTC, 0, comment)) {
