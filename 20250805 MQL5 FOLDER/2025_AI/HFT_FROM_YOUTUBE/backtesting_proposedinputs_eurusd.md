Okay, based on the **EURUSD specifications from DERIV broker** (5 digits, Stops level 20), here are "guesstimates" for your EA's inputs. These are *starting points* for your own optimization process, not definitive optimal values.

**Remember `_Point` for EURUSD will be `0.00001`. "Points" below refer to the smallest price increment (pips on a 5-digit).**

**GENERAL SETTINGS**
*   `InpMagic`: (Your choice, e.g., `12345`)
*   `Slippage`: `1` (Deriv is usually quite good, `1-3` is fine for STOP orders)
*   `InpAutoAdjustParametersForIndices`: `false` (EURUSD is not an index)

**TIME SETTINGS**
*   `StartHour`: (Your choice, e.g., `1` or `7` GMT for London open overlap)
...
*   `InpMinPriceMovementFactor`: `0.1` to `0.5` (e.g., `0.2` means price must move 0.2 points)
...
*   `InpUseOrderBookImbalance`: `false` (Deriv order book might not be deep/useful for this; test if true)
...
*   `InpTakeProfitFixedPoints`: `100` to `300` (10 to 30 pips)
...
*   `MaxSpread`: `20` to `50` (2.0 to 5.0 pips)

**BREAKEVEN SETTINGS**
*   `InpUseBreakeven`: `true`
*   `InpBreakevenProfitPoints`: `50` to `100` (5 to 10 pips)
*   `InpBreakevenPlusPoints`: `10` to `20` (1 to 2 pips)
...
*   `InpMB_MinVolatilityATR`: `0.00030` (3 pips. Must be in PRICE UNITS, e.g., for EURUSD where 1 pip = 0.00010)
*   `InpMB_MaxVolatilityATR`: `0.00150` (15 pips. Or `0.0` for no max)
...
*   `InpFS_SL_SpikeOffsetPips`: `30` to `50` (3 to 5 pips, value here is number of Points)
*   `InpFS_TP_TargetPips`: `50` to `100` (5 to 10 pips)
...
*   `InpEH_PanicSL_Pips`: `20` to `50` (2 to 5 pips, value here is points)
*   `InpEH_PauseNewEntriesSecs`: `60` to `300`

**REGIME PARAMETER SETS (`InpREG_...` These are MULTIPLIERS of AvgSpread for Delta/Stop/Trail if I recall the advanced logic, or direct values for MinOrderInterval):**
    *(Focus on consistency; if one regime is tighter, its numbers should be smaller relative to others)*
...
*   `InpLSD_MinLossPerLotThreshold`: `0.1` to `0.5` (USD; how much 1 lot at min SL must risk)
*   `InpLSD_MaxLotSizeCapFactorAccount`: `0.01` to `0.05` (e.g. max lot value is 1-5% of account equity)

**Most Important Next Step:**
After applying the `PriceToPipRatio` removal, **add detailed PrintFormat() statements inside each logical block of the "Micro-Breakout Entry" module (`CheckMicroBreakoutSignal`)** to see:
...
This will tell you precisely why no trades are being taken. Start by drastically increasing `InpMB_MaxVolatilityATR` or setting it to 0 for EURUSD and ensure `InpMB_MinVolatilityATR` is sensible (e.g., equivalent of 2-3 pips in price units).