...
const ENUM_TIMEFRAMES TIMEFRAME = PERIOD_M5; // Changed to 5-minute timeframe
...
void OnInit()
{
   ...
   vwap_handle = iCustom(SYMBOL, TIMEFRAME, "VWAP_UTC_Final"); // Make sure this indicator works correctly on M5
   ...
   trade.SetDeviationInPoints(10); // Allow 10 points slippage (adjust as needed for your broker/symbol)
   Print("EA initialized successfully on ", SYMBOL, " (M5)");  // Updated timeframe in print message
}
...
void CheckEntryConditions(double &ema9[], double &vwap[])
{
   ...
   {
      //--- Stop Loss is set at the low of the current candle - 3*point
      double sl = NormalizeDouble(vwap[1] - 3 * point, digits); // VWAP as SL
      if(trade.Buy(FIXED_LOT_SIZE, SYMBOL, 0.0, sl, 0.0))
      {
       ...
       {
           Print("Error opening Buy trade: ", GetLastError()); // Print the error code
       }
   }
   // Sell condition: EMA9 crosses below VWAP
   else if(ema9[2] > vwap[2] && ema9[1] < vwap[1])
   {
      //--- Stop Loss is set at the high of the current candle + 3*point
      double sl = NormalizeDouble(vwap[1] + 3 * point, digits); //VWAP as SL
      if(trade.Sell(FIXED_LOT_SIZE, SYMBOL, 0.0, sl, 0.0))
      {
        ...
        {
           Print("Error opening Sell trade: ", GetLastError()); // Print the error code
        }
   }
}
