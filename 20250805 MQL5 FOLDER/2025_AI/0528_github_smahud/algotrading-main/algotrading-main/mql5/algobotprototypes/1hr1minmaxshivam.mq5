...
input double EntryLotSize = 0.25; // Lot size per entry (two entries per signal)
input int NumberOfEntries = 2; // Number of entries per signal
...
ulong activePositionTickets[10]; // Array to store multiple ticket IDs
...
{
    // Initialize log file
    fileHandle = FileOpen("KC_4.7_H1_DualEntry.txt", FILE_WRITE|FILE_TXT);
    ...
    LogMessage("=== KC 4.7 H1 Dual Entry EA ===");
    LogMessage("Test started at: " + TimeToString(TimeCurrent()));
    LogMessage("Timeframe: H1");
    LogMessage("Entry Strategy: " + IntegerToString(NumberOfEntries) + " entries of " + DoubleToString(EntryLotSize, 2) + " lots each");
    LogMessage("Fixed Stop Loss: $" + DoubleToString(FixedStopLossUSD, 2) + " per position");
    ...
    else LogMessage("- Using fixed monetary Stop Loss of $" + DoubleToString(FixedStopLossUSD, 2) + " per position");
    ...
    LogMessage("KC 4.7 H1 Dual Entry EA initialized successfully");
...
void CheckForOpenPositions()
{
    ...
    for(int i = 0; i < 10; i++) activePositionTickets[i] = 0;
    ...
    for(int i = 0; i < PositionsTotal(); i++)
    {
        ...
        if(ticket > 0 && PositionGetString(POSITION_SYMBOL) == _Symbol)
        {
            ...
            {
                activePositionTickets[activePositionCount] = ticket;
                activePositionCount++;
            }
            
            // Store the open price of the first position
            if(activePositionOpenPrice == 0.0)
...
{
    if(!UseBreakEven || activePositionCount == 0 || breakEvenActivated)
        return false;
    ...
    // Get position direction from the first position
    if(activePositionCount == 0 || !PositionSelectByTicket(activePositionTickets[0]))
        return false;
    ...
    if(weakSignalDetected || fakeSignalDetected)
    {
        ...
        for(int i = 0; i < activePositionCount; i++)
        {
            if(!PositionSelectByTicket(activePositionTickets[i]))
                continue;
            ...
            request.sl = entryPrice; // Set SL to entry price
            request.tp = PositionGetDouble(POSITION_TP);
            request.position = activePositionTickets[i];
            
            bool success = OrderSend(request, result);
            
            if(!success || result.retcode != TRADE_RETCODE_DONE)
            {
                LogMessage("Failed to set break-even stop loss for position " + IntegerToString(i+1) + 
        ...
        if(allSuccess)
        {
            ...
            LogMessage("Break-even activated! " + (isLong ? "LONG" : "SHORT") + 
                      " positions stop loss moved to entry price " + 
...
{
    if(!UseBreakEven || activePositionCount == 0 || breakEvenActivated || safetyNetActivated)
        return false;
    ...
    // Get position direction from the first position
    if(!PositionSelectByTicket(activePositionTickets[0]))
        return false;
    ...
    if(inPlateau)
    {
        ...
        if(plateauCounter >= MinCandlesForPlateau)
        {
            ...
            // Apply safety net to all positions
            bool allSuccess = true;
            ...
            for(int i = 0; i < activePositionCount; i++)
            {
                ...
                double recoverySL = isLong ? 
                                  NormalizeDouble(entryPrice + RecoveryStopLossOffset, digits) : 
                                  NormalizeDouble(entryPrice - RecoveryStopLossOffset, digits);
                ...
                if(!success || result.retcode != TRADE_RETCODE_DONE)
                {
                    LogMessage("Failed to set safety net stop loss for position " + IntegerToString(i+1) + 
                              ". Error: " + IntegerToString(GetLastError()));
                    allSuccess = false;
                }
            }
            ...
            if(allSuccess)
            {
                // Update the stop loss level from the first position
                if(PositionSelectByTicket(activePositionTickets[0]))
                    currentStopLossLevel = PositionGetDouble(POSITION_SL);
                
                LogMessage("STRONG SIGNAL SAFETY NET ACTIVATED! " + (isLong ? "LONG" : "SHORT") + 
                          " positions stop loss moved to fixed " + DoubleToString(RecoveryStopLossOffset, 2) + 
                          " points profit - Max diff: " + DoubleToString(maxDifferential, 2) + 
...
bool IsStopLossHit()
{
    if(activePositionCount == 0) return false;
    
    // Check if any position has been closed by stop loss
    for(int i = 0; i < activePositionCount; i++)
    {
        if(activePositionTickets[i] == 0) continue;
        
        if(!PositionSelectByTicket(activePositionTickets[i]))
        {
            // Position was closed - check if it was by SL
            // Note: In MQL5 we can't directly determine if a position was closed by SL
            // We can only infer this from price action
            
            double currentBid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            double currentAsk = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            bool isLong = inLongPosition; // Use stored direction
    ...
    // Check active positions price vs SL
    double currentBid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    double currentAsk = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    bool isLong = inLongPosition;
...
//+------------------------------------------------------------------+
//| Execute Order with multiple entries |
//+------------------------------------------------------------------+
void ExecuteOrder(bool isBuy)
{
    ...
    for(int entryNum = 0; entryNum < NumberOfEntries; entryNum++)
    {
        MqlTradeRequest request = {};
        MqlTradeResult result = {};
        ...
        request.deviation = 10;
        request.comment = "ETH EMA9-VWAP " + (isBuy ? "Buy" : "Sell") + " Entry #" + IntegerToString(entryNum+1);
        ...
        while(!orderSent && retryCount < MaxTradeRetries)
        {
            ...
            if(sent && result.retcode == TRADE_RETCODE_DONE)
            {
                // Store the ticket in our array
                if(activePositionCount < 10)
                {
                    activePositionTickets[activePositionCount] = result.deal;
                    activePositionCount++;
                }
                
                // Store the open price (from the first entry)
                if(entryNum == 0)
                    activePositionOpenPrice = request.price;
                
                // Set position state flags
                ...
                // Reset break-even tracking (only on first entry)
                if(entryNum == 0)
                {
                    breakEvenActivated = false;
                    safetyNetActivated = false;
                ...
                string entryMsg = (isBuy ? "LONG" : "SHORT") + " Entry #" + IntegerToString(entryNum+1) + 
            ...
            else
            {
                ...
                if(retryCount < MaxTradeRetries)
                {
                    LogMessage("Retrying " + (isBuy ? "LONG" : "SHORT") + " entry #" + IntegerToString(entryNum+1) + 
                              " - Attempt " + IntegerToString(retryCount + 1) + 
                              " - Error: " + IntegerToString(result.retcode));
                    Sleep(500);
                }
                else
                {
                    LogMessage((isBuy ? "LONG" : "SHORT") + " Entry #" + IntegerToString(entryNum+1) + 
                              " failed after " + IntegerToString(MaxTradeRetries) + 
                              " attempts - Error: " + IntegerToString(result.retcode), "!!!");
                    failedTrades++;
                    
                    // If first entry fails, abort subsequent entries
                    if(entryNum == 0)
                        return;
                }
            }
...
void OnTick()
{
    ...
    if(IsStopLossHit())
    {
        ...
        LogMessage("Stop Loss triggered - closing all positions", "!!!");
...
void OnDeinit(const int reason)
{
    ...
    Print("KC 4.7 H1 Dual Entry EA deinitialized");
}