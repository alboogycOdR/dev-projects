...
//--- Buy stop
input bool     InpUseBuyStop           = true;        // Use Buy stop
input ushort   InpStopLossBuyStop      = 30;          // Stop Loss Buy stop, in pips (1.00045-1.00055=1 pips)
input ushort   InpTakeProfitBuyStop    = 300;         // Take Profit Buy stop, in pips (1.00045-1.00055=1 pips)
//--- Sell stop
input bool     InpUseSellStop          = true;        // Use Sell stop
input ushort   InpStopLossSellStop     = 30;          // Stop Loss Sell stop, in pips (1.00045-1.00055=1 pips)
input ushort   InpTakeProfitSellStop   = 300;         // Take Profit Sell stop, in pips (1.00045-1.00055=1 pips)
//---
input group             
"==========Time control=========="
...
input uchar    InpStartHour         = 13;          // Start Hour
input uchar    InpStartMinute       = 00;          // Start Minute
input uchar    InpEndHour           = 16;          // End Hour
input uchar    InpEndMinute         = 59;          // End Minute
...
void OnTick()
{
   ...
   //BOS ONLY
   if (openPriceUpBorder > 0
         && Bid >= openPriceUpBorder
...
//---
   ...
   if(free_margin_check>0.0)
     {
      if(obj_Trade.OrderOpen(m_symbol.Name(),order_type,check_lot,0.0,
         m_symbol.NormalizePrice(price),m_symbol.NormalizePrice(sl),m_symbol.NormalizePrice(tp),ORDER_TIME_GTC))
        {
...
void CalculateAllPendingOrders(int &count_buy_stops,int &count_buy_limits,int &count_sell_stops,int &count_sell_limits)
  {
   count_buy_stops   = 0;  count_buy_limits  = 0;
   count_sell_stops  = 0;  count_sell_limits = 0;
