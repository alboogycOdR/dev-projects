         ...
         if (hour >= ny_killzone_start_hour_broker && hour <= ny_killzone_end_hour_broker)
           {
            ...
            if (hour == ny_killzone_end_hour_broker && minute > ny_killzone_end_minute_minute) return false;
            return true;
           }
         return false;
      case SESSION_GLOBAL:
...
bool Filter_System::ApplySessionFilters()
  {
   ...
   if (InpEnableMondayFilter && TimeDayOfWeek(TimeCurrent()) == MONDAY)
     {
      Print("Monday filter active. No trades on Monday.");
      return false;
     }
   if (InpEnableFridayFilter && TimeDayOfWeek(TimeCurrent()) == FRIDAY)
     {
...
void Dashboard_Manager::DeleteDashboard()
  {
   ...
   ObjectsDeleteAll(0, OBJ_ALL, -1, -1, m_dashboard_name);
  }
...
int OnInit()
  {
   ...
   switch (InpOperationalMode)
     {
      case Operational_Modes::MODE_AUTO_TRADING:
         g_operational_modes.SetAutoTradingMode();
         break;
      case Operational_Modes::MODE_MANUAL_TRADING:
         g_operational_modes.SetManualTradingMode();
         break;
      case Operational_Modes::MODE_HYBRID:
...
//--- Hierarchical Trade Framing Protocol Execution
   ...
   int hour_gmt = TimeHour(current_time_gmt);
   int minute_gmt = TimeMinute(current_time_gmt);
   ...
   if (hour_gmt == ny_8am_et_close_hour_gmt && minute_gmt == ny_8am_et_close_minute_gmt)
     {
      ...
      if (TimeDay(current_time_gmt) != TimeDay(last_capture_time))
        {
   ...
   Operational_Modes::ENUM_OPERATIONAL_MODE current_mode = g_operational_modes.GetCurrentMode();

   if (current_mode == Operational_Modes::MODE_AUTO_TRADING || current_mode == Operational_Modes::MODE_HYBRID)
     {
   ...
   if (current_mode == Operational_Modes::MODE_MANUAL_TRADING || current_mode == Operational_Modes::MODE_HYBRID)
     {
...
//+------------------------------------------------------------------+
//| Dashboard_Manager Class (continued)                              |
//+------------------------------------------------------------------+
void Dashboard_Manager::CreateDashboard()
  {
   // This function will create graphical objects on the chart to form the dashboard.
   ...
   ObjectSetInteger(0, "ConfirmTradeButton", OBJPROP_VISIBLE, false);
  }
...
void Dashboard_Manager::UpdateDashboard(ENUM_CRT_PHASE current_crt_phase, double win_rate, double profit_loss, int trades_count, double risk_exposure)
  {
   ...
   if (g_operational_modes.GetCurrentMode() == Operational_Modes::MODE_HYBRID)
     {
      ObjectSetInteger(0, "ConfirmTradeButton", OBJPROP_VISIBLE, true);
     }
   else
     {
      ObjectSetInteger(0, "ConfirmTradeButton", OBJPROP_VISIBLE, false);
