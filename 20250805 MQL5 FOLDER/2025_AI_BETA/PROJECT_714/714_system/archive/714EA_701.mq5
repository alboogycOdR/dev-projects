//+------------------------------------------------------------------+
//|                                             SevenOneFourEA_V6.01.mq5
//|                                                Developed by [Your Name/Handle]
//|                                                Based on The 714 Trading Method by Mashaya
//+------------------------------------------------------------------+
#property version "7.01" // Updated version
#property description "2024.05.12" // Date of this version
...
v7.01
2025.05.13
   finalized integrate of smc indicator
   errors with parameters
   TODO: ondeinit; clean

v6.01
2025.05.12
- Added a new input parameter for the server's GMT offset.
- Added a new input parameter for the session end time.
- Added a new input parameter for the entry search end time.
- Added a new input parameter for the entry search end minute.
- Added a new input parameter for the entry search end hour.
- Added a new input parameter for the entry search end minute.
- Integrate LuxAlgo  SMC Indicator
*/
...
input double   ob_MinMovePips             = 7;    // Min price move (pips) to confirm OB
...
input color    morning_zone_color = clrCornflowerBlue;
//==
...
input color    afternoon_zone_color = clrDarkOrange;
...
//#define ADD_PARAM_INT(val) {ArrayResize(params, param_idx+1);params[param_idx].type=TYPE_INT;params[param_idx].integer_value=val;param_idx++;}
//#define ADD_PARAM_BOOL(val)   { ArrayResize(params, param_idx+1); params[param_idx].type=TYPE_BOOL;   params[param_idx].integer_value=val; param_idx++; }
//#define ADD_PARAM_DOUBLE(val) { ArrayResize(params, param_idx+1); params[param_idx].type=TYPE_DOUBLE; params[param_idx].double_value=val;  param_idx++; }
//#define ADD_PARAM_COLOR(val)  { ArrayResize(params, param_idx+1); params[param_idx].type=TYPE_INT; params[param_idx].integer_value=(val); param_idx++; }
...
void DrawVerticalLineAtLocalTimeAuto()
{
    ...
    MqlDateTime localTargetDateTime;
    localTargetDateTime.year = localDateTime.year;
    localTargetDateTime.mon = localDateTime.mon;
    localTargetDateTime.day = localDateTime.day;
    localTargetDateTime.hour = 13;
    localTargetDateTime.min = 0;
    localTargetDateTime.sec = 0;
    localTargetDateTime.day_of_week = 0;  // Not needed for StructToTime
    localTargetDateTime.day_of_year = 0;  // Not needed for StructToTime
    
    datetime localTargetTime = StructToTime(localTargetDateTime);
...
//--- Check Timeframe ---
   if(Period() != PERIOD_M5 && Period() != PERIOD_M15 ) {
      Print("ERROR: 714 Method EA requires the M5 or M15 timeframe. Please switch to M5 or M15.");
...
//--- Add Custom Indicator ---
   ...
   if(!indicator_exists) {
      ...
      int indicator_handle=iCustom(Symbol(),Period(),indicator_path,
                    ...
                    "OB",
                    true,5,true,5,0,clrLightBlue,clrLightPink,clrLightBlue, clrLightPink,
                    "EQH EQL",
                    true,3,0.1,
                    "FVG",
   ...
   if(visual_enabled && visual_obs_price_line && !g_key_price_obtained_today && current_completed_bar_time >= g_TodayKeyTime_Server) {
      ...
      if(key_time_actual_bar_idx >= 0 && key_time_actual_bar_idx < Bars(Symbol(), Period())) {
         ...
         if(g_KeyPrice_At_KeyTime > 0) {
            ...
            if(ObjectFind(0, obs_price_line_name) > 0)
               ObjectDelete(0, obs_price_line_name);
            ...
            Print("Warning: Failed to obtain valid Key Price...");
         }
         g_key_price_obtained_today = true;
      }
      else {
         Print("Warning: Key Time bar index not found (for key price) even though current time is after Key Time.");
      }
   }
//--- Obtain Morning Zone Key Price and Draw Line ---
   if(enable_morning_zone && visual_enabled && visual_obs_price_line && !g_MorningZone_KeyPrice_Obtained_Today && g_MorningZoneStartTime_Server > 0 && current_completed_bar_time >= g_MorningZoneStartTime_Server) {
      int key_time_idx = iBarShift(Symbol(), Period(), g_MorningZoneStartTime_Server, false);
      if(key_time_idx >= 0 && key_time_idx < Bars(Symbol(), Period())) 
      {
         g_MorningZone_KeyPrice = iOpen(Symbol(), Period(), key_time_idx);
         if(g_MorningZone_KeyPrice > 0) 
         {
            string line_name = "Trend_ObsPrice_Morning_" + Symbol() + "_" + EnumToString(Period()) + "_" + TimeToString(g_TodayKeyTime_Server, TIME_DATE);
            if(ObjectFind(0, line_name) > 0)
               ObjectDelete(0, line_name);
            if(ObjectCreate(0, line_name, OBJ_TREND, 0, g_MorningZoneStartTime_Server, g_MorningZone_KeyPrice, g_MorningZoneEndTime_Server, g_MorningZone_KeyPrice)) {
               ObjectSetInteger(0, line_name, OBJPROP_COLOR, morning_zone_color);
         ...
         g_MorningZone_KeyPrice_Obtained_Today = true;
      }
   }

//--- Obtain Afternoon Zone Key Price and Draw Line ---
   if(enable_afternoon_zone && visual_enabled && visual_obs_price_line && !g_AfternoonZone_KeyPrice_Obtained_Today && g_AfternoonZoneStartTime_Server > 0 && current_completed_bar_time >= g_AfternoonZoneStartTime_Server) {
      int key_time_idx = iBarShift(Symbol(), Period(), g_AfternoonZoneStartTime_Server, false);
      if(key_time_idx >= 0 && key_time_idx < Bars(Symbol(), Period())) {
         g_AfternoonZone_KeyPrice = iOpen(Symbol(), Period(), key_time_idx);
         if(g_AfternoonZone_KeyPrice > 0) {
            string line_name = "Trend_ObsPrice_Afternoon_" + Symbol() + "_" + EnumToString(Period()) + "_" + TimeToString(g_TodayKeyTime_Server, TIME_DATE);
            if(ObjectFind(0, line_name) > 0)
               ObjectDelete(0, line_name);
            if(ObjectCreate(0, line_name, OBJ_TREND, 0, g_AfternoonZoneStartTime_Server, g_AfternoonZone_KeyPrice, g_AfternoonZoneEndTime_Server, g_AfternoonZone_KeyPrice)) {
               ObjectSetInteger(0, line_name, OBJPROP_COLOR, afternoon_zone_color);
               ObjectSetInteger(0, line_name, OBJPROP_STYLE, STYLE_SOLID);
               ObjectSetInteger(0, line_name, OBJPROP_WIDTH, 1);
               ObjectSetInteger(0, line_name, OBJPROP_SELECTABLE, false);
               ObjectSetInteger(0, line_name, OBJPROP_BACK, true);
         ...
         g_AfternoonZone_KeyPrice_Obtained_Today = true;
      }
   }
...
//--- Step 4: Initial Trend Observation (Determine Bias) ---
   if(!g_bias_determined_today && current_completed_bar_time >= g_ObservationEndTime_Server) {
      ...
      if(key_bar_idx >= 0)
         Print("Time of key_bar_idx: ", TimeToString(iTime(Symbol(),Period(),key_bar_idx), TIME_DATE|TIME_SECONDS));
      ...
      if(obs_end_bar_idx >= 0)
         Print("Time of obs_end_bar_idx: ", TimeToString(iTime(Symbol(),Period(),obs_end_bar_idx), TIME_DATE|TIME_SECONDS));
...
// 1. Candidate candle (at index_in_array) must be bearish
   if(close_arr[index_in_array] >= open_arr[index_in_array])
      return false;
...
// Loop for ob_Lookback_Bars_For_Impulse *after* the candidate: from (index_in_array + 1)
   for(int i = 1; i <= ob_Lookback_Bars_For_Impulse; i++) {
      ...
      if(current_impulse_check_idx >= array_size)
         break;
...
//    should have a high greater than the candidate candle's high.
   ...
   for(int j = 0; j < ob_MaxBlockCandles; j++) {
      ...
      if(block_structure_check_idx >= array_size)
...
// So, the overall newest chart bar we need is the `scan_newest_candidate_chart_idx`
// minus `max_lookahead_into_newer_bars`.
   ...
   for(int chart_candidate_idx = scan_oldest_candidate_chart_idx; chart_candidate_idx >= scan_newest_candidate_chart_idx; chart_candidate_idx--) {
      ...
      if(is_bullish_ob && g_bullishOB_count < 100) {
         ...
         if(visual_enabled && visual_order_blocks) {
            ...
            if(ObjectFind(0, new_ob.objectName) >= 0)
               ObjectDelete(0, new_ob.objectName); // Use >=0 for ObjectFind
            if(ObjectCreate(0, new_ob.objectName, OBJ_RECTANGLE, 0, rect_time1, block_visual_high, rect_time2, block_visual_low)) {
               ...
               if(ObjectFind(0, new_ob.labelName) >=0)
                  ObjectDelete(0,new_ob.labelName);
               ...
               Print("Err Creating BullOB rect:", GetLastError(), " for OB at ", TimeToString(new_ob.startTime,TIME_DATE|TIME_SECONDS));
            }
         }
         g_bullishOB_count++;
      }
      if(is_bearish_ob && g_bearishOB_count < 100) {
         ...
         if(visual_enabled && visual_order_blocks) {
            ...
            datetime rect_time2 = new_ob.startTime + (datetime)((long)PeriodSeconds(Period()) * ob_MaxBlockCandles);
            if(ObjectFind(0, new_ob.objectName) >= 0)
               ObjectDelete(0, new_ob.objectName);
            if(ObjectCreate(0, new_ob.objectName, OBJ_RECTANGLE, 0, rect_time1, block_visual_high, rect_time2, block_visual_low)) {
               ...
               Print("Err Creating BearOB rect:", GetLastError(), " for OB at ", TimeToString(new_ob.startTime,TIME_DATE|TIME_SECONDS));
            }
         }
         g_bearishOB_count++;
      }
...
// Ensure target_server_hour is within 0-23
   ...
   if(target_server_hour < 0)
      target_server_hour += 24;
...
//--- Calculate Entry Search End Time (if enabled) ---
   if(use_entry_search_window) {
      if(server_midnight > 0) {
         ...
         if(target_end_server_hour < 0)
            target_end_server_hour += 24;
...
//--- Calculate Morning Zone Timings ---
   if(enable_morning_zone) 
   {
      int mz_start_gmt = morning_zone_start_hour_utc2 - 2;
      int mz_start_server = (mz_start_gmt + server_GMT_Offset_Manual) % 24;
      if(mz_start_server < 0)
         mz_start_server += 24;
      g_MorningZoneStartTime_Server = (datetime)(server_midnight + (long)mz_start_server * 3600L);

      int mz_end_gmt = morning_zone_end_hour_utc2 - 2;
      int mz_end_server = (mz_end_gmt + server_GMT_Offset_Manual) % 24;
      if(mz_end_server < 0)
         mz_end_server += 24;
      g_MorningZoneEndTime_Server = (datetime)(server_midnight + (long)mz_end_server * 3600L);
   }

//--- Calculate Afternoon Zone Timings ---
   if(enable_afternoon_zone) {
      int az_start_gmt = afternoon_zone_start_hour_utc2 - 2;
      int az_start_server = (az_start_gmt + server_GMT_Offset_Manual) % 24;
      if(az_start_server < 0)
         az_start_server += 24;
      g_AfternoonZoneStartTime_Server = (datetime)(server_midnight + (long)az_start_server * 3600L);

      int az_end_gmt = afternoon_zone_end_hour_utc2 - 2;
      int az_end_server = (az_end_gmt + server_GMT_Offset_Manual) % 24;
      if(az_end_server < 0)
         az_end_server += 24;
      g_AfternoonZoneEndTime_Server = (datetime)(server_midnight + (long)az_end_server * 3600L);
   }
...
//--- Draw Visual Lines ---
   if(visual_enabled) {
      ...
      if(visual_main_timing_lines) {
         ...
         if(ObjectFind(0, key_time_vline_name) > 0)
            ObjectDelete(0, key_time_vline_name);
         ...
         if(ObjectFind(0, obs_end_vline_name) > 0)
            ObjectDelete(0, obs_end_vline_name);
         ...
         // Draw Morning Zone VLines
         if(enable_morning_zone && g_MorningZoneStartTime_Server > 0) {
            string mz_start_name = "VLine_MorningStart_" + Symbol() + "_" + EnumToString(Period()) + "_" + TimeToString(server_midnight, TIME_DATE);
            if(ObjectFind(0, mz_start_name) > 0) ObjectDelete(0, mz_start_name);
            if(ObjectCreate(0, mz_start_name, OBJ_VLINE, 0, g_MorningZoneStartTime_Server, 0.0)) {
               ObjectSetInteger(0, mz_start_name, OBJPROP_COLOR, morning_zone_color);
               ObjectSetString(0, mz_start_name, OBJPROP_TEXT, "Morning Zone Start");
            }
            string mz_end_name = "VLine_MorningEnd_" + Symbol() + "_" + EnumToString(Period()) + "_" + TimeToString(server_midnight, TIME_DATE);
            if(ObjectFind(0, mz_end_name) > 0) ObjectDelete(0, mz_end_name);
            if(ObjectCreate(0, mz_end_name, OBJ_VLINE, 0, g_MorningZoneEndTime_Server, 0.0)) {
               ObjectSetInteger(0, mz_end_name, OBJPROP_COLOR, morning_zone_color);
               ObjectSetString(0, mz_end_name, OBJPROP_TEXT, "Morning Zone End");
            }
         }

         // Draw Afternoon Zone VLines
         if(enable_afternoon_zone && g_AfternoonZoneStartTime_Server > 0) {
            string az_start_name = "VLine_AfternoonStart_" + Symbol() + "_" + EnumToString(Period()) + "_" + TimeToString(server_midnight, TIME_DATE);
            if(ObjectFind(0, az_start_name) > 0) ObjectDelete(0, az_start_name);
            if(ObjectCreate(0, az_start_name, OBJ_VLINE, 0, g_AfternoonZoneStartTime_Server, 0.0)) {
               ObjectSetInteger(0, az_start_name, OBJPROP_COLOR, afternoon_zone_color);
               ObjectSetString(0, az_start_name, OBJPROP_TEXT, "Afternoon Zone Start");
            }
            string az_end_name = "VLine_AfternoonEnd_" + Symbol() + "_" + EnumToString(Period()) + "_" + TimeToString(server_midnight, TIME_DATE);
            if(ObjectFind(0, az_end_name) > 0) ObjectDelete(0, az_end_name);
            if(ObjectCreate(0, az_end_name, OBJ_VLINE, 0, g_AfternoonZoneEndTime_Server, 0.0)) {
               ObjectSetInteger(0, az_end_name, OBJPROP_COLOR, afternoon_zone_color);
               ObjectSetString(0, az_end_name, OBJPROP_TEXT, "Afternoon Zone End");
            }
         }
...
// --- Iterate backwards through chart objects to safely remove ---
   for(int i = ObjectsTotal(chart_id) - 1; i >= 0; i--) {
      ...
      if(StringFind(obj_name, "VLine_KeyTime_", 0) == 0 ||
            ...
            StringFind(obj_name, "VLine_MorningStart_", 0) == 0 ||
            StringFind(obj_name, "VLine_MorningEnd_", 0) == 0 ||
            StringFind(obj_name, "VLine_AfternoonStart_", 0) == 0 ||
            StringFind(obj_name, "VLine_AfternoonEnd_", 0) == 0 ||
...
// Ensure hour is within 0-23
   ...
   if(session_end_server_hour < 0)
      session_end_server_hour += 24;
...
{
   if(g_InitialBias != -1)
      return false; // Safety check, should be -1 if this is called
