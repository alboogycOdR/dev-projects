...
input bool     visual_key_price_line     = false;  // Enable/Disable drawing a horizontal line at the Key Time price - DEPRECATED by ObservationPriceLine
...
int OnInit()
{
    ...
    Print("714 Method EA V4 Core initialized successfully on ", TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES));
...
void OnDeinit(const int reason)
{
    ...
    Print("714 Method EA V4 Core deinitialized. Reason: ", reason);
}
         ...
         if (key_bar_idx >= 0 && obs_end_bar_idx >= 0 && obs_end_bar_idx < total_bars && obs_end_bar_idx >= key_bar_idx)
         {
             ...
             // --- After bias is determined, perform OB detection scan if not already done for today ---
              if (!g_order_blocks_scanned_today)
              {
                   ...
                   // Call the detection function with a relevant scan range (indices of bars to scan for potential OB candles)
                   // A reasonable scan range is from a few bars BEFORE the Key Time up to a few bars AFTER the Observation End Time
                   int scan_start_period_idx = iBarShift(Symbol(), Period(), g_TodayKeyTime_Server - PeriodSeconds(Period()) * ob_MaxBlockCandles, false); // Start scanning from here
                   int scan_end_period_idx = iBarShift(Symbol(), Period(), g_ObservationEndTime_Server + PeriodSeconds(Period()) * ob_Lookback_Bars_For_Impulse, false); // Scan up to here

                   // Ensure indices are valid and in correct order (older bar index > newer bar index)
                   if(scan_start_period_idx < 0 || scan_end_period_idx < 0 || scan_start_period_idx >= total_bars || scan_end_period_idx >= total_bars || scan_end_period_idx >= scan_start_period_idx) // Added scan_end >= scan_start check
                   {
                       Print("Warning: Could not establish a valid scanning period for Order Block detection based on key times.");
                       // We might still allow trade checks if bias determined, but no OB confluence will be found
                   } else
                   {
                       // Perform the detection scan within this time range
                       DetectOrderBlocksForToday(scan_start_period_idx, scan_end_period_idx); // Now indices are chart indices
                   }
              }
         }
         else
...
// Check if a candle at 'index' (in array, relative to array start) is a Bullish Order Block candidate
...
bool IsBullishOrderBlockCandidate(int index_in_array, const double &open_arr[], const double &high_arr[], const double &low_arr[], const double &close_arr[], int array_size)
{
    ...
    if (index_in_array + 1 + 2 > array_size) return false; // index_in_array + 1 is first candle after OB
    ...
    // Scan *after* the potential OB candle (index_in_array + 1, index_in_array + 2, ...) for the bullish move
    // The scan range after the OB relates to ob_Lookback_Bars_For_Impulse input.
    for (int i = index_in_array + 1; i < index_in_array + 1 + ob_Lookback_Bars_For_Impulse && i < array_size; i++)
    {
         if (close_arr[i] > open_arr[i]) // Found a bullish candle
         {
              ...
              if ((cumulative_move_after_ob / GetPipValue()) >= ob_MinMovePips && bullish_candles_count >= 2) // Added parentheses
              {
                   found_strong_move = true; // Criteria for strong move met
                   break; // Stop scanning
              }
         }
         // If a bearish or doji candle breaks the sequence before move criteria met, stop checking this potential OB's impulse.
         else
         {
              break;
         }
    ...
    // Loop from the OB candle's index (index_in_array) up to index + ob_MaxBlockCandles - 1
    for(int j = index_in_array; j < index_in_array + ob_MaxBlockCandles && j < array_size; j++)
    {
        ...
        if (j > index_in_array && high_arr[j] > high_arr[index_in_array]) // Corrected to use high_arr
        {
...
bool IsBearishOrderBlockCandidate(int index_in_array, const double &open_arr[], const double &high_arr[], const double &low_arr[], const double &close_arr[], int array_size)
{
    ...
    // Scan *after* the potential OB candle (index_in_array + 1, index_in_array + 2, ...) for the bearish move
     for (int i = index_in_array + 1; i < index_in_array + 1 + ob_Lookback_Bars_For_Impulse && i < array_size; i++)
    {
         if (close_arr[i] < open_arr[i]) // Found a bearish candle
         {
              ...
              if ((cumulative_move_after_ob / GetPipValue()) >= ob_MinMovePips && bearish_candles_count >= 2) // Added parentheses
              {
                   found_strong_move = true; // Criteria for strong move met
                   break; // Stop scanning
              }
         }
         // If a bullish or doji candle breaks the sequence before move criteria met, stop checking
         else
         {
              break;
         }
    ...
    bool no_lower_low_in_block_period = true;
     ...
     for(int j = index_in_array; j < index_in_array + ob_MaxBlockCandles && j < array_size; j++)
     {
         ...
         if (j > index_in_array && low_arr[j] < low_arr[index_in_array]) // Corrected to use low_arr
         {
...
//+------------------------------------------------------------------+
// scan_from_index and scan_to_index are CHART bar indices
void DetectOrderBlocksForToday(int scan_from_chart_index, int scan_to_chart_index)
{
     if (g_order_blocks_scanned_today) return; // Only run detection scan once per day

     int total_bars = Bars(Symbol(), Period());

     // Ensure valid scan range (scan_from is the older index, scan_to is the newer index)
      if (scan_from_chart_index < 0 || scan_to_chart_index < 0 || scan_from_chart_index >= total_bars || scan_to_chart_index >= total_bars || scan_from_chart_index <= scan_to_chart_index)
     {
          Print(visual_comment_text + " - DetectOrderBlocksForToday Error: Invalid chart index scan range (from: ", scan_from_chart_index, ", to: ", scan_to_chart_index, ", total bars: ", total_bars, ").");
           g_order_blocks_scanned_today = true; // Avoid trying again
          return;
     }

     // Determine the number of bars we need to copy data for. This range covers the scan area PLUS the bars needed *ahead* for the impulse/block checks
     // Copy data from the oldest bar in the scan range (`scan_to_chart_index`) back for `ob_Lookback_Bars_For_Impulse + ob_MaxBlockCandles`
     // The array index 0 will correspond to chart index `copy_start_chart_index`.
     int copy_start_chart_index = MathMax(0, scan_to_chart_index - (ob_Lookback_Bars_For_Impulse + ob_MaxBlockCandles)); // Calculate the oldest index we need data for
     int num_bars_to_copy = scan_from_chart_index - copy_start_chart_index + 1; // Number of bars from copy_start to scan_from inclusive

      if (num_bars_to_copy <= (ob_Lookback_Bars_For_Impulse + ob_MaxBlockCandles) ) // Check if enough bars copied for at least one full OB check starting early in scan range
     {
          Print(visual_comment_text + " - Not enough copied bars (", num_bars_to_copy, ") to perform OB detection scan in the relevant period.");
         ...
         CopyClose(Symbol(), Period(), copy_start_chart_index, num_bars_to_copy, close_arr) < 0)
     {
      ...
      // Reset daily OB counts (done in CalculateAndDrawDailyTimings for the new day)

     Print(visual_comment_text + " - Scanning chart indices from ", scan_from_chart_index, " back to ", scan_to_chart_index, " for Order Blocks in Stage 2 period (around Key Time). Using array data from chart index ", copy_start_chart_index);


     // Iterate through the relevant M5 bars within the specified scan range (using chart indices).
     // The scan is for *candidate OB candles*. For each candidate at chart index 'i', the OB logic
     // needs to look *ahead* in the copied array to find the impulse move/block period.
     // The corresponding array index for chart index 'i' is 'i - copy_start_chart_index'.

     for (int i = scan_from_chart_index; i >= scan_to_chart_index; i--)
     {
          int arr_idx = i - copy_start_chart_index; // Get the array index for the current chart index 'i'

          // Ensure we have enough bars *ahead* in the array from this candidate bar 'arr_idx' to perform the impulse/block period checks
          // (impulse check looks up to arr_idx + 1 + ob_Lookback_Bars_For_Impulse - 1 ahead, block check up to arr_idx + ob_MaxBlockCandles - 1 ahead)
           int furthest_lookahead_needed = MathMax(ob_Lookback_Bars_For_Impulse + 1, ob_MaxBlockCandles) -1; // How many bars are checked AFTER the OB candidate (arr_idx)
          if (arr_idx + furthest_lookahead_needed >= num_bars_to_copy)
          {
              // Not enough future bars in the copied array from this point to complete OB check
              // Print("Warning: Skipping OB candidate at chart index ", i, " (array index ", arr_idx, "). Not enough future bars in copied array."); // Verbose skipping
              continue;
          }


           // --- Check if this candle at chart index 'i' is a Bullish or Bearish Order Block candidate ---
          ...
          if (is_bullish_ob)
          {
               ...
               {
                    // Store details using data corresponding to chart index 'i'
                    g_bullishOrderBlocks[g_bullishOB_count].startTime = iTime(Symbol(), Period(), i);
                    g_bullishOrderBlocks[g_bullishOB_count].high = iHigh(Symbol(), Period(), i);
                    g_bullishOrderBlocks[g_bullishOB_count].low = iLow(Symbol(), Period(), i);
                    g_bullishOrderBlocks[g_bullishOB_count].type = POSITION_TYPE_BUY;
                    g_bullishOrderBlocks[g_bullishOB_count].isMitigated = false;
                    g_bullishOrderBlocks[g_bullishOB_count].objectName = "BullOB_" + TimeToString(g_bullishOrderBlocks[g_bullishOB_count].startTime, TIME_DATE|TIME_MINUTES|TIME_SECONDS) + "_" + IntegerToString(i); // Even more unique
                    ...
                    // Print(visual_comment_text + " - Detected BULLISH Order Block at Server Time: ", TimeToString(g_bullishOrderBlocks[g_bullishOB_count].startTime, TIME_DATE|TIME_MINUTES), " (Chart Index ", i, ")",
                    ...
                    if(visual_enabled && visual_order_blocks)
                    {
                        ...
                        } else { Print(visual_comment_text + " - Error creating Bull OB visual object at chart index ", i, ": ", GetLastError());}
                    }
               ...
               {
                  // Print(visual_comment_text + " - Warning: Maximum daily Bullish Order Blocks storage capacity reached. Skipping detection from chart index ", i); // Verbose skipping
               }
          ...
          if (is_bearish_ob)
          {
              if (g_bearishOB_count < 100)
               {
                    g_bearishOrderBlocks[g_bearishOB_count].startTime = iTime(Symbol(), Period(), i);
                    g_bearishOrderBlocks[g_bearishOB_count].high = iHigh(Symbol(), Period(), i);
                    g_bearishOrderBlocks[g_bearishOB_count].low = iLow(Symbol(), Period(), i);
                    g_bearishOrderBlocks[g_bearishOB_count].type = POSITION_TYPE_SELL;
                    g_bearishOrderBlocks[g_bearishOB_count].isMitigated = false;
                    g_bearishOrderBlocks[g_bearishOB_count].objectName = "BearOB_" + TimeToString(g_bearishOrderBlocks[g_bearishOB_count].startTime, TIME_DATE|TIME_MINUTES|TIME_SECONDS) + "_" + IntegerToString(i); // Even more unique
                    ...
                    // Print(visual_comment_text + " - Detected BEARISH Order Block at Server Time: ", TimeToString(g_bearishOrderBlocks[g_bearishOB_count].startTime, TIME_DATE|TIME_MINUTES), " (Chart Index ", i, ")",
                    ...
                    if(visual_enabled && visual_order_blocks)
                    {
                        ...
                        } else { Print("Error creating Bear OB visual object at chart index ", i, ": ", GetLastError());}
                    }
               ...
               {
                  // Print(visual_comment_text + " - Warning: Maximum daily Bearish Order Blocks storage capacity reached. Skipping detection from chart index ", i); // Verbose skipping
               }
...
void CalculateAndDrawDailyTimings()
{
    ...
    if (key_bar_idx_for_count >= 0) // Ensure the starting bar for the count exists
    {
        ...
        if(target_entry_bar_idx >= 0 && target_entry_bar_idx < Bars(Symbol(), Period()))
        {
         ...
         {
              Print(visual_comment_text + " - Warning: Not enough historical bars to determine the 15th candlestick time based on today's key time (idx: ", target_entry_bar_idx, ", bars: ", Bars(Symbol(),Period()), ").");
    ...
    if (visual_enabled)
    {
        ...
        if (visual_obs_price_line && g_KeyPrice_At_KeyTime > 0 && g_TodayKeyTime_Server > 0 && g_ObservationEndTime_Server > 0 && g_TodayKeyTime_Server < g_ObservationEndTime_Server) // Ensure valid times and price
        {
             ...
             if(ObjectCreate(0, obs_price_line_name, OBJ_TREND, 0, g_TodayKeyTime_Server, g_KeyPrice_At_KeyTime, g_ObservationEndTime_Server, g_KeyPrice_At_KeyTime))
             {
                 ...
                 Print("Error creating Observation Price Trend line: ", GetLastError());
             }
        }
    ...
    g_bearishOB_count = 0;
     ...
     {
           // Define scan range again (copy logic from OnTimer for this initial check)
            int total_bars = Bars(Symbol(), Period());
            if(total_bars > 1) // Ensure there's enough data
            {
                 int key_bar_idx = iBarShift(Symbol(), Period(), g_TodayKeyTime_Server, false);
                 int obs_end_bar_idx = iBarShift(Symbol(), Period(), g_ObservationEndTime_Server, false);
                  ...
                  if (key_bar_idx >= 0 && obs_end_bar_idx >= 0 && obs_end_bar_idx < total_bars && obs_end_bar_idx >= key_bar_idx)
                  {
                        int scan_start_period_idx = iBarShift(Symbol(), Period(), g_TodayKeyTime_Server - PeriodSeconds(Period()) * ob_MaxBlockCandles, false); // Start scanning from here
                        int scan_end_period_idx = iBarShift(Symbol(), Period(), g_ObservationEndTime_Server + PeriodSeconds(Period()) * ob_Lookback_Bars_For_Impulse, false); // Scan up to here
                        if(scan_start_period_idx < 0) scan_start_period_idx = 0; // Prevent negative index
                         if(scan_end_period_idx >= total_bars) scan_end_period_idx = total_bars -1; // Prevent out of bounds
                       if(scan_start_period_idx < scan_end_period_idx) // Ensure valid range start < end index (time goes from left to right)
                        {
                             DetectOrderBlocksForToday(scan_end_period_idx, scan_start_period_idx); // Note: DetectOrderBlocksForToday expects newest_index, oldest_index
                        }
                        else if(scan_start_period_idx > scan_end_period_idx) // Normal time flow, oldest index is > newest
                        {
                            DetectOrderBlocksForToday(scan_start_period_idx, scan_end_period_idx); // Newest_index, oldest_index (Oops, swap order based on function logic)
                            DetectOrderBlocksForToday(scan_end_period_idx, scan_start_period_idx); // Newer bar index, older bar index -> pass indices to func correctly
                        }
                        // Let's use the correct order for the detection function scan parameters: newest index first, oldest index second
                        // Example range: from index 100 back to 50. newest = 100, oldest = 50.
                         if (scan_end_period_idx > scan_start_period_idx) // Check the calculated period indices in terms of time order
                         {
                             DetectOrderBlocksForToday(scan_end_period_idx, scan_start_period_idx); // Newer index first, older index second
                         }
                         else
                         {
                             DetectOrderBlocksForToday(scan_start_period_idx, scan_end_period_idx); // Assume default if logic above is complex, pass as is
                         }

                  } else {
                      // No valid period to scan. Bias was likely manually set or logic path during historical data load.
                  }
...
void RemoveDailyVisualObjects()
{
     ...
     //string name_prefix = visual_comment_text + ":"; // Standard prefix for our comments - Not used for object names directly
     ...
     for (int i = ObjectsTotal(chart_id) - 1; i >= 0; i--)
     {
         ...
         // Remove our OB rectangles and labels (using substring search based on objectName structure in st_OrderBlock prefixes and _Label suffix)
          if (StringFind(obj_name, "BullOB_", 0) == 0) // Starts with BullOB_ (covers rects)
         {
             ObjectDelete(chart_id, obj_name);
         }
          if (StringFind(obj_name, "BearOB_", 0) == 0) // Starts with BearOB_ (covers rects)
         {
             ObjectDelete(chart_id, obj_name);
         }
          if (StringFind(obj_name, "_Label", StringLen(obj_name) - StringLen("_Label")) > 0 && (StringFind(obj_name, "BullOB_", 0) == 0 || StringFind(obj_name, "BearOB_", 0) == 0)) // Is a label AND its rectangle prefix matches OBs
         {
              ObjectDelete(chart_id, obj_name);
         }
...
void UpdateMitigationStatus(int current_closed_bar_index)
{
     ...
     {
         // Ensure the OB hasn't been mitigated already AND it's not too far in the past (optional cleanup)
         if (!g_bullishOrderBlocks[i].isMitigated ) // && (TimeCurrent() - g_bullishOrderBlocks[i].startTime < 24 * 3600 * 5) ) // Example: Remove OBs older than 5 days
         {
              // Check if the *previous closed bar's* Low went below the Bullish OB's Low
              // Check against index 1 (previous completed bar)
              // Use _Point * 2 as a small buffer below OB low for clean penetration check
              if (iLow(Symbol(), Period(), 1) < g_bullishOrderBlocks[i].low - _Point * 2)
              {
                   ...
                   if(visual_enabled && visual_order_blocks && ObjectFind(0, g_bullishOrderBlocks[i].objectName) > 0)
                  {
                      ...
                      // Update label text and color if exists
     ...
     for (int i = 0; i < g_bearishOB_count; i++)
     {
          ...
          if (!g_bearishOrderBlocks[i].isMitigated) // && (TimeCurrent() - g_bearishOrderBlocks[i].startTime < 24 * 3600 * 5) ) // Optional: cleanup old mitigated OBs from storage
          {
              // Check if the *previous closed bar's* High went above the Bearish OB's High
               ...
               if (iHigh(Symbol(), Period(), 1) > g_bearishOrderBlocks[i].high + _Point * 2)
              {
     ...
     // Optional: Remove objects or clear from array storage that are very old to keep resource usage low, even if mitigated
     // For instance, after mitigation, you could schedule removal or skip iterating very old entries.
}
