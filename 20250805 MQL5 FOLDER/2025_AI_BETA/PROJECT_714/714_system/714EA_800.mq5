...
version 8.00
- added anchor price alert triggers
- added anchor price interaction
- added anchor price interaction
- added on-chart alerts
...
//--- Struct to hold details of a triggered event
struct st_TriggerDetails
  {
   ...
   color             alert_color; // Color for the visual alert
   uint              symbol_code; // Symbol code for the visual alert
  };
...
//--- Forward Declarations for functions used before their main definition
    ...
    bool           CheckAnchorPriceInteraction_v8(int bar_idx, st_TriggerDetails &details);
    bool           CheckOBInteraction_v8(int bar_idx, st_TriggerDetails &details);
    bool           CheckFVGInteraction_v8(int bar_idx, st_TriggerDetails &details); // Placeholder
   ...
   if(Operating_Mode == ANALYSIS_ALERTS && g_bias_determined_today)
     {
      ...
      CheckForConfluenceAlert(current_bar_idx);
     }
   ...
   if(price_at_obs_end > price_at_keytime)
     {
      ...
      double obs_end_high = iHigh(_Symbol, _Period, obs_end_bar_idx);
      DrawAlertSymbol(obs_end_bar_idx, obs_end_high + (10 * _Point), "Bias for Sells", clrRed, 242);
     }
   else
      if(price_at_obs_end < price_at_keytime)
        {
         ...
         double obs_end_low = iLow(_Symbol, _Period, obs_end_bar_idx);
         DrawAlertSymbol(obs_end_bar_idx, obs_end_low - (10 * _Point), "Bias for Buys", clrDodgerBlue, 241);
        }
      else
        {
...
bool CheckAnchorPriceInteraction_v8(int bar_idx,st_TriggerDetails &details)
  {
   if(g_KeyPrice_At_KeyTime <= 0)
      return false; // Can't check if price wasn't set

// Get current closed bar's data
   ...
   if(alert_on_close_across_anchor && !g_close_across_alert_sent)
     {
      if(prev_close < g_KeyPrice_At_KeyTime && close > g_KeyPrice_At_KeyTime)
        {
         g_anchor_price_broken = true;
         g_close_across_alert_sent = true;
         details.description = "Close Above Anchor";
         details.price_level = close;
         details.alert_color = clrDodgerBlue;
         details.symbol_code = 241;
         return true;
        }
      if(prev_close > g_KeyPrice_At_KeyTime && close < g_KeyPrice_At_KeyTime)
        {
         g_anchor_price_broken = true;
         g_close_across_alert_sent = true;
         details.description = "Close Below Anchor";
         details.price_level = close;
         details.alert_color = clrMediumVioletRed;
         details.symbol_code = 242;
         return true;
        }
     }
   ...
   if(alert_on_rejection_of_anchor)
     {
      ...
      double pip_value = _Point * (_Digits % 2 != 0 ? 10 : 1);

      if(total_size >= min_candle_size_pips * pip_value)
        {
         bool atr_filter_passed = !use_atr_filter;
         if(use_atr_filter && g_atr_handle != INVALID_HANDLE)
           {
            double atr_buffer[1];
            if(CopyBuffer(g_atr_handle,0,1,1,atr_buffer) > 0 && total_size >= atr_buffer[0])
         ...
         if(atr_filter_passed)
           {
            // Bullish Rejection
            if(low < g_KeyPrice_At_KeyTime && close > g_KeyPrice_At_KeyTime)
              {
               ...
               if(body_size > 0 && lower_wick / body_size >= min_wick_to_body_ratio)
                 {
                  ...
                  if(TimeCurrent() > last_alert + 300)
                    {
                     details.description = "High-Quality Bullish Rejection";
                     details.price_level = low;
                     details.alert_color = clrLimeGreen;
                     details.symbol_code = 241;
            ...
            // Bearish Rejection
            if(high > g_KeyPrice_At_KeyTime && close < g_KeyPrice_At_KeyTime)
              {
               ...
               if(body_size > 0 && upper_wick / body_size >= min_wick_to_body_ratio)
                 {
                  ...
                  if(TimeCurrent() > last_alert + 300)
                    {
                     details.description = "High-Quality Bearish Rejection";
                     details.price_level = high;
                     details.alert_color = clrRed;
                     details.symbol_code = 242;
                     last_alert = TimeCurrent();
                     return true;
                    }
...
// --- Alert Trigger 3: Price Breaks and then Retests the Anchor Price ---
   if(alert_on_break_and_retest_of_anchor && g_anchor_price_broken && !g_retest_alert_sent)
     {
      if(low <= g_KeyPrice_At_KeyTime && high >= g_KeyPrice_At_KeyTime)
        {
         g_retest_alert_sent = true;
         details.description = "Break & Retest";
         details.price_level = g_KeyPrice_At_KeyTime;
         details.alert_color = clrGold;
         details.symbol_code = 159;
         return true;
        }
     }
   return false;
...
//+------------------------------------------------------------------+
//| Checks for interaction with a detected Order Block               |
//+------------------------------------------------------------------+
bool CheckOBInteraction_v8(int bar_idx,st_TriggerDetails &details)
  {
   ...
   for(int i = 0; i < g_bullishOB_count; i++)
     {
      if(!g_bullishOrderBlocks[i].isMitigated)
        {
         // Check if the current bar's low has touched or gone into the Bullish OB range
         if(low <= g_bullishOrderBlocks[i].high && high >= g_bullishOrderBlocks[i].low)
           {
            details.description = "Bullish OB Interaction";
            details.price_level = g_bullishOrderBlocks[i].high; // Top of the OB
            details.alert_color = clrTeal;
            details.symbol_code = 110;
            return true;
           }
        }
     }
   ...
   for(int i = 0; i < g_bearishOB_count; i++)
     {
      if(!g_bearishOrderBlocks[i].isMitigated)
        {
         // Check if the current bar's high has touched or gone into the Bearish OB range
         if(high >= g_bearishOrderBlocks[i].low && low <= g_bearishOrderBlocks[i].high)
           {
            details.description = "Bearish OB Interaction";
            details.price_level = g_bearishOrderBlocks[i].low; // Bottom of the OB
            details.alert_color = clrMagenta;
            details.symbol_code = 110;
            return true;
           }
        }
...
//+------------------------------------------------------------------+
//| Placeholder for FVG Interaction Checks                           |
//+------------------------------------------------------------------+
bool CheckFVGInteraction_v8(int bar_idx,st_TriggerDetails &details)
  {
// Future implementation will go here. For now, it does nothing.
   return false;
  }
...
// Check for Anchor Price Interaction
   ...
   bool found_anchor_interaction = CheckAnchorPriceInteraction_v8(bar_idx,anchor_trigger);

// Check for OB Interaction
   st_TriggerDetails ob_trigger;
   bool found_ob_interaction = CheckOBInteraction_v8(bar_idx,ob_trigger);
      ...
      if(found_anchor_interaction)
        {
         ...
         Print(full_desc);
         DrawAlertSymbol(bar_idx,anchor_trigger.price_level,anchor_trigger.description,anchor_trigger.alert_color,anchor_trigger.symbol_code);
         string screenshot_path = TakeScreenshot();
         if(screenshot_path != "")
            SendTelegramAlert(full_desc,screenshot_path);
        }
      ...
      if(found_ob_interaction)
        {
         ...
         Print(full_desc);
         DrawAlertSymbol(bar_idx,ob_trigger.price_level,ob_trigger.description,ob_trigger.alert_color,ob_trigger.symbol_code);
         string screenshot_path = TakeScreenshot();
         if(screenshot_path != "")
            SendTelegramAlert(full_desc,screenshot_path);
        }
...
// Create a unique name for the alert object using its time and description type
   ...
   StringReplace(safe_description, " ", "_"); // Make name safe for objects
   string obj_name = "Alert_" + safe_description + "_" + TimeToString(alert_time, TIME_DATE|TIME_MINUTES|TIME_SECONDS);
   ...
   if(ObjectCreate(0, obj_name, OBJ_ARROW, 0, alert_time, price))
     {
      ...
      string label_name = "AlertLabel_" + safe_description + "_" + TimeToString(alert_time, TIME_DATE|TIME_MINUTES|TIME_SECONDS);
      
      double price_offset;
      ENUM_ANCHOR_POINT anchor;

      // If it's a bullish arrow (pointing up), place the label below it.
      if (symbol_code == 241) // Wingdings Up Arrow
        {
         price_offset = -15 * _Point; // Place it slightly below the arrow's price point
         anchor = ANCHOR_TOP;         // Anchor the top of the text to the calculated label_price
        }
      else // For down arrows or other symbols, place label above
        {
         price_offset = 15 * _Point;  // Place it slightly above
         anchor = ANCHOR_BOTTOM;      // Anchor the bottom of the text
        }
      
      double label_price = price + price_offset;
