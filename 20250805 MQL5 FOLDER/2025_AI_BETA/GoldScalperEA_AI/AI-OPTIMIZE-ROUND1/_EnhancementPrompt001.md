...
i am building a MQ5 EXPERT ADVISOR that queries an AI endpoint by passing MARKET DATA in json format. 
The AI endpoint passes back a trading decision.
Currently the market data is as follows and detailed in the Spec.
I would like to improve this and make it world class.
What other market data (preferably not indicator based), should i consider?
Focus on non-indicator-based market data that reflects price action, liquidity, order flow, and market microstructure.
EA modes of operation:
(a) intraday -swing trades, and
(b) intraday - scalping trades
...
timeframe: The timeframe the analysis is based on (e.g., "PERIOD_H1").
Current Price:
current_price: The closing price of the most recently closed M15 bar (rates[0].close).
Technical Indicators (Current Values & Status):
