...
A professional Grid advisor works according to RSI ( Relative Strength Index ) indicators.

And has the function of Reducing the Drawdown on the account by overlapping unprofitable orders.

The chart displays information about the profit.

It can be a very useful tool, For manual trading.
...
input   ENUM_STRATEGY   STRATEGY                 = 1;
input bool VOLATILITY_DERIVED_METHOD=true;
input bool LINEAR_METHOD=false;

input group "RSI SETTINGS"
//---
//input int               RSI_PERIOD               = 10;
input int               UP_LEVEL                 = 70;
input int               DN_LEVEL                 = 30;
...
input int volatilityPeriod = 14;  // Lookback period for volatility calculation (atr period)
input double baseGridSpacing = 50;  // Base grid spacing (in points)
input double volatilityMultiplier = 2; // Adjust multiplier to fine-tune sensitivity (default 0.5)
...
//ExtTrade.LogLevel(LOG_LEVEL_ERRORS);           // logging level
   ...
   rsihndlr=iRSI(_Symbol, RSI_TIMEFRAME, RSI_PERIOD, InpPrice);
   if(rsihndlr==INVALID_HANDLE)
     {
      Print("Error creating rsi indicator");
      return(INIT_FAILED);
     }
   ChartIndicatorAdd(0,1,rsihndlr);
...
double GETRSI(int index)
  {
   ...
   if(CopyBuffer(rsihndlr, 0, index, 1, indicator_values)<0)
     {
   ...
   if(WORKING_HOURS(START_TIME,END_TIME)==true)
     {
      ...
      double atrValue = iATR(_Symbol, RSI_TIMEFRAME, volatilityPeriod);
         ...
         if(BUYS>0)
           {
            ...
            double dynamicGridSpacing = baseGridSpacing + atrValue * volatilityMultiplier;
            double STEP = dynamicGridSpacing;
            ...
            GRID_BUY_PRICE = NormalizeDouble(LAST_BUY_PRICE-STEP*_Point,Digits());

            //Print("ATR BASED BUY:  "+GRID_BUY_PRICE);


            if(SymbolInfoDouble(_Symbol,SYMBOL_ASK)<=GRID_BUY_PRICE
               && CLOSE_ALL==false && CLOSE_BUY==false)
              {
               ORDER_SEND(OP_BUY,LOT,ORDERS_COMMENT,MAGIC_NUMBER);
              }
           }
         ...
         if(SELLS>0)
           {
            ...
            GRID_SELL_PRICE = NormalizeDouble(LAST_SELL_PRICE+STEP*_Point,Digits());
...
//  }
  ...
  void  TOTAL_VALUE (double &PROFIT_BUY,double &LOTS_BUY,double &PROFIT_SELL,double &LOTS_SELL, int MAGIC=-1){ 
   ...
   for(int v = PositionsTotal() - 1; v >= 0; v--){
      ...
      double profitforpos=NormalizeDouble(PositionGetDouble(POSITION_PROFIT), 2);
      double posLots= NormalizeDouble(PositionGetDouble(POSITION_VOLUME), 2);
      if(PositionSelectByTicket(positionticket)){
         if(SYMB==_Symbol && magic ==MAGIC ){            
            //total_open_positions ++ ;

            //==buy positions
            if(PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_BUY){
                  LOTS_BUY+=posLots;
                  PROFIT_BUY+=profitforpos;
            }

            //==sell positions
            if(PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_SELL){
                  LOTS_SELL+=posLots;
                  PROFIT_SELL+=profitforpos;
            }
           }
        }
     }
...
void DRAW_BUTTON()
  {
   ...
   if(ExtSymbolInfo.Bid()!=LAST_BID && ExtSymbolInfo.Bid()>LAST_BID)
   ...
   if(ObjectGetInteger(0,"BUTTON_+LOT",OBJPROP_STATE)==true)
     {
      ...
      ObjectSetString(0,"BUTTON_LOT",OBJPROP_TEXT,DoubleToString(LOTS,2));
      TEMP_LOT=StringToDouble(ObjectGetString(0,"BUTTON_LOT",OBJPROP_TEXT));
            ...
            if(CMD==OP_BUY && OrderType()==OP_BUY)
              {
               ...
               ZEROLEVEL_BUY=NormalizeDouble(ExtSymbolInfo.Bid()-(BUY_PROFIT/(TICKVALUE*BUY_LOTS)*_Point),_Digits);

               BREAKEVENSTOP_BUY = NormalizeDouble(ZEROLEVEL_BUY+(BREAKEVEN_STOP+BREAKEVEN_STEP)*_Point,_Digits);

               if(ExtSymbolInfo.Bid()>=BREAKEVENSTOP_BUY)
            ...
            if(CMD==OP_SELL && OrderType()==OP_SELL)
              {
               ...
               ZEROLEVEL_SELL=NormalizeDouble(ExtSymbolInfo.Ask()+(SELL_PROFIT/(TICKVALUE*SELL_LOTS)*_Point),_Digits);

               BREAKEVENSTOP_SELL = NormalizeDouble(ZEROLEVEL_SELL-(BREAKEVEN_STOP+BREAKEVEN_STEP)*_Point,_Digits);

               if(ExtSymbolInfo.Ask()<=BREAKEVENSTOP_SELL)
            ...
            if(CMD==OP_BUY && OrderType()==OP_BUY)
              {
               ...
               if(TAKEPROFIT>0 && TAKEPROFIT_BUY!=0 && ExtSymbolInfo.Bid()>=TAKEPROFIT_BUY)
                  CLOSE_BUY=true;

               if(STOPLOSS>0 && STOPLOSS_BUY!=0 && ExtSymbolInfo.Bid()<=STOPLOSS_BUY)
                  CLOSE_BUY=true;
              }
            ...
            if(CMD==OP_SELL && OrderType()==OP_SELL)
              {
               ...
               if(TAKEPROFIT>0 && TAKEPROFIT_SELL!=0 && ExtSymbolInfo.Ask()<=TAKEPROFIT_SELL)
                  CLOSE_SELL=true;

               if(STOPLOSS>0 && STOPLOSS_SELL!=0 && ExtSymbolInfo.Ask()>=STOPLOSS_SELL)
                  CLOSE_SELL=true;
              }
           }
        }
