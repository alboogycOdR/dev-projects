...
input int             magic=111111142193;

input string          comment="TDI_GENERIC_99923";
...
//--- create handle of the indicator iADX
   handle_iADX=iADX(Symbol(),Period(),15);
...
void OnTick()
  {
   ...
   double ADXDIPC,ADXDIMC,ADXDMAINC,ADXDMAIN_BEFORE;
   double ADXDIP_BEFORE,ADXDIMC_BEFORE;

   ADXDMAINC = iADXGet(MAIN_LINE,1);
   ADXDMAIN_BEFORE = iADXGet(MAIN_LINE,2);


   ADXDIPC = iADXGet(PLUSDI_LINE, 1);
   ADXDIP_BEFORE=iADXGet(PLUSDI_LINE, 2);

   ADXDIMC = iADXGet(MINUSDI_LINE, 1);
   ADXDIMC_BEFORE = iADXGet(MINUSDI_LINE, 2);
...
//Trailing();
   ...
   double tdi_green_s1,tdi_green_s1_minus_one,
          tdi_green_s2,tdi_red,tdi_green_s2_minus_one,
          tdi_red_minus_one,
          tdi_yellow;
   /*
      VB HIGH  -- BLUE
...
//(tdi_green_s2_minus_one>=tdi_red_minus_one) &&
   ...
   )
     {
      ...
      req.symbol      = _Symbol;
      req.magic       = magic;
      req.volume      = 150;//fixed_lots;//SymbolInfoDouble(Symbol(),SYMBOL_VOLUME_LIMIT);//fixed_lots;
...
//(tdi_green_s2_minus_one<=tdi_red_minus_one) &&
   ...
   )
     {
      ...
      req.symbol      = _Symbol;
      req.magic       = magic;
      req.volume      =150;//fixed_lots;//SymbolInfoDoufble(Symbol(),SYMBOL_VOLUME_LIMIT);//fixed_lots;
         ...
         if(m_position.Symbol()==m_symbol.Name() && m_position.Magic()==magic)
           {
            ...
            if(m_position.PositionType()==POSITION_TYPE_BUY)
              {
               Print("BUY m_position.PriceCurrent()-m_position.PriceOpen()>ExtTrailingStop+ExtTrailingStep:"+(m_position.PriceCurrent()-m_position.PriceOpen()>ExtTrailingStop+ExtTrailingStep));
               if(m_position.PriceCurrent()-m_position.PriceOpen()>ExtTrailingStop+ExtTrailingStep)
                  Print("buy m_position.StopLoss()<m_position.PriceCurrent()-(ExtTrailingStop+ExtTrailingStep):"+(m_position.StopLoss()<m_position.PriceCurrent()-(ExtTrailingStop+ExtTrailingStep)));
