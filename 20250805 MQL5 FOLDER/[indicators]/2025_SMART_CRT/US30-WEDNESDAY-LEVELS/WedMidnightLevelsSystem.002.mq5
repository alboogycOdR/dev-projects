...
public:
   ...
   bool              is_drawn;
       ...
       void              Draw()
    {
       ...
       // Create end label at current time using OBJ_TEXT for better positioning
       // ALWAYS create end label for every level
       string end_label_name = obj_name + "_end_label_" + DoubleToString(price, _Digits) + "_" + (string)time;
       // Force recreation of end label to ensure it exists
       ...
       // Create text object positioned directly at the current time and price level
       ObjectCreate(0, end_label_name, OBJ_TEXT, 0, TimeCurrent(), price);
       ...
       ObjectSetInteger(0, end_label_name, OBJPROP_ANCHOR, ANCHOR_LEFT_UPPER);
       ObjectSetInteger(0, end_label_name, OBJPROP_BACK, false);
       ObjectSetInteger(0, end_label_name, OBJPROP_SELECTABLE, false);
       ObjectSetInteger(0, end_label_name, OBJPROP_HIDDEN, true);
       ...
       if(InpShowLabels)
       {
        ...
        //      ObjectSetString(0, label_name, OBJPROP_TEXT, date_str + " " + level_id + " " + DoubleToString(price, _Digits));
        //      ObjectSetInteger(0, label_name, OBJPROP_XDISTANCE, 5); // Distance from left edge
        //      ObjectSetInteger(0, label_name, OBJPROP_YDISTANCE, 5); // Distance from top edge
        //      ObjectSetInteger(0, label_name, OBJPROP_CORNER, CORNER_LEFT_UPPER);
        //      ObjectSetInteger(0, label_name, OBJPROP_ANCHOR, ANCHOR_LEFT_UPPER);
        //      ObjectSetInteger(0, label_name, OBJPROP_BGCOLOR, props.line_color);
        //      ObjectSetInteger(0, label_name, OBJPROP_BORDER_COLOR, props.line_color);
        //      ObjectSetInteger(0, label_name, OBJPROP_COLOR, IsColorDark(props.line_color) ? clrWhite : clrBlack);
        //      ObjectSetInteger(0, label_name, OBJPROP_BACK, false);
        //      ObjectSetInteger(0, label_name, OBJPROP_TIME, 0, time);
       ...
       void              Update()
    {
       if(is_drawn && InpExtendLines)
       {
          ...
          // Update end label position - ensure it's always at the current time
          string end_label_name = obj_name + "_end_label_" + DoubleToString(price, _Digits) + "_" + (string)time;
          if(ObjectFind(0, end_label_name) == 0)
          {
             ObjectSetInteger(0, end_label_name, OBJPROP_TIME, 0, TimeCurrent());
             ObjectSetDouble(0, end_label_name, OBJPROP_PRICE, 0, price);
          }
          else
          {
             // If label doesn't exist, recreate it
             MqlDateTime dt;
             TimeToStruct(time, dt);
             string date_str = StringFormat("%d %s", dt.day, MonthToString(dt.mon));
                ...
                const int &spread[])
{
    ...
    {
        // Update existing lines before calculating new ones
            ...
            if(CopyRates(_Symbol, PERIOD_M15, 0, bars_to_copy, m15_rates) > 0)
            {
                ...
                for(int i = ArraySize(m15_rates) - 1; i >= 0; i--)
                {
                    ...
                    if(dt.day_of_week == WEDNESDAY && sast_dt.hour == 20 && sast_dt.min == 30)
                    {
                        ...
                        {
                           CPriceLevel* high_level = new CPriceLevel(m15_rates[i].time, m15_rates[i].high, "High");
                           high_level.props = g_wed_high_props[wed_of_month-1];
                           high_level.Draw();
                           g_wednesday_levels.Add(high_level);

                           CPriceLevel* low_level = new CPriceLevel(m15_rates[i].time, m15_rates[i].low, "Low");
                           ...
                           {
                              CPriceLevel* open_level = new CPriceLevel(m15_rates[i].time, m15_rates[i].open, "Open");
