...
input int             InpEndLabelFontSize = 8;      // End Label Font Size
...
public:
   ...
   void              Draw()
   {
      ...
      if(InpShowLabels)
      {
         ...
         // *** FIX: Create the object first, then set its coordinates to ensure proper positioning ***
         ObjectCreate(0, end_label_name, OBJ_EDIT, 0, 0, 0); // Create at default (0,0)

         // Now set all properties, including the coordinates
         ...
         ObjectSetInteger(0, end_label_name, OBJPROP_BGCOLOR, props.line_color);
         ObjectSetInteger(0, end_label_name, OBJPROP_COLOR, IsColorDark(props.line_color) ? clrWhite : clrBlack);
         ObjectSetInteger(0, end_label_name, OBJPROP_BORDER_COLOR, props.line_color);
         ...
         ObjectSetInteger(0, end_label_name, OBJPROP_ANCHOR, ANCHOR_LEFT);
         ObjectSetInteger(0, end_label_name, OBJPROP_READONLY, true);
   ...
   void              Update()
   {
      if(is_drawn && InpExtendLines)
      {
         ...
         if(InpShowLabels)
         {
            ...
            {
               ObjectSetInteger(0, end_label_name, OBJPROP_TIME, 0, TimeCurrent());
               ObjectSetDouble(0, end_label_name, OBJPROP_PRICE, 0, price); // Also update price on move
            }
         }
      }
   }
